...
object MoreCheckers {
    ...
    private def printUsage(): Unit = println("Usage: java …Main <JAR file containing class files>+")
    ...
    def main(args: Array[String]): Unit = {
        ...
        results.foreach { X =>
            val (id, times) = X; println(id + "," + times.mkString(","))
        }
        results.clear();
    ...
    def analyze(jarFiles: Array[String]): Unit = {
        ...
        time {
            for (classFile <- classFiles if !classFile.isInterfaceDeclaration) {
                ...
                if (privateFields.size > 0) unusedFields = (classFile, privateFields) :: unusedFields
            }
