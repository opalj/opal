...
object SimpleProjectStatistics extends ProjectAnalysisApplication {
    ...
    override def doAnalyze(
        ...
        val classFilesDistribution = project.allClassFiles.groupBy(cf =>
            org.opalj.bi.jdkVersion(cf.majorVersion)).toSeq.sortWith((l, r) => l._1 < r._1).map { e =>
            val (group, es) = e
            (group, es.size)
        }

        val maxInstanceFieldsInAClass = project.allClassFiles.map(_.fields.filter(f => !f.isStatic).size).max
        val classWithMaxInstanceFields = project.allClassFiles.find(
        ...
        val classWithMaxClassFields = project.allClassFiles.find(
        ...
        val classWithMaxMethods = project.allClassFiles.find(
