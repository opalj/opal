...
object FieldAccessInformationAnalysis extends ProjectAnalysisApplication {
    ...
    def doAnalyze(
        ...
        if (parameters.nonEmpty) {
            val Array(declaringClassName, fieldName) = parameters.head.substring(7).replace('.', '/').split(' ')
            ...
            def accessInformationToString(data: Seq[(DefinedMethod, PCs)]): String = data.map { e =>
                val (method, pcs) = e
