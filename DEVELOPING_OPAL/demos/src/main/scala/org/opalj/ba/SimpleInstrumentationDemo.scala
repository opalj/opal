...
class SimpleInstrumentationDemo {
    ...
    def main(args: Array[String]): Unit = new SimpleInstrumentationDemo().callsToString()

    def callsToString(): Unit = println("the length of the toString representation is: " + this.toString().length())

    def returnsValue(i: Int): Int =
        if (i % 2 == 0) return -1;
        else return 2;

    def playingWithTypes(a: AnyRef): Unit = a match {
        case i: Integer =>
            println("integer ")
            println(i.intValue())
