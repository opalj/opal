...
class Method(val methodId: Int, val signature: String) {
    val pcToInstances: mutable.Map[Int, Set[Instance]] =
        mutable.Map.empty[Int, Set[Instance]].withDefaultValue(Set.empty[Instance])
    val paramToInstances: mutable.Map[Int, Set[Instance]] =
        mutable.Map.empty[Int, Set[Instance]].withDefaultValue(Set.empty[Instance])
}
...
case class DefsiteData(
    objClass:               String,
    allocSiteId:            Long,
    allocSitePC:            Int,
    defsiteMethodSignature: String,
    defsitePC:              Int
)
...
case class TruePositive(
    defsiteMethod: String,
    defsitePC:     Int,
...
object GroundTruthParser {
    ...
    def parseGroundTruth(groundTruthPath: String): Dataset = {
        ...
        val instancePattern =
            new Regex("<(allocation|traceevent) methodId=\"(\\d+)\" (param|pc)=\"([\\d-]+)\" instanceId=\"(\\d+)\" class=\"([^\"]+)\"/>")
                ...
                case Some(m) =>
                    ...
                    dataset.methods.get(methodId) match {
                        case Some(method) => {
                            ...
                            val allocSite = dataset.instanceIdToAllocation.getOrElseUpdate(
                                instanceId, {
                                    val dummyAllocation = dataset.allocations.getOrElseUpdate(
                                        (method.signature, pcOrParam),
                                        DummyAllocation(method, pcOrParam, objClass)
                                    )
                                    dummyAllocation
                                }
                            )
    ...
    def generateMapping(
        groundTruth: Set[GroundTruthLogEvent],
        test:        Set[DefsiteData]
    ): (Map[Long, Set[Int]], Map[Int, Option[Long]]) = {
        val allocSiteToInstanceId = mutable.Map[Long, Set[Int]]()
        val instanceIdToAllocSite = mutable.Map[Int, Option[Long]]()
        val groundTruthDict = groundTruth.map(event =>
            ((normalizeType(event.instanceClass), event.methodSignature, event.pc), event.instanceId)
        ).toMap
        for (defsiteData <- test) {
            ...
            groundTruthDict.get((normalized, defsiteData.defsiteMethodSignature, defsiteData.defsitePC)) match {
                case Some(instanceId) => {
                    allocSiteToInstanceId.put(
                        defsiteData.allocSiteId,
                        allocSiteToInstanceId.getOrElse(defsiteData.allocSiteId, Set.empty[Int]) + instanceId
                    )
    ...
    def evaluate(
        groundTruth: Dataset,
        test:        Set[(DefinedMethod, Int, Option[AllocationSitePointsToSet])],
        project:     Project[URL]
    ): Map[Int, MethodResult] = {
        ...
        for ((methodId, method) <- groundTruth.methods) {
            ...
            for ((param, instances) <- method.paramToInstances) {
                for (instance <- instances) {
                    ...
                    groundTruthData add GroundTruthLogEvent(
                        instance.objClass,
                        instance.instanceId,
                        method.signature,
                        -2 - param
            ...
            for (pts <- sorted) {
                pts._3 match {
                    case Some(as) => for (allocSiteId <- as.elements.iterator) {
                            val (_, allocSitePC, typeId) = longToAllocationSite(allocSiteId)
                            val objClass: String =
                                if (typeId >= 0) ObjectType.lookup(typeId).toJava else ArrayType.lookup(typeId).toJava
                            testData add DefsiteData(objClass, allocSiteId, allocSitePC, methodSignature, pts._2)
                            println(
                                "<pointsto method=\"" + methodSignature + "\" pc=\"" + pts._2 + "\" type=\"" + objClass + "\" allocSiteId=\"" + allocSiteId + "\" allocSitePc=\"" + allocSitePC + "\" />"
                            )
                        }
                    case None =>
                }
        ...
        val (allocSiteToInstanceIds, _): (Map[Long, Set[Int]], Map[Int, Option[Long]]) =
            generateMapping(groundTruthData.toSet, testData.toSet)
        ...
        for ((method, testPTS) <- methodsAndPTSs.filter(m => !m._1.contains("$string_concat$"))) {
            ...
            for (defsite <- testPTS) {
                val groundTruthInstancesAtPC = groundTruthMethod.pcToInstances.getOrElse(
                    defsite.defsitePC,
                    Set.empty
                ) ++ groundTruthMethod.paramToInstances.getOrElse(-defsite.defsitePC - 2, Set.empty)

                allocSiteToInstanceIds.get(defsite.allocSiteId) match {
                    case Some(groundTruthInstanceIds) => {

                        val groundTruthInstancesForDefSite = groundTruthInstancesAtPC.filter(instance =>
                            groundTruthInstanceIds.contains(instance.instanceId)
                        )

                        if (Set(normalizeType(defsite.objClass)) equals groundTruthInstancesForDefSite.map(inst =>
                                normalizeType(inst.objClass)
                            )
                        ) {
                            val existing = testAllocationsAtPC.getOrElse(defsite.defsitePC, Set.empty)
                            testAllocationsAtPC += (defsite.defsitePC -> groundTruthInstanceIds.map(
                                groundTruth.instanceIdToAllocation
                            ).union(existing))
                            truePositiveData += TruePositive(
                                method,
                                defsite.defsitePC,
                                groundTruthInstancesForDefSite,
                                defsite.allocSiteId,
                                defsite.allocSitePC,
                                defsite.objClass
                            )
                        } else {
                            falsePositiveData += FalsePositive(
                                method,
                                defsite.defsitePC,
                                defsite.objClass,
                                defsite.allocSiteId,
            ...
            val precision =
                if (truePositive + falsePositive > 0) truePositive.toDouble / (truePositive + falsePositive) else 0
            val recall =
                if (truePositive + falseNegative > 0) truePositive.toDouble / (truePositive + falseNegative) else 0
        ...
        results.filter(result => !result._2.method.contains("<init>")).filter(testMethod =>
            project.allMethods.exists(m => m.fullyQualifiedSignature.equals(testMethod._2.method))
        ).toMap
    }
    ...
    def printTotalPrecisionRecall(results: Map[Int, MethodResult]): (Int, Int, Int, Double, Double) = {
        ...
        val precision =
            if (truePositive + falsePositive > 0) 100 * truePositive.toDouble / (truePositive + falsePositive) else 0
        val recall =
            if (truePositive + falseNegative > 0) 100 * truePositive.toDouble / (truePositive + falseNegative) else 0
...
object ComparePTS {
    def main(args: Array[String]): Unit = {
        ...
        for ((id) <- evalWithoutTAJS.keys.toList.sorted) {
            ...
            if (
                resultWithoutTAJS.falseNegative + resultWithoutTAJS.falsePositive + resultWithoutTAJS.truePositive > 0
            ) {
...
class PointsToAnalysisRunner extends ProjectAnalysisApplication {
    ...
    override def doAnalyze(
        ...
        for (m <- definedMethods) {
            ...
            if (dm.body.nonEmpty) {
                val defsitesInMethod = ps1.entities(propertyFilter = _.e.isInstanceOf[DefinitionSite]).map(
                    _.asInstanceOf[DefinitionSite]
                ).filter(_.method == dm).toSet
                val ptsInMethod = defsitesInMethod.toArray.map(defsite =>
                    (
                        m,
                        defsite.pc,
                        ps1.properties(defsite).map(_.toFinalEP.p).find(_.isInstanceOf[AllocationSitePointsToSet]).map(
                ...
                val params = ps1.entities(propertyFilter = _.e.isInstanceOf[VirtualFormalParameter]).map(
                    _.asInstanceOf[VirtualFormalParameter]
                ).filter(_.method == m).toList

                val ptsInMethodParams = params.toArray.map(param =>
                    (
                        m,
                        param.origin,
                        ps1.properties(param).map(_.toFinalEP.p).find(_.isInstanceOf[AllocationSitePointsToSet]).map(
                            _.asInstanceOf[AllocationSitePointsToSet]
                        )
                    )
                )
