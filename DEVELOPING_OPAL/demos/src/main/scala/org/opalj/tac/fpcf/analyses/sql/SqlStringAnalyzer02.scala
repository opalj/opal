...
object SqlStringAnalyzer02 {
   ...
   */
  var taintMemory = new SqlTaintMemory02(Set("TAINTED_VALUE","'TAINTED_VALUE'"))
   ...
   */
  def doAnalyze(sqlString: String, taintMemorySQL: SqlTaintMemory02): Boolean = {
  ...
  def doAnalyze(sqlString: String):Boolean = {
    ...
    if(DEBUG){
      ...
      println(s"Result of SqlAnalyzer: $result \n")
    }
    result
  }
