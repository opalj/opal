...
object TACAItoGraphs extends ProjectAnalysisApplication {
    ...
    override def checkAnalysisSpecificParameters(parameters: Seq[String]): Seq[String] =
        if (parameters.size == 1 && parameters.head.startsWith("-o=")) Seq.empty
        else if (parameters.isEmpty) Seq("output folder is missing")
        else parameters.filterNot(_.startsWith("-o=")).map("unknown parameter: " + _)
