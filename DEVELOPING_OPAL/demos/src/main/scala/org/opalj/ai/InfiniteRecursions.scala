...
object InfiniteRecursions extends ProjectAnalysisApplication {
    ...
    override def description: String = "identifies method which calls themselves using infinite recursion"
    ...
    def inifiniteRecursions(
        ...
        project:           SomeProject,
        method:            Method,
        pcs:               List[Int /*PC*/ ]): Option[InfiniteRecursion] = {
        ...
        def analyze(depth: Int, callOperands: Operands): Option[InfiniteRecursion] = {
            if (depth > maxRecursionDepth) return None;
            ...
            val callOperandsList = reduceCallOperands(operandsArray) filter { callOperands =>
                    ...
                    if (callOperands.forall {
                            case domain.DomainSingleOriginReferenceValueTag(v) =>
                                if (v.origin < 0 /* === the value is a parameter*/ ||
                                    ...
                                    body.instructions(v.origin).opcode == NEW.opcode) true
                                else false
                            case v: domain.AnIntegerValue =>
                                if (localsArray(0).exists(_ eq v)) true // the value is parameter
                                else false
