...
object IdentifyResourcesAnalysis extends ProjectAnalysisApplication {
    ...
    override def description: String = "Identifies java.io.File object instantiations using constant strings."
    ...
    override def doAnalyze(
        ...
        val callSites = time {
            (for {
                ...
                val pcs = m.body.get.foldLeft(List.empty[Int /*PC*/ ]) { (pcs, pc, instruction) =>
                    instruction match {
                        case INVOKESPECIAL(
        ...
        BasicReport(
            if (callSitesWithConstantStringParameter.isEmpty) "Only found " + callSites.size + " candidates."
            else callSitesWithConstantStringParameter.map(callSiteToString).mkString("Methods:\n", "\n", ".\n")
        )
    }
