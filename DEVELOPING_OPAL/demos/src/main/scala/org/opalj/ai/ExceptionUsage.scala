    ...
    override def doAnalyze(
        ...
        val usages = (for {
            ...
            for {
                ...
                operand match {
                    case v: result.domain.SingleOriginReferenceValue => collectExceptions(v)
            ...
            def updateUsageKindForValue(
                ...
                usageKind: UsageKind.Value): Unit =
                exceptionUsages.get((value.origin, typeName(value))).map(_ += usageKind)

            def updateUsageKind(
                value:     result.domain.DomainValue,
                usageKind: UsageKind.Value): Unit = value match {
                case v: result.domain.SingleOriginReferenceValue => updateUsageKindForValue(v, usageKind)
                case result.domain.MultipleReferenceValues(singleOriginReferenceValues) =>
            ...
            body.iterate { (pc, instruction) =>
                ...
                if (operands != null) { // the instruction is reached...
                    instruction match {
                        case ATHROW => updateUsageKind(operands.head, UsageKind.IsThrown)
                        ...
                        case _: FieldWriteAccess => updateUsageKind(operands.head, UsageKind.StoredInField)
                        case ARETURN             => updateUsageKind(operands.head, UsageKind.IsReturned)
                        case AASTORE             => updateUsageKind(operands.head, UsageKind.StoredInArray)
                        case _                   =>
                        /*nothing to do*/
                    }
                }
            ...
            val usages =
                for { ((pc, typeName), exceptionUsage) <- exceptionUsages } yield ExceptionUsage(method,
                                                                                                 pc,
                                                                                                 typeName,
                                                                                                 exceptionUsage)

            if (usages.isEmpty) None
            else {
                Some(usages)
            }
...
case class ExceptionUsage(
        ...
        usageInformation: scala.collection.Set[UsageKind.Value]) extends scala.math.Ordered[ExceptionUsage] {
    ...
    override def toString: String = {
        ...
        method.toJava(
            usageInformation.mkString(
                s"$BOLD[$lineNumber;pc=$definitionSite]$exceptionType => ",
                ", ",
                RESET
            )
        )
...
class ExceptionUsageAnalysisDomain(val project: Project[java.net.URL], val method: Method)
    ...
    def throwExceptionsOnMethodCall: ExceptionsRaisedByCalledMethods.Value = ExceptionsRaisedByCalledMethods.Any
