...
object IfNullParameterAnalysis extends ProjectAnalysisApplication {
    ...
    override def title: String = "Identifies methods that are sensitive to parameters that are \"null\""

    override def description: String = "Identifies methods that exhibit different behavior w.r.t. " +
    ...
    override def doAnalyze(
        ...
        def setToNonNull(domain: DefaultDomain[URL])(locals: domain.Locals): domain.Locals = locals.map { value =>
            if (value == null)
        ...
        val methodsWithDifferentExceptions = time {
            for {
                ...
                val domain1 = new DefaultDomain(theProject, method) with domain.RecordAllThrownExceptions
                ...
                val domain2       = new DefaultDomain(theProject, method) with domain.RecordAllThrownExceptions
                ...
                domain1.allThrownExceptions.foreach { e =>
                    ...
                    if (d2ThrownException.isDefined) {
                        val adaptedD2ThrownException = d2ThrownException.get.map(ex =>
                            ex.adapt(
                                domain1,
                        ...
                        val diff = d1thrownException.diff(adaptedD2ThrownException) ++
                            adaptedD2ThrownException.diff(d1thrownException)
                        if (diff.nonEmpty) result = result.updated(pc, diff)
