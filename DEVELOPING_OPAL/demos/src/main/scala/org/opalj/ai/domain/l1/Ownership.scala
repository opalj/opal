...
object OwnershipAnalysis extends ProjectAnalysisApplication {
    ...
    override def title: String = "basic ownership analysis for arrays"

    override def description: String = "a very basic ownership analysis for arrays"
        ...
        val classes = for {
            ...
            ownershipViolatingMethods = publicAndProtectedMethods.collect {
                                            case method if {
                                                    ...
                                                    val aiResult = BaseAI(
                                                        method,
                                                        new DefaultDomain(theProject, method)
                                                            with RecordLastReturnedValues
                                                    )
                                                    import aiResult.domain
                                                    if (method.returnType.isReferenceType) {
                                                        isOwner = domain.allReturnedValues.forall { kv =>
                                                            ...
                                                            def checkValue(
                                                                returnValue: domain.DomainSingleOriginReferenceValue)
