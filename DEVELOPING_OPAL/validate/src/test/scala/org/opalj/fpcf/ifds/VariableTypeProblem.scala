...
case class CalleeType(line: Int, t: ReferenceType, upperBound: Boolean) extends VTAFact {
    ...
    override def subsumes(other: AbstractIFDSFact, project: SomeProject): Boolean =
        if (upperBound) other match {
            case CalleeType(lineOther, tOther, _) if line == lineOther => project.classHierarchy.isSubtypeOf(tOther, t)
            case _                                                     => false
        }
        else false
}
...
class VariableTypeProblem(project: SomeProject, override val subsumeFacts: Boolean = false)
    ...
    override def normalFlow(
        ...
        in:          VTAFact,
        predecessor: Option[JavaStatement]): Set[VTAFact] = {
        val inSet = Set(in)
    ...
    override def callFlow(start: JavaStatement, in: VTAFact, call: JavaStatement, callee: Method): Set[VTAFact] = {
        ...
        val flow = scala.collection.mutable.Set.empty[VTAFact]
        inSet.foreach {
            case VariableType(definedBy, t, upperBound) => allParams.iterator.zipWithIndex.foreach {
                    /*
    ...
    override def outsideAnalysisContextCall(callee: Method): Option[OutsideAnalysisContextCallHandler] =
        if (classInsideAnalysisContext(callee.classFile) &&
            super.outsideAnalysisContextCall(callee).isEmpty) None
        else {
            Some((
                (call: JavaStatement,
                    successor: Option[JavaStatement],
    ...
    private def newFacts(
        ...
        statementIndex: Int,
        in:             VTAFact): Iterator[VariableType] = {
        val inSet = Set(in)
        expression.astID match {
            case New.ASTID => inSet.iterator.collect {
                    // When a constructor is called, we always know the exact type.
                    case VTANullFact => VariableType(statementIndex, expression.asNew.tpe, upperBound = false)
                }
            case Var.ASTID => inSet.iterator.collect {
            ...
            case ArrayLoad.ASTID => inSet.iterator.collect {
            ...
            case GetField.ASTID | GetStatic.ASTID =>
                 ...
                 * compile time type.
                 */
                if (t.isReferenceType) Iterator(VariableType(statementIndex, t.asReferenceType, upperBound = true))
                else Iterator.empty
    ...
    @tailrec private def isArrayOfObjectType(
        t:                 FieldType,
        includeObjectType: Boolean = false): Boolean =
     ...
     */
    private def isEntryPoint(method: Method): Boolean = method.body.isDefined && canBeCalledFromOutside(method)
