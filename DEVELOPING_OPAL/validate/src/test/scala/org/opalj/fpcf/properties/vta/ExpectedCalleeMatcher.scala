...
class ExpectedCalleeMatcher extends VTAMatcher {
    ...
    def validateSingleAnnotation(
        ...
        val allReachableExitFacts = propertyStore.entities(propertyKey).collect {
            case EPS((m: Method, inputFact)) if m == method => (m, inputFact)
        }.flatMap(propertyStore(_, propertyKey) match {
            case FinalEP(_, VTAResult(result, _)) => result.values.fold(Set.empty)((acc, facts) => acc ++ facts).collect {
                    case CalleeType(index, t, upperBound) => Seq((
