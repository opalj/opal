...
/**
 * Annotation to state that the allocation site or parameter escapes via the method return,
 * via an exception and a parameter, but it could escape any further (if a proper analysis was
 * scheduled).
 *
 * @author Florian Kuebler
 */
@PropertyValidator(key = "EscapeProperty",
        validator = AtMostEscapeViaParameterAndNormalAndAbnormalReturnMatcher.class)
@Target({ TYPE_USE, PARAMETER })
@Documented
@Retention(RetentionPolicy.CLASS)
public @interface AtMostEscapeViaParameterAndNormalAndAbnormalReturn {
