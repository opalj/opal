    ...
    def main(args: Array[String]): Unit = {
        ...
        val doIdentifyDeadVariables =
            if (remainingArgs.nonEmpty && remainingArgs.head.startsWith("-identifyDeadVariables=")) {
                val result = remainingArgs.head == "-identifyDeadVariables=true" ||
        ...
        val method = (
            if (methodName.contains("(")) classFile.methods.find(m => m.descriptor.toJava(m.name).contains(methodName))
            else classFile.methods.find(_.name == methodName)
        ) match {
            case Some(method) =>
                if (method.body.isDefined) method
                else {
                    println(RED +
        ...
        try {
                ...
                dump(
                    ...
                    Some(
                        s"Analyzed: ${new Date}<br>Domain: ${domainClass.getName}<br>" +
                            (
                                if (doIdentifyDeadVariables) "<b>Dead Variables Identification</b><br>"
                                else "<u>Dead Variables are not filtered</u>.<br>"
                            ) +
            ...
            case ife: InterpretationFailedException =>
                ...
                def causeToString(ife: InterpretationFailedException, nested: Boolean): String = {
                    ...
                    val parameters =
                        if (nested) {
                            ife.localsArray(0).toSeq.reverse.filter(_ != null).map(_.toString).mkString(
                                "Parameters:<i>",
                                ", ",
                                "</i><br>")
                        } else ""

                    val aiState = s"<p><i>${domain.getClass.getName}</i><b>( ${domain.toString} )</b></p>" +
                        parameters +
                        "Current instruction: " + ife.pc + "<br>" +
                        XHTML.evaluatedInstructionsToXHTML(ife.evaluatedPCs) + {
                            if (ife.worklist.nonEmpty) ife.worklist.mkString("Remaining worklist:\n<br>", ", ", "<br>")
                            else "Remaining worklist: <i>EMPTY</i><br>"
                        }
                    ...
                    val metaInformation = ife.cause match {
                        ...
                        case e: Throwable =>
                            ...
                            if (message != null) aiState + "<br>Underlying cause: " + util.XHTML.htmlify(message)
                            else aiState + "<br>Underlying cause: <NULL>"
                        case _ => aiState
                    }

                    if (nested && ife.domain.isInstanceOf[TheCode]) metaInformation +
                        dumpAIState(
