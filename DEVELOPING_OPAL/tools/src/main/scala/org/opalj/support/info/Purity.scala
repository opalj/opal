...
object Purity {
    ...
    def usage: String = "Usage: java …PurityAnalysisEvaluation \n" +
    ...
    def evaluate(
        ...
        packages:              Option[Array[String]]): Unit = {
        ...
        val allMethods: Iterable[DefinedMethod] = for {
        ...
        val reachableMethods = ps.entities(Callers.key).collect {
        ...
        if (projectEvalDir.isDefined) {
            ...
            try {
                if (resultsNew) {
                    ...
                    if (!individual) resultsWriter.println("analysisName;compile time pure;pure;" +
    ...
    def main(args: Array[String]): Unit = {
        ...
        escapeAnalysisName match {
            case Some("L0") => support ::= LazySimpleEscapeAnalysis

            case None | Some("L1") => support ::= LazyInterProceduralEscapeAnalysis
        ...
        fieldAssignabilityAnalysisName match {
            ...
            case Some("L2") if eager => support ::= EagerL2FieldAssignabilityAnalysis

            case Some("L2") => support ::= LazyL2FieldAssignabilityAnalysis
        ...
        val d =
            if (domainName.isEmpty) classOf[domain.l2.DefaultPerformInvocationsDomainWithCFGAndDefUse[_]]
            else {
