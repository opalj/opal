...
trait ConsoleTracer extends AITracer { tracer =>
    ...
    private def localsToString(domain: Domain)(locals: domain.Locals): String =
        if (locals eq null) "\tlocals: not available (null);\n"
        else locals.zipWithIndex.map { vi =>
            val (v, i) = vi
            s"$i:" + (
                if (v == null) correctIndent("-", false)
                else correctIndent(v, printOIDs)
            )
        }.mkString("\tlocals:\n\t\t", "\n\t\t", "\n")

    def initialLocals(
        domain: Domain
      )(locals: domain.Locals): Unit = println(localsToString(domain)(locals))

    private def correctIndent(value: Object, printOIDs: Boolean): String = {
        if (value eq null) return "<EMPTY>";

        def toString(value: Object) = value.toString.replaceAll("\n\t", "\n\t\t\t").replaceAll("\n\\)", "\n\t\t)")
    ...
    private def line(domain: Domain, pc: Int): String = domain match {
    ...
    override def instructionEvalution(
        ...
        val os =
            if (operands eq null) "\toperands: not available (null);\n"
            else {
                val os = operands.map { o => correctIndent(o, printOIDs) }
                if (os.isEmpty) "\toperands <NONE>;\n"
                else os.mkString("\toperands:\n\t\t", "\n\t\t", "\n\t;\n")
            }
    ...
    override def rescheduled(
        ...
        worklist:                 List[Int /*PC*/ ]): Unit = println(
    ...
    override def deadLocalVariable(domain: Domain)(pc: Int, lvIndex: Int): Unit = println(
        pc.toString + line(domain, pc).toString + ":" +
            Console.BLACK_B + Console.WHITE + s"local variable $lvIndex is dead"
    )

    override def noFlow(domain: Domain)(currentPC: Int, targetPC: Int): Unit = println(Console.RED_B + Console.YELLOW +
            ...
            case u @ SomeUpdate((updatedOperands, updatedLocals)) =>
                ...
                println(
                    thisOperands.zip(otherOperands).zip(updatedOperands).map { v =>
                        ...
                        val s =
                            if (thisOp eq updatedOp) "✓ " + Console.GREEN + updatedOp.toString
                            else {
                ...
                println(
                    thisLocals.zip(otherLocals).zip(updatedLocals.iterator).map(v =>
                        (v._1._1, v._1._2, v._2)).zipWithIndex.map(v => (v._2, v._1._1, v._1._2, v._1._3)).filterNot(
                        v => (v._2 eq null) && (v._3 eq null)).map(v =>
                        s"${v._1}" + {
                            if (v._2 == v._3)
                                if (v._2 eq v._3) Console.GREEN + ": ✓ "
                                else Console.YELLOW + ":(✓) " +
                                    oidString(v._2) +
                                    " join " +
                                    oidString(v._3) +
                            ...
                            correctIndent(v._4, printOIDs)).mkString("\tLocals:\n\t\t",
                                                                     "\n\t\t" + Console.BLUE,
                                                                     Console.BLUE)
                )
        }
    ...
    override def establishedConstraint(
        ...
        val changedLocals = locals.zip(newLocals).zipWithIndex.map(localsWithIdx =>
    ...
    override def abruptMethodExecution(
        ...
        exception: domain.ExceptionValue): Unit = println(
    ...
    override def returnFromSubroutine(
        ...
        returnAddress: Int,
        subroutinePCs: List[Int /*PC*/ ]): Unit = println(
    ...
    override def abruptSubroutineTermination(
        ...
        newWorklist:                List[Int /*PC*/ ]): Unit = println(
