...
object PTSTracerInstrumentation {
    ...
    def main(args: Array[String]): Unit = {
                        ...
                        case Some(code) =>
                                ...
                                lCode.insert(
                                    ...
                                    Seq(
                                        ...
                                        INVOKESTATIC(
                                            PTSLoggerType,
                                            isInterface = false,
                                            "logParameterInstance",
                                            MethodDescriptor(
                                                ArraySeq(ObjectType.Object, IntegerType, IntegerType),
                                                VoidType
                                            )
                                        )
                                    )
                            ...
                            for {
                                ...
                                inst match {
                                    ...
                                    case GETFIELD(_, _, fieldType) => {
                                        if (!fieldType.isBaseType)
                                            insertDefsiteLog = true
                                    }
                                    case GETSTATIC(_, _, fieldType) => {
                                        if (!fieldType.isBaseType)
                                            insertDefsiteLog = true
                                    }
                                    case load: LDC[_] => {
                                    ...
                                    case MethodInvocationInstruction(_, _, _, methodDescriptor) => {
        ...
        val ContainerClassTypeFile = PTSTracerInstrumentation.readFile(
            outputClassPath + "/org/opalj/fpcf/fixtures/xl/js/testpts/SimpleContainerClass.class"
        )
        Thread.currentThread().setContextClassLoader(classloaderScriptEngineJars);

        val cl = new InMemoryClassLoader(
            testCases.toMap ++ otherClasses.toMap ++ Map(
        ...
        for ((className, code) <- testCases) {
            ...
            if (main != null) {
                val args: Array[String] =
                    Array("/home/julius/Downloads/test.jpg", "/home/julius/Downloads/testout.jpg", "-custom")
                try {
