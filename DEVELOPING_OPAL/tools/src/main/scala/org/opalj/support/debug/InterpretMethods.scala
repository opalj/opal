...
object InterpretMethods extends AnalysisApplication {
    ...
    override def checkAnalysisSpecificParameters(parameters: Seq[String]): Iterable[String] = {
        def isDomainParameter(parameter: String) = parameter.startsWith("-domain=") && parameter.length() > 8
        def isVerbose(parameter:         String) = parameter == "-verbose=true" || parameter == "-verbose=false"

        parameters match {
            case Nil => Iterable.empty
            case Seq(parameter) =>
                if (isDomainParameter(parameter) || isVerbose(parameter)) Iterable.empty
                else Iterable("unknown parameter: " + parameter)
            case Seq(parameter1, parameter2) =>
                ...
                else if (!isVerbose(parameter2)) Seq("the second parameter has to be \"verbose\": " + parameter2)
                else Iterable.empty
...
object InterpretMethodsAnalysis {
    ...
    def interpret[Source](
        ...
        logContext: LogContext): (String, Option[File]) = {
        ...
        if (collectedExceptions.nonEmpty) {
            ...
            val body = Seq(header) ++
                (for ((exResource, exInstances) <- collectedExceptions.groupBy(e => e._1)) yield {
                    val exDetails = exInstances.map { ex =>
