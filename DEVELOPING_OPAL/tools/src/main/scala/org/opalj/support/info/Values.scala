...
object Values extends ProjectAnalysisApplication {
    ...
    override def doAnalyze(
        ...
        val (ps, _) = PerformanceEvaluation.time {
        ...
        val objectValuesReturningMethodsCount = project.allMethodsWithBody.filter(_.returnType.isObjectType).size

        val mMethods = methodReturnValues
            .filter { eps =>
