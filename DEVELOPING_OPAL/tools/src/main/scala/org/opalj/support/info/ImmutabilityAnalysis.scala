...
object ImmutabilityAnalysis extends ProjectAnalysisApplication {
    ...
    override def doAnalyze(
        ...
        val immutableClasses = ps.entities(ClassImmutability.key).toSeq.filter(ep =>
            project.classHierarchy.isInterface(ep.e.asInstanceOf[ObjectType]).isNo).groupBy { _.ub }.map { kv =>
            (
                kv._1,
        ...
        val immutableTypes = ps.entities(TypeImmutability.key).toSeq.filter(ep =>
            project.classHierarchy.isInterface(ep.e.asInstanceOf[ObjectType]).isNo).groupBy { _.ub }.map { kv =>
            (kv._1, kv._2.size)
        }
        ...
        val immutableClassesInfo = immutableClasses.values.flatten
