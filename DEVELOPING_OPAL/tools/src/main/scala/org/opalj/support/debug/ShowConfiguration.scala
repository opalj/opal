...
object ShowConfiguration {
    ...
    def renderConfig(config: Config): String = {
        ...
        val renderingOptions        = defaultRenderingOptions.setOriginComments(false).setComments(true).setJson(false)
    ...
    def main(args: Array[String]): Unit = {
        ...
        javaSecurity.stringPropertyNames().asScala.foreach { property =>
            ...
            if (entry.contains(",")) println(entry.split(",").mkString("\t" + property + "=\n\t\t", ",\n\t\t", ""))
            else println(s"\t$property=$entry")
        }
    }
}
