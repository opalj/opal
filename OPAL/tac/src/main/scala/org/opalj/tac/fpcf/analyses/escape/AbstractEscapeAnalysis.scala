...
trait AbstractEscapeAnalysis extends FPCFAnalysis {
     ...
     */
    protected[this] def doDetermineEscape(implicit
        context: AnalysisContext,
        state:   AnalysisState): ProperPropertyComputationResult = {
    ...
    private[this] def checkStmtForEscape(
        ...
        context: AnalysisContext,
        state:   AnalysisState): Unit = (stmt.astID: @switch) match {
        ...
        case ReturnValue.ASTID =>
            if (state.usesDefSite(stmt.asReturnValue.expr)) state.meetMostRestrictive(EscapeViaReturn)

        case PutField.ASTID => handlePutField(stmt.asPutField)

        case ArrayStore.ASTID => handleArrayStore(stmt.asArrayStore)

        case Throw.ASTID => handleThrow(stmt.asThrow)

        case StaticMethodCall.ASTID => handleStaticMethodCall(stmt.asStaticMethodCall)

        case VirtualMethodCall.ASTID => handleVirtualMethodCall(stmt.asVirtualMethodCall)

        case NonVirtualMethodCall.ASTID => handleNonVirtualMethodCall(stmt.asNonVirtualMethodCall)

        case InvokedynamicMethodCall.ASTID => handleInvokedynamicMethodCall(stmt.asInvokedynamicMethodCall)

        case ExprStmt.ASTID => handleExprStmt(stmt.asExprStmt)
    ...
    protected[this] def handleExprStmt(
        ...
        context: AnalysisContext,
        state:   AnalysisState): Unit = handleExpression(exprStmt.expr, hasAssignment = false)
    ...
    protected[this] def handleAssignment(
        ...
        context: AnalysisContext,
        state:   AnalysisState): Unit = handleExpression(assignment.expr, hasAssignment = true)
    ...
    protected[this] def handleExpression(
        ...
        context: AnalysisContext,
        state:   AnalysisState): Unit = (expr.astID: @switch) match {
        case NonVirtualFunctionCall.ASTID => handleNonVirtualFunctionCall(expr.asNonVirtualFunctionCall, hasAssignment)
        case VirtualFunctionCall.ASTID    => handleVirtualFunctionCall(expr.asVirtualFunctionCall, hasAssignment)
        case StaticFunctionCall.ASTID     => handleStaticFunctionCall(expr.asStaticFunctionCall, hasAssignment)
     ...
     */
    protected[this] def returnResult(implicit
        context: AnalysisContext,
        state:   AnalysisState): ProperPropertyComputationResult =
    ...
    protected[this] def c(
        ...
        context: AnalysisContext,
        state:   AnalysisState): ProperPropertyComputationResult = someEPS match {
        case UBP(ub: TACAI) =>
        ...
        case _ => throw new UnknownError(s"unhandled escape property (${someEPS.ub} for ${someEPS.e}")
    }
     ...
     */
    def determineEscape(e: Entity): ProperPropertyComputationResult = e.asInstanceOf[(Context, Entity)]._2 match {
        case _: DefinitionSiteLike     => determineEscapeOfDS(e.asInstanceOf[(Context, DefinitionSiteLike)])
        case _: VirtualFormalParameter => determineEscapeOfFP(e.asInstanceOf[(Context, VirtualFormalParameter)])
        case _                         => throw new IllegalArgumentException(s"$e is unsupported")
    }

    protected[this] def determineEscapeOfDS(
        dsl: (Context, DefinitionSiteLike)): ProperPropertyComputationResult = {
        val ctx = createContext(dsl, dsl._2.method)
        doDetermineEscape(ctx, createState)
    }

    protected[this] def determineEscapeOfFP(
        fp: (Context, VirtualFormalParameter)): ProperPropertyComputationResult
    ...
    protected[this] def createContext(
        ...
        targetMethod: Method): AnalysisContext
}
