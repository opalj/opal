...
class L2FieldAssignabilityAnalysis private[analyses] (val project: SomeProject)
    ...
    val considerLazyInitialization: Boolean = project.config.getBoolean(
    ...
    private def checkWriteDominance(
        ...
        if (writesInMethod.distinctBy(_._2).size > 1) return true; // Field is written in multiple locations, thus must be assignable

        // If we have no information about the receiver, we soundly return
        if (receiverVar.isEmpty) return true;

        val assignedValueObject = receiverVar.get
        if (assignedValueObject.definedBy.exists(_ < 0)) return true;
        ...
        if (fraiEP.hasUBP && fieldReadsNotDominated(fraiEP.ub, 0, 0, Seq(writeAccess))) return true;
    ...
    override def c(eps: SomeEPS)(implicit state: State): ProperPropertyComputationResult = eps.pk match {
    ...
    private def fieldReadsNotDominated(
      ...
      )(implicit state: State): Boolean = writes.exists { writeAccess =>
    ...
    case class State(
            field: Field) extends AbstractFieldAssignabilityAnalysisState {
    ...
    def dominates(
        ...
        taCode:                    TACode[TACMethodParameter, V]): Boolean = {
    ...
    def determineLazyInitialization(
        ...
        if (hasMultipleNonConstructorWrites(method)) return Assignable;
         ...
         */
        def noInterferingExceptions(): Boolean = resultCatchesAndThrows._1.forall {
            case (catchPC, originsCaughtException) => resultCatchesAndThrows._2.exists {
        ...
        if (findGuardsResult.isEmpty) return Assignable;
        ...
        if (!dominates(defaultCaseIndex, writeIndex, taCode)) return Assignable;

        val elseBB = cfg.bb(elseCaseIndex)

        // prevents wrong control flow
        if (isTransitivePredecessor(elseBB, writeBB)) return Assignable;
        ...
        if (method.returnType == state.field.fieldType) {
            ...
            if (!isFieldValueReturned(write, writeIndex, readIndex, taCode, findGuardsResult)) return Assignable;
                 ...
                 taCode.stmts.exists(stmt =>
                     stmt.isReturnValue && !isTransitivePredecessor(
                         ...
                         findGuardsResult.forall { // TODO check...
                             case (indexOfFieldRead, _, _, _) => !isTransitivePredecessor(
        ...
        if (write.value.asVar.definedBy.forall { _ >= 0 } &&
            ...
            if (method.isSynchronized) LazilyInitialized
            else {
                ...
                if (monitorResultsDefined && dominates(indexMonitorEnter.get, readIndex, taCode)) LazilyInitialized
                else UnsafelyLazilyInitialized
            }
        } else Assignable
    }
    ...
    def doFieldReadsEscape(
        ...
        if (reads.exists(r => contextProvider.contextFromId(r._1).method.definedMethod ne method)) return true;
    ...
    def findCatchesAndThrows(
        tacCode: TACode[TACMethodParameter, V]): (List[(Int, IntTrieSet)], List[(Int, IntTrieSet)]) = {
    ...
    def findMonitors(
         ...
         */
        def checkMonitor(v: V)(implicit state: State): Boolean = v.definedBy.forall { definedByIndex =>
            if (definedByIndex >= 0) {
    ...
    def findGuards(
        ...
        result.foreach { result =>
            ...
            if (definitions.forall(_ >= 0)) {
                ...
                val fieldReadUsedCorrectly = fieldReadUses.forall(use => use == result._1 || use == result._3)
    ...
    def isGuard(
        ...
        def isGuardInternal(
            expr:    V,
            tacCode: TACode[TACMethodParameter, V]): Boolean = expr.definedBy forall { index =>
            if (index < 0) false // If the value is from a parameter, this can not be the guard
                ...
                } else isReadOfCurrentField(expression, tacCode, index)
            }
        }
    ...
    def isFieldValueReturned(
        ...
        def isSimpleReadOfField(expr: Expr[V]) = expr.astID match {
