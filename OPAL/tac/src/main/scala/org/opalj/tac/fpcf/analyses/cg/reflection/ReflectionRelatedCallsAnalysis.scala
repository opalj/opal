...
class ClassForNameAnalysis private[analyses] (
    ...
    private class State(
        ...
        callContext:     ContextType,
        val callPC:      Int
    ) extends CGState[ContextType](callContext) {
        var hasFailed = false

        private var _loadedClassesUB: UIDSet[ClassType] = loadedClassesUB
        private var _newLoadedClasses: UIDSet[ClassType] = UIDSet.empty
        ...
        def loadedClassesPartialResult: PartialResult[SomeProject, LoadedClasses] = {
            ...
            PartialResult[SomeProject, LoadedClasses](
                ...
                LoadedClasses.key,
                {
                    case InterimEUBP(p: SomeProject, ub: LoadedClasses) =>
     ...
     */
    private def loadedClassesUB(): UIDSet[ClassType] = {
...
class ClassNewInstanceAnalysis private[analyses] (
    ...
    private def handleNewInstance(
        callContext: ContextType,
        callPC:      Int,
        classExpr:   V,
        stmts:       Array[Stmt[V]]
    )(implicit indirectCalls: IndirectCalls, state: TACAIBasedCGState[ContextType]): Unit = {
...
class ConstructorNewInstanceAnalysis private[analyses] (
    ...
    private val ConstructorT = ClassType.Constructor
    ...
    def c(constructor: V, state: TACAIBasedCGState[ContextType])(eps: SomeEPS): ProperPropertyComputationResult = {
        implicit val indirectCalls: IndirectCalls = new IndirectCalls()
        implicit val _state: TACAIBasedCGState[ContextType] = state
    ...
    private def handleConstructorNewInstance(
        callContext:       ContextType,
        callPC:            Int,
        constructor:       V,
        newInstanceParams: Seq[Option[Expr[V]]],
        stmts:             Array[Stmt[V]]
    )(implicit state: TACAIBasedCGState[ContextType], indirectCalls: IndirectCalls): Unit = {
        ...
        val persistentActualParams =
            actualParamsNewInstanceOpt.map(_.map(persistentUVar(_)(using stmts))).getOrElse(Seq.empty)
    ...
    private def handleGetConstructor(
        context:            ContextType,
        callPC:             Int,
        ...
        stmts:              Array[Stmt[V]]
    )(implicit indirectCalls: IndirectCalls, state: TACAIBasedCGState[ContextType]): Set[MethodMatcher] = {
...
class MethodInvokeAnalysis private[analyses] (
    ...
    def c(methodVar: V, state: TACAIBasedCGState[ContextType])(eps: SomeEPS): ProperPropertyComputationResult = {
        implicit val indirectCalls: IndirectCalls = new IndirectCalls()
        implicit val _state: TACAIBasedCGState[ContextType] = state
    ...
    private def handleMethodInvoke(
        ...
        val persistentReceiver = methodInvokeReceiver.flatMap(r => persistentUVar(r)(using stmts))
        val persistentActualParams =
            methodInvokeActualParamsOpt.map(_.map(persistentUVar(_)(using stmts))).getOrElse(Seq.empty)
...
class MethodHandleInvokeAnalysis private[analyses] (
    ...
    def c(methodHandle: V, state: TACAIBasedCGState[ContextType])(eps: SomeEPS): ProperPropertyComputationResult = {
        implicit val indirectCalls: IndirectCalls = new IndirectCalls()
        implicit val _state: TACAIBasedCGState[ContextType] = state
        ...
        if (epk.pk == ForNameClasses.key) {
            ...
            AllocationsUtil.continuationForAllocation[classDependerType, ContextType](
                ...
                val classes = TypesUtil.getPossibleClasses(
                    ...
                    if (data._2) project.classHierarchy.allSubtypes(tpe.asClassType, reflexive = true)
