...
object MethodHandlesUtil {
    ...
    private[reflection] def retrieveMatchersForMethodHandleConst(
        ...
        fieldType:      FieldType,
        isStatic:       Boolean
      )(implicit project: SomeProject): Set[FieldMatcher] = Set(
        ...
        else if (!isStatic) new ClassBasedFieldMatcher(
            project.classHierarchy.allSubtypes(declaringClass.asObjectType, reflexive = true),
            onlyFieldsExactlyInClass = false
        )
        else new ClassBasedFieldMatcher(Set(declaringClass.asObjectType), onlyFieldsExactlyInClass = false)
    )
}
