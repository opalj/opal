 ...
 * [[AllocationSitePointsToSetScala]] mappings.
 *
 ...
 */
class AbstractDoPrivilegedPointsToScalaCGAnalysis private[cg] (
    ...
    def analyze(): ProperPropertyComputationResult = {
        ...
        val fp = fps(1)
        val pointsToParam = ps(fp, AllocationSitePointsToSetScala.key)
    ...
    def continuationForParameterValue(
        ...
        case EUBP(_: VirtualFormalParameter, _: AllocationSitePointsToSetScala) ⇒
            methodMapping(
                eps.asInstanceOf[EPS[VirtualFormalParameter, AllocationSitePointsToSetScala]],
                seenElements,
                seenTypes
            )
        case _ ⇒
    ...
    def methodMapping(
        dependeeEOptP: EOptionP[VirtualFormalParameter, AllocationSitePointsToSetScala],
        seenElements:  Int,
        seenTypes:     Int
        ...
        val (newSeenElements, newSeenTypes) = if (dependeeEOptP.hasUBP) {
            ...
            dependeePointsTo.types.foreach { t ⇒
                ...
                if (callR.hasValue) {
                    ...
                    results ::= PartialResult[VirtualFormalParameter, AllocationSitePointsToSetScala](
                        tgtThis,
                        AllocationSitePointsToSetScala.key,
                        {
                            case UBP(oldPointsToUB: AllocationSitePointsToSetScala) ⇒
                            ...
                            case _: EPK[VirtualFormalParameter, AllocationSitePointsToSetScala] ⇒
                                val newPointsToUB = NoAllocationSitesScala.included(
                                    dependeePointsTo,
                                    seenElements,
                                    seenTypes,
                                    { x: ReferenceType ⇒ x == t }
                    ...
                    val returnPointsTo = ps(tgtMethod, AllocationSitePointsToSetScala.key)
    ...
    def returnMapping(
        returnPointsTo: EOptionP[DeclaredMethod, AllocationSitePointsToSetScala],
        seenElements:   Int,
        seenTypes:      Int
    ): ProperPropertyComputationResult = {
        var results: List[ProperPropertyComputationResult] = Nil
        val (newSeenElements, newSeenTypes) = if (returnPointsTo.hasUBP) {
            ...
            results ::= PartialResult[DeclaredMethod, AllocationSitePointsToSetScala](
                sourceMethod,
                AllocationSitePointsToSetScala.key,
                {
                    case UBP(ub: AllocationSitePointsToSetScala) ⇒
    ...
    def continuationForReturnValue(
        ...
        case EUBP(_: DeclaredMethod, _: AllocationSitePointsToSetScala) ⇒
            returnMapping(
                eps.asInstanceOf[EPS[DeclaredMethod, AllocationSitePointsToSetScala]],
                seenElements,
                seenTypes
            )
...
class DoPrivilegedPointsToScalaCGAnalysis private[cg] (
        final val project: SomeProject
) extends FPCFAnalysis {
    def analyze(p: SomeProject): PropertyComputationResult = {
        ...
        if (doPrivileged1.hasSingleDefinedMethod)
            results ::= new AbstractDoPrivilegedPointsToScalaCGAnalysis(doPrivileged1, runMethod, p).analyze()
        ...
        if (doPrivileged2.hasSingleDefinedMethod)
            results ::= new AbstractDoPrivilegedPointsToScalaCGAnalysis(doPrivileged2, runMethod, p).analyze()
        ...
        if (doPrivileged3.hasSingleDefinedMethod)
            results ::= new AbstractDoPrivilegedPointsToScalaCGAnalysis(doPrivileged3, runMethod, p).analyze()
        ...
        if (doPrivileged4.hasSingleDefinedMethod)
            results ::= new AbstractDoPrivilegedPointsToScalaCGAnalysis(doPrivileged4, runMethod, p).analyze()
        ...
        if (doPrivileged5.hasSingleDefinedMethod)
            results ::= new AbstractDoPrivilegedPointsToScalaCGAnalysis(doPrivileged5, runMethod, p).analyze()
        ...
        if (doPrivileged6.hasSingleDefinedMethod)
            results ::= new AbstractDoPrivilegedPointsToScalaCGAnalysis(doPrivileged6, runMethod, p).analyze()
        ...
        if (doPrivilegedWithCombiner1.hasSingleDefinedMethod)
            results ::= new AbstractDoPrivilegedPointsToScalaCGAnalysis(doPrivilegedWithCombiner1, runMethod, p).analyze()
        ...
        if (doPrivilegedWithCombiner2.hasSingleDefinedMethod)
            results ::= new AbstractDoPrivilegedPointsToScalaCGAnalysis(doPrivilegedWithCombiner2, runMethod, p).analyze()
        ...
        if (doPrivilegedWithCombiner3.hasSingleDefinedMethod)
            results ::= new AbstractDoPrivilegedPointsToScalaCGAnalysis(doPrivilegedWithCombiner3, runMethod, p).analyze()
        ...
        if (doPrivilegedWithCombiner4.hasSingleDefinedMethod)
            results ::= new AbstractDoPrivilegedPointsToScalaCGAnalysis(doPrivilegedWithCombiner4, runMethod, p).analyze()
...
object DoPrivilegedPointsToScalaCGAnalysisScheduler extends BasicFPCFEagerAnalysisScheduler {
    ...
    override def derivesCollaboratively: Set[PropertyBounds] =
        PropertyBounds.ubs(AllocationSitePointsToSetScala, Callers, Callees)

    override def derivesEagerly: Set[PropertyBounds] = Set.empty

    override def start(
        p: SomeProject, ps: PropertyStore, unused: Null
    ): DoPrivilegedPointsToScalaCGAnalysis = {
        val analysis = new DoPrivilegedPointsToScalaCGAnalysis(p)
