...
abstract class TypeIterator(val project: SomeProject) extends ContextProvider {
    ...
    def typesProperty(
        ...
        state: TypeIteratorState): InformationType
    ...
    def typesProperty(
        ...
        propertyStore: PropertyStore,
        state:         TypeIteratorState): InformationType = typesProperty(field, depender)
    ...
    def foreachAllocation(
        ...
        typesProperty:   InformationType,
        additionalTypes: Set[ReferenceType] = Set.empty
      )(handleAllocation: (ReferenceType, Context, Int) => Unit): Unit = {
        ...
        use.definedBy.foreach { index =>
            if (index >= 0) {
                ...
                if (allocO.isDefined) handleAllocation(
                    allocO.get._1,
                    context,
                    allocO.get._2
                )
    ...
    @inline protected[this] def continuation(
        ...
        handleNewType: ReferenceType => Unit): Unit
    ...
    @inline protected[this] def continuationForAllocations(
        ...
        oldEOptP:            EOptionP[Entity, PropertyType],
        additionalTypes:     Set[ReferenceType],
        handleNewAllocation: (ReferenceType, Context, Int) => Unit): Unit = {
        // Do nothing
    }
    ...
    @inline protected[this] def continuationForAllocations(
        ...
        @nowarn state: TypeIteratorState): Unit = {
        // Do nothing
    }
    ...
    private[cg] def isPossibleType(use: V, tpe: ReferenceType): Boolean = {
        ...
        if (lut.isDefined && !ch.isSubtypeOf(tpe, lut.get)) false
        else rv.allValues.exists {
            ...
            case mv: IsMObjectValue =>
                ...
                typeBounds.forall { supertype => ch.isSubtypeOf(tpe, supertype) }

            case _: IsNullValue => false
        }
    }
...
class CHATypeIterator(project: SomeProject)
    ...
    @inline override def typesProperty(
        ...
        propertyStore: PropertyStore,
        state:         TypeIteratorState): Null = null
            ...
            case mv: IsMObjectValue =>
                ...
                val potentialTypes = ch.allSubtypesForeachIterator(
                    ...
                    cfOption.isDefined && {
                        val cf = cfOption.get
                        !cf.isInterfaceDeclaration && !cf.isAbstract &&
                        remainingTypeBounds.forall { supertype => ch.isSubtypeOf(subtype, supertype) }
                    }
                }
    ...
    def foreachType(
        ...
        } else handleType(field.fieldType.asReferenceType)
    ...
    @inline override protected[this] def continuation(
        ...
        additionalTypes: Set[ReferenceType],
        handleNewType:   ReferenceType => Unit): Unit = throw new UnsupportedOperationException
    ...
    @inline override protected[this] def continuation(
        ...
        oldEOptP:      EOptionP[Entity, Property],
        handleNewType: ReferenceType => Unit
      )(implicit state: TypeIteratorState): Unit = throw new UnsupportedOperationException
}
...
class RTATypeIterator(project: SomeProject)
    ...
    @inline override def typesProperty(
        ...
        depender: Entity,
        stmts:    Array[Stmt[V]]
      )(implicit state: TypeIteratorState): InstantiatedTypes = typesProperty(depender, requiresDependency = true)
    ...
    @inline override def typesProperty(
        ...
        propertyStore: PropertyStore,
        state:         TypeIteratorState): InstantiatedTypes = typesProperty(depender, field.fieldType.isObjectType)
    ...
    @inline def typesProperty(
        ...
        val instantiatedTypesProperty =
            if (state.hasDependee(epk)) state.getProperty(epk)
    ...
    @inline override def foreachType(
        use:             V,
        typesProperty:   InstantiatedTypes,
        additionalTypes: Set[ReferenceType]
      )(handleType: ReferenceType => Unit): Unit = {
        ...
        use.value.asReferenceValue.allValues.foreach {
            case av: IsSArrayValue => handleType(av.theUpperTypeBound)
            case _                 =>
        }
    ...
    @inline override def foreachType(
        ...
        // The InstantiatedTypes do not track array types, we just assume them to be instantiated
        if (field.fieldType.isArrayType) handleType(field.fieldType.asReferenceType)
        else typesProperty.types.iterator.filter(isPossibleType(field, _)).foreach(handleType)
    ...
    @inline protected[this] def continuation(
        ...
        val seenTypes = if (oldEOptP.hasUBP) oldEOptP.ub.asInstanceOf[InstantiatedTypes].numElements else 0
...
class PropagationBasedTypeIterator(
        project: SomeProject,
        typeSetEntitySelector: TypeSetEntitySelector) extends TypeIterator(project) with SimpleContextProvider {
    ...
    @inline override def typesProperty(
        ...
        propertyStore: PropertyStore,
        state:         TypeIteratorState): (InstantiatedTypes, InstantiatedTypes) = (
        getProperty(
...
trait PointsToTypeIterator[ElementType, PointsToSet >: Null <: PointsToSetLike[ElementType, _, PointsToSet]]
    ...
    override def typesProperty(
        ...
        propertyStore: PropertyStore,
        state:         TypeIteratorState): PointsToSet = {
    ...
    @inline protected[this] def combine(pts1: PointsToSet, pts2: PointsToSet): PointsToSet = pts1.included(pts2)

    @inline override def foreachType(
        use:             V,
        typesProperty:   PointsToSet,
        additionalTypes: Set[ReferenceType]
      )(handleType: ReferenceType => Unit): Unit = typesProperty.forNewestNTypes(typesProperty.numTypes) { tpe =>
    ...
    @inline protected[this] def currentPointsTo(
        ...
        if (p2s.isRefinable) state.addDependency(depender, p2s)
    ...
    def extractPropertyUB[E <: Entity, P <: Property](
        ...
        propertyStore: PropertyStore,
        state:         TypeIteratorState): Option[P] = {
...
trait TypesBasedPointsToTypeIterator
    ...
    override def typesProperty(
        ...
        propertyStore: PropertyStore,
        state:         TypeIteratorState): TypeBasedPointsToSet = {
    ...
    @inline override protected[this] def createPointsToSet(
        ...
        allocatedType: ReferenceType,
        isConstant:    Boolean,
        isEmptyArray:  Boolean = false): TypeBasedPointsToSet = TypeBasedPointsToSet(UIDSet(allocatedType))
}
...
abstract class AbstractAllocationSitesPointsToTypeIterator(project: SomeProject)
    ...
    val mergeStringBuilderBuffer: Boolean = project.config.getBoolean(mergeStringBuilderBufferConfigKey)
    ...
    @inline override def foreachAllocation(
        ...
        typesProperty.forNewestNElements(typesProperty.numElements) { as =>
            ...
            if (isPossibleType(use, tpe) || additionalTypes.contains(tpe)) handleAllocation(tpe, context, pc)
        }
    ...
    @inline override protected[this] def continuationForAllocations(
        ...
        ub.forNewestNElements(ub.numElements - seenElements) { as =>
            val (context, pc, typeId) = longToAllocationSite(as)(this)
            val tpe                   = ReferenceType.lookup(typeId)
            if (isPossibleType(use, tpe) || additionalTypes.contains(tpe)) handleNewAllocation(tpe, context, pc)
        }
    }
    ...
    @inline protected[this] def createPointsToSet(
        ...
        allocatedType: ReferenceType,
        isConstant:    Boolean,
        isEmptyArray:  Boolean = false): AllocationSitePointsToSet = {
        ...
        (allocatedType.id: @switch) match {
            case StringBuilderId =>
                if (mergeStringBuilderBuffer) stringBuilderPointsToSet
                else createNewPointsToSet()
            case StringBufferId =>
                if (mergeStringBuilderBuffer) stringBufferPointsToSet
                else createNewPointsToSet()
            case StringId =>
                if (mergeStringConstants && isConstant) stringConstPointsToSet
                else createNewPointsToSet()
            case ClassId =>
                if (mergeClassConstants && isConstant) classConstPointsToSet
                else createNewPointsToSet()
            case _ =>
                if (mergeExceptions &&
                    project.classHierarchy.isSubtypeOf(allocatedType, ObjectType.Throwable)) {
                    val ptsO = exceptionPointsToSets.get(allocatedType.id)
                    if (ptsO.isDefined) ptsO.get
                    else {
...
class AllocationSitesPointsToTypeIterator(project: SomeProject)
    ...
    override def typesProperty(
        ...
        propertyStore: PropertyStore,
        state:         TypeIteratorState): AllocationSitePointsToSet =
        if (field.isDefinedField && field.definedField.isStatic) {
            ...
            for {
                ...
                fai <- extractPropertyUB(
                           EPK(field, FieldWriteAccessInformation.key),
                           state.addDependency(depender, _)
                       )
    ...
    @inline override def foreachAllocation(
        ...
        typesProperty.forNewestNElements(typesProperty.numElements) { as =>
            val (context, pc, typeId) = longToAllocationSite(as)(this)
            val tpe                   = ReferenceType.lookup(typeId)
            if (isPossibleType(field, tpe)) handleAllocation(tpe, context, pc)
        }
    ...
    @inline protected[this] def handleAllocationTacUpdate(
        ...
        uVarForDefSites(receiver, theTAC.pcToIndex).definedBy.foreach { defSite =>
            ...
            val objects = currentPointsTo(
                depender,
                pointsto.toEntity(defPC, newContext(definedMethod), theTAC.stmts)(formalParameters,
                                                                                  definitionSites,
                                                                                  this)
            )
    ...
    @inline override protected[this] def continuation(
        ...
        def handleAllocationSiteTypes(depender: Entity, as: AllocationSite): Unit = {
            ...
            objects.forNewestNTypes(objects.numTypes) { tpe => if (isPossibleType(field, tpe)) handleNewType(tpe) }
        }
        ...
        ub match {
            case pts: AllocationSitePointsToSet =>
                val seenElements =
                    if (oldEOptP.hasUBP) oldEOptP.ub.asInstanceOf[AllocationSitePointsToSet].numElements
                    else 0
                updatedEPS.e match {
                    case (_, `field`) => pts.forNewestNElements(pts.numElements - seenElements)(handleType)
                    case _ => state.dependersOf(updatedEPS.toEPK).foreach { depender =>
    ...
    @inline override protected[this] def continuationForAllocations(
        ...
        def handleAllocation(as: AllocationSite): Unit = {
            val (context, pc, typeId) = longToAllocationSite(as)(this)
            val tpe                   = ReferenceType.lookup(typeId)
            if (isPossibleType(field, tpe)) handleNewAllocation(tpe, context, pc)
        }
        ...
        ub match {
            case pts: AllocationSitePointsToSet =>
                ...
                updatedEPS.e match {
                    case (_, `field`) => pts.forNewestNElements(pts.numElements - seenElements)(handleAllocation)
                    case _ => pts.forNewestNElements(pts.numElements - seenElements) { oas =>
                            state.dependersOf(updatedEPS.toEPK).foreach(handlePointsToOfAllocationSite(_, oas))
                        }
                }

            case tac: TheTACAI => state.dependersOf(updatedEPS.toEPK).foreach {
                    case (depender: Entity, definedMethod: DefinedMethod, receiver: AnyRef)
                        if receiver.isInstanceOf[Option[_]] &&
                            receiver.asInstanceOf[Option[_]].isDefined =>
                        handleAllocationTacUpdate(
    ...
    @inline private def continuationForFieldAccesses(
        ...
        state.dependersOf(updatedEPS.toEPK).foreach { depender =>
            fai.getNewestAccesses(
                ...
                val tacEP =
                    if (state.hasDependee(tacEPK)) state.getProperty(tacEPK)
                    else propertyStore(definedMethod.definedMethod, TACAI.key)
                if (tacEP.isRefinable) state.addDependency((depender, definedMethod, receiverOpt), tacEP)

                if (tacEP.hasUBP && tacEP.ub.tac.isDefined && receiverOpt.isDefined) handleAllocationTacUpdate(
                    depender,
                    definedMethod,
                    tacEP.ub.tac.get,
                    receiverOpt.get)(handleAllocationSite(depender, _))
            }
        }
    }
}
