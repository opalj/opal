        ...
        val analyses: List[APIBasedAnalysis] = List(
            ...
            new TamiFlexPointsToNewInstanceAnalysis(
                ...
                declaredMethods(
                    ...
                    "newInstance",
                    MethodDescriptor(
                        ArraySeq(ObjectType.Class, ArrayType(IntegerType)),
                        ObjectType.Object
                    )
                ),
                "Array.newInstance"
            ...
            new TamiFlexPointsToClassGetMemberAnalysis(project, "forName", ObjectType.Class)(
                declaredMethods(
                    ...
                    ObjectType.Class,
                    "forName",
                    MethodDescriptor(ArraySeq(ObjectType.String, BooleanType, ObjectType("java/lang/ClassLoader")),
                                     ObjectType.Class)
                )
            ) with PointsToBase,
...
abstract class TamiFlexPointsToNewInstanceAnalysis(
        ...
        val key: String) extends PointsToAnalysisBase with APIBasedAnalysis {
    ...
    override def handleNewCaller(
        ...
        for (allocatedType <- allocatedTypes) state.includeSharedPointsToSet(
            defSite,
...
abstract class TamiFlexPointsToClassGetMemberAnalysis(
    ...
    override def handleNewCaller(
        ...
        val members = memberType match {
            case ObjectType.Class => tamiFlexLogData.classes(callerContext.method, s"Class.$method", line)
            case ObjectType.Field => tamiFlexLogData.fields(callerContext.method, s"Class.$method", line)
...
abstract class TamiFlexPointsToClassGetMembersAnalysis(
        final val project: SomeProject,
        method:            String,
        val memberType: ObjectType) extends PointsToAnalysisBase with APIBasedAnalysis {
