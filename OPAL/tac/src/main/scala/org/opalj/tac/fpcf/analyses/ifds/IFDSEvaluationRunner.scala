...
abstract class IFDSEvaluationRunner {
    ...
    protected def run(
        ...
        if (evalSchedulingStrategies) {
            ...
            for {
                ...
                if (additionalEvaluationResult.isDefined) additionalEvaluationResults :+= additionalEvaluationResult.get
            }
            if (evaluationFile.nonEmpty) {
    ...
    protected def writeAdditionalEvaluationResultsToFile(
        writer:                      PrintWriter,
        additionalEvaluationResults: Seq[Object]): Unit = {}

    protected def canBeCalledFromOutside(
        method:        DeclaredMethod,
        propertyStore: PropertyStore): Boolean = propertyStore(method, Callers.key) match {
