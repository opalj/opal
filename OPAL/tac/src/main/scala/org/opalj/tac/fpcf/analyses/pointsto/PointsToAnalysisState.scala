...
class PointsToAnalysisState[
    ...
    ContextType <: Context
        ...
        with TACAIBasedAnalysisState[ContextType] {
    ...
    def addPutFieldEntity(fakeEntity: (IntTrieSet, Option[DeclaredField])): Unit = putFields += fakeEntity
    ...
    def addArrayLoadEntity(
        fakeEntity: (Entity, ArrayType, ReferenceType => Boolean)): Unit = arrayLoads += fakeEntity

    def arrayLoadsIterator: Iterator[(Entity, ArrayType, ReferenceType => Boolean)] = arrayLoads.iterator

    def addArrayStoreEntity(fakeEntity: (IntTrieSet, ArrayType)): Unit = arrayStores += fakeEntity

    def arrayStoresIterator: Iterator[(IntTrieSet, ArrayType)] = arrayStores.iterator

    private[this] val _allocationSitePointsToSets: mutable.Map[Entity, PointsToSet] = mutable.Map.empty
    ...
    def allocationSitePointsToSetsIterator: Iterator[(Entity, PointsToSet)] = _allocationSitePointsToSets.iterator
    ...
    def includeSharedPointsToSet(
        ...
        typeFilter:  ReferenceType => Boolean = PointsToSetLike.noFilter): Unit =
    ...
    def includeSharedPointsToSets(
        ...
        typeFilter:   ReferenceType => Boolean): Unit =
        pointsToSets.foreach(pointsToSet => includeSharedPointsToSet(e, pointsToSet, typeFilter))

    def sharedPointsToSetsIterator: Iterator[(Entity, PointsToSet)] = _sharedPointsToSets.iterator
    ...
    final def hasDependee(dependee: EPK[Entity, Property]): Boolean = _dependees.contains(dependee)
    ...
    final def hasDependency(depender: Entity, dependee: SomeEPK): Boolean = _dependerToDependees.contains(depender) &&
    ...
    def callees(ps: PropertyStore): Callees = {
        ...
        if (calleesProperty.isEPK) NoCallees
        else calleesProperty.ub
    }

    def hasCalleesDepenedee: Boolean = _calleesDependee.nonEmpty && _calleesDependee.get.isRefinable
    ...
    private[this] def accesses[P <: MethodFieldAccessInformation[P]](
        ...
        key:                PropertyKey[P],
        noAccessesProperty: P): P = {
        val accessesProperty =
            if (dependee.isDefined) {
                dependee.get
            } else {
    ...
    def hasReadAccessDependee: Boolean = _readAccessesDependee.nonEmpty && _readAccessesDependee.get.isRefinable
    ...
    def hasWriteAccessDependee: Boolean = _writeAccessesDependee.nonEmpty && _writeAccessesDependee.get.isRefinable
    ...
    override def hasOpenDependencies: Boolean = hasCalleesDepenedee || super.hasOpenDependencies

    override def dependees: Set[SomeEOptionP] = {
        val otherDependees = super.dependees
        if (hasCalleesDepenedee) otherDependees + _calleesDependee.get
        else otherDependees
    }

    def addIncompletePointsToInfo(pc: Int)(implicit logContext: LogContext): Unit = OPALLogger.logOnce(Warn(
