...
trait DomainSpecificRater {
    ...
    def handleCall(
        ...
        project: SomeProject,
        code: Array[Stmt[V]]): Option[Purity]
...
class BaseDomainSpecificRater extends DomainSpecificRater {
    def handleCall(
        ...
        project: SomeProject,
        code:    Array[Stmt[V]]): Option[Purity] = None
    ...
    def handleException(stmt: Stmt[V]): Option[Purity] = None
}
...
trait SystemOutErrRater extends DomainSpecificRater {
    ...
    abstract override def handleCall(
        ...
        project: SomeProject,
        code:    Array[Stmt[V]]): Option[Purity] =
        if (receiver.isDefined && call.declaringClass == printStream && isOutErr(receiver.get)) Some(DPure)
    ...
    abstract override def handleGetStatic(
        ...
        if (declaringClass == ObjectType.System && (name == "out" || name == "err")) Some(Pure)
        else super.handleGetStatic(expr)
    }

    abstract override def handleException(stmt: Stmt[V]): Option[Purity] = super.handleException(stmt)
...
trait LoggingRater extends DomainSpecificRater {
    ...
    abstract override def handleCall(
        ...
        if (call.declaringClass.isObjectType) {
            ...
            if (loggers.exists(declClass.isSubtypeOf(_)(project.classHierarchy))) Some(DPure)
            else super.handleCall(call, receiver)
        } else super.handleCall(call, receiver)
    ...
    abstract override def handleGetStatic(
        ...
        if (logLevels.contains(declaringClass)) Some(DPure)
        else super.handleGetStatic(expr)
    }
}
...
trait ExceptionRater extends DomainSpecificRater {
    abstract override def handleCall(
        ...
        if (declClass.isObjectType && call.name == "<init>" &&
            ...
            !org.opalj.control.find(project.instanceMethods(declClass.asObjectType))(mdc =>
                ...
                )).exists(_.method.classFile.thisType != ObjectType.Throwable)) Some(DPure)
        else super.handleCall(call, receiver)
    }
    ...
    abstract override def handleGetStatic(
        ...
        project: SomeProject,
        code:    Array[Stmt[V]]): Option[Purity] = super.handleGetStatic(expr)

    override def handleException(stmt: Stmt[V]): Option[Purity] = Some(DPure)
}
...
trait AssertionExceptionRater extends DomainSpecificRater {
    ...
    abstract override def handleCall(
        ...
        if (call.declaringClass.isObjectType && call.name == "<init>" &&
            exceptionTypes.exists(call.declaringClass.asObjectType.isSubtypeOf)) Some(DPure)
        else super.handleCall(call, receiver)
    }
    ...
    override def handleException(stmt: Stmt[V]): Option[Purity] = None
}
