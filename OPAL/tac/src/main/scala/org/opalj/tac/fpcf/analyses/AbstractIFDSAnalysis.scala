...
abstract class AbstractIFDSAnalysis[DataFlowFact] extends FPCFAnalysis {
  ...
  class State(
      ...
      var pendingIfdsDependees: Map[(DeclaredMethod, DataFlowFact),
        EOptionP[(DeclaredMethod, DataFlowFact), IFDSProperty[DataFlowFact]]] = Map.empty,
  ...
  def collectResult(node: CFGNode)(implicit state: State): Map[Statement, Set[DataFlowFact]] =
    node.predecessors.collect {
      case bb: BasicBlock
          if state.outgoingFacts.contains(bb) && state.outgoingFacts(bb).contains(node) ⇒
        val index = bb.endPC
        Statement(
          state.method,
          state.code(index),
