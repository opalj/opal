...
class LoadedClassesAnalysis(
     ...
     */
    def handleCaller(
        declaredMethod: DeclaredMethod): PropertyComputationResult = {
        ...
        callersOfMethod match {
            ...
            case _: EPK[_, _] => throw new IllegalStateException("unexpected state")
            ...
            case _: EPS[_, _] =>
                if (!declaredMethod.hasSingleDefinedMethod) return NoResult;

                val method        = declaredMethod.definedMethod
                val declClassType = declaredMethod.declaringClassType

                if (declClassType ne method.classFile.thisType) return NoResult;
                ...
                if (method.body.isEmpty) {
                    if (!currentLoadedClasses.contains(declClassType)) {
                        ...
                        val newLoadedClasses = getSuperclassesNotYetLoaded(declClassType, currentLoadedClasses)
    ...
    private[this] def continuationForTAC(
        ...
        case UBP(tac: TACAI) if tac.tac.isDefined => processMethod(method, eps.asInstanceOf[EPS[Method, TACAI]])
        case _ => InterimPartialResult(
                Nil,
                Set(eps),
    ...
    private[this] def processMethod(
        ...
        tacaiEP:        EPS[Method, TACAI]): PropertyComputationResult = {
        assert(tacaiEP.hasUBP && tacaiEP.ub.tac.isDefined)

        // the method has callers. we have to analyze it
        val newLoadedClasses = handleNewReachableMethod(declaredMethod.declaringClassType, tacaiEP.ub.tac.get.stmts)
    ...
    private[this] def update(
      ...
      )(eop: EOptionP[_, LoadedClasses]): Option[InterimEP[SomeProject, LoadedClasses]] = eop match {
        case InterimUBP(ub: LoadedClasses) =>
            ...
            if (newUb.size > ub.classes.size) Some(InterimEUBP(project, ub.updated(newLoadedClasses)))
            else None
    ...
    def handleNewReachableMethod(
        ...
        stmts:         Array[Stmt[V]]): UIDSet[ObjectType] = {
        ...
        for (stmt <- stmts) {
            stmt match {
                ...
                case PutStatic(_, dc, _, _, _) if isNewLoadedClass(dc)                => newLoadedClasses += dc
                case Assignment(_, _, GetStatic(_, dc, _, _)) if isNewLoadedClass(dc) => newLoadedClasses += dc
                case ExprStmt(_, GetStatic(_, dc, _, _)) if isNewLoadedClass(dc)      => newLoadedClasses += dc
                case _                                                                =>
            }
        }
    ...
    private[this] def getSuperclassesNotYetLoaded(
        declClassType:        ObjectType,
        currentLoadedClasses: UIDSet[ObjectType]): UIDSet[ObjectType] =
    ...
    private[this] def getCurrentLoadedClasses(): UIDSet[ObjectType] = {
        val currentLoadedClassesEPS: EOptionP[SomeProject, LoadedClasses] = propertyStore(project, LoadedClasses.key)
