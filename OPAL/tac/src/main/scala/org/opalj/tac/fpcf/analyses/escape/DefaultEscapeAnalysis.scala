...
trait DefaultEscapeAnalysis extends AbstractEscapeAnalysis {
    ...
    override protected[this] def handlePutField(
        ...
        state:   AnalysisState): Unit = if (state.usesDefSite(putField.value)) state.meetMostRestrictive(AtMost(NoEscape))
    ...
    override protected[this] def handleArrayStore(
        ...
        if (state.usesDefSite(arrayStore.value)) state.meetMostRestrictive(AtMost(NoEscape))
    ...
    override protected[this] def handleThrow(
        ...
        context: AnalysisContext,
        state:   AnalysisState): Unit =
        if (state.usesDefSite(aThrow.exception)) state.meetMostRestrictive(AtMost(NoEscape))
    ...
    override protected[this] def handleStaticMethodCall(
        ...
        context: AnalysisContext,
        state:   AnalysisState): Unit =
        if (state.anyParameterUsesDefSite(call.params)) state.meetMostRestrictive(AtMost(EscapeInCallee))

    override protected[this] def handleVirtualMethodCall(
        call: VirtualMethodCall[V]
      )(implicit
        context: AnalysisContext,
        state:   AnalysisState): Unit = if (state.usesDefSite(call.receiver) || state.anyParameterUsesDefSite(call.params))
    ...
    override protected[this] def handleThisLocalOfConstructor(
        ...
        if (state.usesDefSite(call.receiver)) state.meetMostRestrictive(AtMost(NoEscape))
    }
