...
class FinalizerAnalysis private[analyses] (final val project: SomeProject) extends FPCFAnalysis {
    ...
    def analyze(dm: DeclaredMethod): PropertyComputationResult = {
        ...
        if (dm.name != "<init>") return NoResult;
            ...
            PartialResult[DeclaredMethod, Callers](
                ...
                Callers.key,
                {
                    case InterimUBP(ub: Callers) =>
                        if (!ub.hasVMLevelCallers) Some(InterimEUBP(finalizer, ub.updatedWithVMLevelCall()))
                        else None
                    case _: EPK[_, _] => Some(InterimEUBP(finalizer, OnlyVMLevelCallers))
