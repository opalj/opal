...
object ToTxt {
    ...
    def callToTxt[V <: Var[V]](name: String, params: Seq[Expr[V]]): String = params map {
        toTxtExpr[V]
    } mkString (s".$name(", ", ", ")")

    @inline final def toTxtExpr[V <: Var[V]](expr: Expr[V]): String = expr match {
        case v: Var[_] =>
            if (v.cTpe == ComputationalTypeReturnAddress) v.name + "/* return address */"
            else v.name
        ...
        case DynamicConst(_, bootstrapMethod, name, _) => s"DynamicConstant[${bootstrapMethod.toJava}]($name)"
        ...
        case InstanceOf(_, value, tpe) => s"${toTxtExpr(value)} instanceof ${tpe.asReferenceType.toJava}"

        case Compare(_, left, op, right) => toTxtExpr(left) + " " + op.toString + " " + toTxtExpr[V](right)

        case BinaryExpr(_, _ /*cTpe*/, op, left, right) =>
            toTxtExpr[V](left) + " " + op.toString + " " + toTxtExpr[V](right)

        case PrimitiveTypecastExpr(_, baseTpe, operand) => s"(${baseTpe.toJava}) ${toTxtExpr(operand)}"

        case NewArray(_, counts, arrayType) =>
            val initializedDimensions = counts.size
            val dimensions            = arrayType.dimensions
            val initializer = counts.map(c => s"[${toTxtExpr(c)}]").reverse.mkString("") +
        ...
        case GetStatic(_, declaringClass, name, _) => s"${declaringClass.toJava}.$name"
    ...
    private def qualify(index: Int, javaLikeStmt: String, indented: Boolean): String =
        if (indented) f"$index%5d:${javaLikeStmt.replace("\n", "\n       ")}"
        else s"$index:$javaLikeStmt"
    ...
    def apply[P <: AnyRef, V <: Var[V]](tac: TACode[P, V]): scala.collection.Seq[String] = apply(
        tac.params,
        tac.stmts,
        tac.cfg,
        skipParams = false,
    ...
    def apply[P <: AnyRef, V <: Var[V]](
        ...
        skipParams: Boolean,
        indented:   Boolean,
        includePC:  Boolean): scala.collection.Seq[String] = {
        ...
        while (index < max) {
            ...
            if (bb.endPC == index && bb.successors.exists(!_.isBasicBlock)) {
                val successors = bb.successors.toSeq.sorted.collect {
                ...
                if (successors.nonEmpty) javaLikeCode +=
    ...
    def apply(
        ...
        aiResult:       Option[AIResult { val domain: Domain with RecordDefUse }] = None): String = aiResult.map { aiResult =>
