...
trait AbstractPurityAnalysis_new extends FPCFAnalysis {
            ...
            fieldRef.asFieldRead.resolveField match {
                ...
                case Some(field) ⇒
                    checkFieldMutability(
                        propertyStore(field, ReferenceImmutability.key), Some(fieldRef.asGetField.objRef) //FieldMutability.key), Some(fieldRef.asGetField.objRef)
                    )
    ...
    def checkFieldMutability(
        ...
        case LBP(ImmutableReference | LazyInitializedReference) ⇒ //_: FinalField) ⇒ // Final fields don't impede purity
        case _: FinalEP[Field, ReferenceImmutability] ⇒ //FieldMutability] ⇒ // Mutable field
            if (objRef.isDefined) {
                if (state.ubPurity.isDeterministic)
    ...
    def checkTypeMutability(
        ...
        case LBP(DeepImmutableType | DeepImmutableClass) ⇒ true // ImmutableType | ImmutableObject) ⇒ true7
