...
private[xta] class TypePropagationTrace {
    ...
    private def traceMsg(msg: String): Unit = if (TypePropagationTrace.WriteTextualTrace) {
        _out.println(msg)
        _out.flush()
    }
    ...
    def traceInit(
        ...
        ps:           PropertyStore,
        typeIterator: TypeIterator): Unit = {
        val initialTypes = {
        ...
        val initialCallees = {
            ...
            if (calleesEOptP.hasUBP) calleesEOptP.ub.callSites(typeIterator.newContext(method)).flatMap(_._2)
            else Iterator.empty
        }
