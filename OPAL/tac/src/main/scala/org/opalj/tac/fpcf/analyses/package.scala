...
package object analyses {
    ...
    final def isTypeCompatible(
        ...
        case (ft: ReferenceType, at: ReferenceType) => ch.isSubtypeOf(at, ft)
        ...
        case (ft: BaseType, at: ReferenceType) => ch.isSubtypeOf(at, ft.WrapperType) ||
            (ft match {
        ...
        case (ft: ObjectType, at: BaseType) => primitiveType(ft) match {
    ...
    final def isTypeCompatible(
        ...
        // declared type and actual type are reference types and assignable
        case (pType: ReferenceType, v: IsReferenceValue) => v.isValueASubtypeOf(pType).isNotNo

        // boolean values will be encoded as integer values in bytecode
        case (_: BooleanType, IsPrimitiveValue(v)) => (v eq BooleanType) || (v eq IntegerType)
        ...
        case (_: BaseType, _: IsNullValue) => false
