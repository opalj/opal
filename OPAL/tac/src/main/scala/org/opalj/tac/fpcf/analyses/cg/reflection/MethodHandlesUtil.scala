...
object MethodHandlesUtil {
    ...
    private[reflection] def retrieveMethodHandleLookupMatchers(
        refc:          Expr[V],
        name:          Expr[V],
        methodType:    Expr[V],
        ...
        Seq(
            MatcherUtil.retrieveClassBasedMethodMatcher(refc, pc, stmts, project),
    ...
    private[reflection] def retrieveDescriptorBasedMethodMatcher(
        ...
        val descriptorsOpt =
            ...
            else getPossibleDescriptorsFormMethodTypes(expr, pc, stmts, cfg, project).map(_.toSet)
     ...
     */
    private[reflection] def getPossibleDescriptorsFormMethodTypes(
        value:   Expr[V],
        pc:      Int,
        stmts:   Array[Stmt[V]],
        cfg:     CFG[Stmt[V], TACStmts[V]],
        ...
        while (defSitesIterator.hasNext) {
            ...
            if (expr.isMethodTypeConst)
                ...
                val pmtOpt = getPossibleMethodTypes(call.params, call.descriptor, pc, stmts, cfg, project)
                if (pmtOpt.isEmpty) {
                    return None;
                }
    ...
    private[this] def getPossibleMethodTypes(
        ...
        val returnTypesOpt = TypesUtil.getPossibleTypes(params.head, pc, stmts, project)
        ...
        if (params.size == 1) { // methodType(T) => ()T
            ...
            val firstParamTypesOpt = TypesUtil.getPossibleTypes(params(1), pc, stmts, project)
            if (firstParamTypesOpt.isEmpty) {
                return None;
            }

            val possibleOtherParamTypes = VarargsUtil.getTypesFromVararg(params(2), stmts, cfg)
            ...
            if (secondParamType.isArrayType) { // methodType(T1, Class[]{T2, ...}) => (T2, ...)T1
                val possibleOtherParamTypes = VarargsUtil.getTypesFromVararg(params(1), stmts, cfg)
                ...
                val paramTypesOpt = TypesUtil.getPossibleTypes(params(1), pc, stmts, project)
