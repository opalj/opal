...
trait Expr[+V <: Var[V]] extends ASTNode[V] {
    ...
    override def toCanonicalForm(
        implicit
        ev: V <:< DUVar[ValueInformation]): Expr[DUVar[ValueInformation]]
...
case class InstanceOf[+V <: Var[V]](pc: PC, value: Expr[V], cmpTpe: ReferenceType) extends Expr[V] {
    ...
    override def toCanonicalForm(
        implicit
        ev: V <:< DUVar[ValueInformation]): Expr[DUVar[ValueInformation]] = InstanceOf(pc, value.toCanonicalForm, cmpTpe)
...
trait NoVariableExpr extends ValueExpr[Nothing] {
    ...
    override final def toCanonicalForm(
        implicit
        ev: Nothing <:< DUVar[ValueInformation]): Expr[DUVar[ValueInformation]] = this
}
...
case class DynamicConst(
        ...
        name:            String,
        descriptor: FieldType) extends SimpleValueConst {
...
case class NewArray[+V <: Var[V]](pc: PC, counts: Seq[Expr[V]], tpe: ArrayType) extends ArrayExpr[V] {
    ...
    override def toString: String = s"NewArray(pc=$pc,${counts.mkString("[", ",", "]")},${tpe.toJava})"
}
...
case class ArrayLength[+V <: Var[V]](pc: PC, arrayRef: Expr[V]) extends ArrayExpr[V] {
    ...
    override def toCanonicalForm(
        implicit
        ev: V <:< DUVar[ValueInformation]): Expr[DUVar[ValueInformation]] = ArrayLength(pc, arrayRef.toCanonicalForm)
...
case class GetField[+V <: Var[V]](
        ...
        objRef: Expr[V]) extends FieldRead[V] {
...
case class GetStatic(
        ...
        declaringClass: ObjectType,
        name:           String,
        declaredFieldType: FieldType) extends FieldRead[Nothing] {
    ...
    override def toString: String = s"GetStatic(pc=$pc,${declaringClass.toJava},$name,${declaredFieldType.toJava})"
}
...
case class InvokedynamicFunctionCall[+V <: Var[V]](
        ...
        params: Seq[Expr[V]]) extends Expr[V] {
...
case class NonVirtualFunctionCall[+V <: Var[V]](
        ...
        descriptor:     MethodDescriptor,
        receiver:       Expr[V],
        params: Seq[Expr[V]]) extends InstanceFunctionCall[V] {
    ...
    override def toCanonicalForm(
        implicit
        ev: V <:< DUVar[ValueInformation]): Expr[DUVar[ValueInformation]] = NonVirtualFunctionCall(
        pc,
        declaringClass,
        isInterface,
        name,
...
case class VirtualFunctionCall[+V <: Var[V]](
    ...
    override def toCanonicalForm(
        implicit
        ev: V <:< DUVar[ValueInformation]): Expr[DUVar[ValueInformation]] = VirtualFunctionCall(
        pc,
        declaringClass,
        isInterface,
        name,
...
case class StaticFunctionCall[+V <: Var[V]](
        ...
        name:           String,
        descriptor:     MethodDescriptor,
        params: Seq[Expr[V]]) extends FunctionCall[V] {
    ...
    override def toCanonicalForm(
        implicit
        ev: V <:< DUVar[ValueInformation]): Expr[DUVar[ValueInformation]] = StaticFunctionCall(
        pc,
        declaringClass,
        isInterface,
        name,
