...
abstract class JavaBackwardTaintProblem(project: SomeProject)
    ...
    override def normalFlow(jstmt: JavaStatement, in: TaintFact, predecessor: Option[JavaStatement]): Set[TaintFact] =
        jstmt.stmt.astID match {
            ...
            case PutField.ASTID =>
                ...
                if (in match {
                        case InstanceField(index, classType, fieldName) => definedBy.contains(index) &&
     ...
     */
    protected def createFlowFactAtCall(call: JavaStatement, in: TaintFact, callChain: Seq[Callable])
     ...
     */
    private def createNewArrayElementTaints(expression: Expr[V], taintedElement: Int, statement: JavaStatement)
        : Set[TaintFact] = createNewTaints(expression, statement).map {
    ...
    private def createNewInstanceFieldTaints(
        ...
        name:           String,
        statement:      JavaStatement): Set[TaintFact] = createNewTaints(expression, statement).map {
    ...
    protected def applyFlowFactFromCallee(
        ...
        caller: Method,
        in: TaintFact,
        callChain: Seq[Callable]): Option[FlowFact]
}
