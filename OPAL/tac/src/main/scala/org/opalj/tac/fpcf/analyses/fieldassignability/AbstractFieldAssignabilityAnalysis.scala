...
trait AbstractFieldAssignabilityAnalysis extends FPCFAnalysis {
    ...
    trait AbstractFieldAssignabilityAnalysisState {
        ...
        def hasDependees: Boolean = escapeDependees.nonEmpty || fieldWriteAccessDependee.exists(_.isRefinable) ||
            tacDependees.valuesIterator.exists(_.isRefinable) || callerDependees.valuesIterator.exists(_.isRefinable)

        def dependees: Set[SomeEOptionP] = escapeDependees ++ fieldWriteAccessDependee.filter(_.isRefinable) ++
    ...
    def doDetermineFieldAssignability(entity: Entity): ProperPropertyComputationResult = entity match {
    ...
    private[analyses] def determineFieldAssignability(
        ...
        else state.fieldAssignability = EffectivelyNonAssignable

        if (field.isPublic) return Result(field, Assignable);
        ...
        if (handleFieldWriteAccessInformation(fwaiEP)) return Result(field, Assignable);
    ...
    def referenceHasNotEscaped(
        ...
        method:  DefinedMethod,
        callers: Callers
      )(implicit state: AnalysisState): Boolean = ref.definedBy.forall { defSite =>
        ...
        val assignable =
            if (newEP.hasUBP) {
                ...
                newFai.getNewestAccesses(
                    ...
                    if (tacEP.hasUBP && callersEP.hasUBP) methodUpdatesField(
                        method,
                        tacEP.ub.tac.get,
                        callersEP.ub,
                        writeAccess._2,
    ...
    def c(eps: SomeEPS)(implicit state: AnalysisState): ProperPropertyComputationResult = {
        val isNonFinal = eps.pk match {
            ...
            case TACAI.key =>
                ...
                if (callersProperty.hasUBP && accesses.isDefined) accesses.get.exists(access =>
        ...
        if (isNonFinal) Result(state.field, Assignable)
        else createResult()
    }
