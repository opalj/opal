    ...
    def analyze(dm: DeclaredMethod): PropertyComputationResult = {
        ...
        val instantiatedTypes =
            if (nativeMethodData.contains(dm)) {
                ...
                if (dataO.isEmpty) return NoResult;
                dataO.get.collect {
        ...
        val instantiatedTypesUB = getInstantiatedTypesUB(propertyStore(project, InstantiatedTypes.key))

        val newInstantiatedTypes = UIDSet(
            ArraySeq.unsafeWrapArray(instantiatedTypes.filterNot(instantiatedTypesUB.contains)): _*
        )

        if (newInstantiatedTypes.isEmpty) return NoResult;
...
object ConfiguredNativeMethodsInstantiatedTypesAnalysisScheduler extends BasicFPCFTriggeredAnalysisScheduler {
    ...
    override def uses: Set[PropertyBounds] = PropertyBounds.ubs(Callers, InstantiatedTypes)
    ...
    override def register(
        p:      SomeProject,
        ps:     PropertyStore,
        unused: Null): ConfiguredNativeMethodsInstantiatedTypesAnalysis = {
