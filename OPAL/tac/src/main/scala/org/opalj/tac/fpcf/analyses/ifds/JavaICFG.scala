...
abstract class JavaICFG(project: SomeProject)
     ...
     *         contain a call.
     */
    override def getCalleesIfCallStatement(statement: JavaStatement): Option[Set[Method]] = statement.stmt.astID match {
        case StaticMethodCall.ASTID | NonVirtualMethodCall.ASTID | VirtualMethodCall.ASTID => Some(getCallees(statement))
        case Assignment.ASTID | ExprStmt.ASTID => getExpression(statement.stmt).astID match {
     ...
     */
    private def definedMethods(declaredMethods: Iterator[DeclaredMethod]): Set[Method] = declaredMethods
        .flatMap(declaredMethod =>
            if (declaredMethod.hasSingleDefinedMethod) Seq(declaredMethod.definedMethod)
            else if (declaredMethod.hasMultipleDefinedMethods) declaredMethod.definedMethods
            else Seq.empty).toSet
    ...
    override def getCallers(callee: Method): Set[JavaStatement] = {
        ...
        propertyStore(declaredCallee, Callers.key) match {
            case FinalEP(_, p: Callers) => p.callers(declaredCallee).iterator
    ...
    def canBeCalledFromOutside(method: Method): Boolean = canBeCalledFromOutside(declaredMethods(method))
