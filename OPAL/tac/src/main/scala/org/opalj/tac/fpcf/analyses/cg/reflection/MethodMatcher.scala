...
final class NameBasedMethodMatcher(val possibleNames: Set[String]) extends MethodMatcher {
    ...
    override def contains(m: Method)(implicit p: SomeProject): Boolean = possibleNames.contains(m.name)
...
class ClassBasedMethodMatcher(
        ...
        val onlyMethodsExactlyInClass: Boolean) extends MethodMatcher {
    ...
    private[this] def methods(implicit p: SomeProject): Set[Method] = possibleClasses.flatMap { c =>
        ...
        if (onlyMethodsExactlyInClass) methodsInClassFile
        else methodsInClassFile ++ p.instanceMethods.getOrElse(c, ArraySeq.empty).map(_.method)
...
class DescriptorBasedMethodMatcher(
        val possibleDescriptors: Set[MethodDescriptor]) extends MethodMatcher {

    override def initialMethods(implicit p: SomeProject): Iterator[Method] =
        p.allMethods.iterator.filter(m => possibleDescriptors.contains(m.descriptor))

    override def contains(m: Method)(implicit p: SomeProject): Boolean = possibleDescriptors.contains(m.descriptor)
...
class ParameterTypesBasedMethodMatcher(val parameterTypes: FieldTypes) extends MethodMatcher {
    ...
    override def contains(m: Method)(implicit p: SomeProject): Boolean = m.parameterTypes == parameterTypes
...
class ActualParameterBasedMethodMatcher(val actualParams: Seq[V]) extends MethodMatcher {
    ...
    override def initialMethods(implicit p: SomeProject): Iterator[Method] = p.allMethods.iterator.filter(contains)
    ...
    override def initialMethods(implicit p: SomeProject): Iterator[Method] = {
        ...
        p.allClassFiles.iterator.flatMap { cf =>
            ...
            if (receiver.isNull.isNoOrUnknown && receiver.isValueASubtypeOf(cf.thisType).isYesOrUnknown)
                r ++= cf.methods
            else if (receiver.isNull.isYesOrUnknown) r ++= cf.methods.filter(_.isStatic)
            r
        }
    }
