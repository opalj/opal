...
trait TACAIBasedAnalysisState[TheContextType <: Context]
    ...
    abstract override def dependees: Set[SomeEOptionP] = {
        ...
        if ((_tacDependee ne null) && _tacDependee.isRefinable) otherDependees + _tacDependee
        else otherDependees
    }

    final def updateTACDependee(tacDependee: EOptionP[Method, TACAI]): Unit = _tacDependee = tacDependee
