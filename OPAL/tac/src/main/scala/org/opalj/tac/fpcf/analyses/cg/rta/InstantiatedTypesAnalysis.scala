...
class InstantiatedTypesAnalysis private[analyses] (
    ...
    def analyze(declaredMethod: DeclaredMethod): PropertyComputationResult = {
        ...
        if (declaredMethod.name != "<init>" || cfOpt.isDefined && cfOpt.get.isAbstract) {
            if (instantiatedTypes.isEmpty) return NoResult;
            else return partialResult(instantiatedTypes)
        }
        ...
        if (instantiatedTypesUB.contains(declaredType) && newInstantiatedTypes.isEmpty) return NoResult;
            ...
            (_, callerContext, _, isDirect) =>
                ...
                project.classFile(caller.declaringClassType).foreach { cf =>
                    cf.superclassType.foreach { supertype =>
                        if (supertype != declaredType) return partialResult(instantiatedTypes + declaredType);
                    }
                }
                ...
                if (hasNew) return partialResult(instantiatedTypes + declaredType);
        }

        if (callersEOptP.isFinal) {
            if (instantiatedTypes.isEmpty) NoResult
            else partialResult(instantiatedTypes)
        } else {
    ...
    private[this] def partialResult(
        instantiatedTypes: UIDSet[ReferenceType]): PartialResult[SomeProject, InstantiatedTypes] =
...
object InstantiatedTypesAnalysis {
    def update(
      ...
      )(eop: EOptionP[SomeProject, InstantiatedTypes]): Option[InterimEP[SomeProject, InstantiatedTypes]] = eop match {
        case InterimUBP(ub: InstantiatedTypes) =>
            val newUB = ub.updated(newInstantiatedTypes)
            if (newUB.types.size > ub.types.size) Some(InterimEUBP(p, newUB))
            else None

        case _: EPK[_, _] => throw new IllegalStateException(
