 ...
 * @author Dominik Helm
 */
class L2PurityAnalysis_new private[analyses] (val project: SomeProject)
    extends AbstractPurityAnalysis_new {
  ...
  class State(
    ...
    var fieldLocalityDependees
        : Map[Field, (EOptionP[Field, FieldLocality], Set[(Expr[V], Purity)])] = Map.empty

    var fieldMutabilityDependees
        : Map[Field, (EOptionP[Field, ReferenceImmutability], Set[Option[Expr[V]]])] = Map.empty
    //FieldMutability

    var classImmutabilityDependees
        : Map[ObjectType, (EOptionP[ObjectType, ClassImmutability_new], Set[Expr[V]])] = Map.empty
    //ClassImmutability
    var typeImmutabilityDependees
        : Map[ObjectType, (EOptionP[ObjectType, TypeImmutability_new], Set[Expr[V]])] = Map.empty
    //TypeImmutability

    var purityDependees
        : Map[DeclaredMethod, (EOptionP[DeclaredMethod, Purity], Set[Seq[Expr[V]]])] = Map.empty

    var calleesDependee: Option[EOptionP[DeclaredMethod, Callees]] = None
    var callees: Option[Callees] = None

    var rvfDependees: Map[
      DeclaredMethod,
      (EOptionP[DeclaredMethod, ReturnValueFreshness], Set[(Option[Expr[V]], Purity)])
    ] = Map.empty
    ...
    def addFieldMutabilityDependee(
        f: Field,
        eop: EOptionP[Field, ReferenceImmutability], // FieldMutability],
        owner: Option[Expr[V]]
    ): Unit = {
    ...
    def addClassImmutabilityDependee(
        t: ObjectType,
        eop: EOptionP[ObjectType, ClassImmutability_new], //ClassImmutability],
        value: Expr[V]
    ): Unit = {
    ...
    def addTypeImmutabilityDependee(
        t: ObjectType,
        eop: EOptionP[ObjectType, TypeImmutability_new], // TypeImmutability],
        value: Expr[V]
    ): Unit = {
  ...
  override def handleUnknownTypeMutability(
    ...
    if (ep.pk == ClassImmutability_new.key) //ClassImmutability.key)
      state.addClassImmutabilityDependee(
        ep.e,
        ep.asInstanceOf[EOptionP[ObjectType, ClassImmutability_new]], //ClassImmutability]],
        expr
      )
    else
      state.addTypeImmutabilityDependee(
        ep.e,
        ep.asInstanceOf[EOptionP[ObjectType, TypeImmutability_new]], //TypeImmutability]],
        expr
      )
  }
  ...
  def c(eps: SomeEPS)(implicit state: State): ProperPropertyComputationResult = {
    ...
    eps.ub.key match {
      ...
      case ReferenceImmutability.key =>
        ...
        dependees._2.foreach { e =>
          checkFieldMutability(eps.asInstanceOf[EOptionP[Field, ReferenceImmutability]], e) //FieldMutability]], e)
        }
      //case ClassImmutability.key ⇒
      case ClassImmutability_new.key =>
        ...
        dependees._2.foreach { e =>
          checkTypeMutability(
            eps.asInstanceOf[EOptionP[ObjectType, ClassImmutability_new]], // ClassImmutability]],
            e
          )
        }
      ...
      case TypeImmutability_new.key =>
        ...
        dependees._2.foreach { e =>
          checkTypeMutability(eps.asInstanceOf[EOptionP[ObjectType, TypeImmutability_new]], e) //TypeImmutability]], e)
        }
...
trait L2PurityAnalysisScheduler_new extends FPCFAnalysisScheduler {
  ...
  override def uses: Set[PropertyBounds] = {
    Set(
      ...
      PropertyBounds.lub(ClassImmutability_new),
  ...
  final override type InitializationData = L2PurityAnalysis_new
  final override def init(p: SomeProject, ps: PropertyStore): InitializationData = {
    new L2PurityAnalysis_new(p)
  }
...
object EagerL2PurityAnalysis_new
  ...
  override def start(
    ...
    val methods = dms.collect {
      ...
      case dm
          if dm.hasSingleDefinedMethod && dm.definedMethod.body.isDefined && !analysis.configuredPurity
