...
class L2PurityAnalysis_new private[analyses] (val project: SomeProject)
    ...
    class State(
        ...
        var fieldMutabilityDependees: Map[Field, (EOptionP[Field, ReferenceImmutability], Set[Option[Expr[V]]])] = Map.empty
        //FieldMutability

        var classImmutabilityDependees: Map[ObjectType, (EOptionP[ObjectType, ClassImmutability_new], Set[Expr[V]])] = Map.empty
        //ClassImmutability
        var typeImmutabilityDependees: Map[ObjectType, (EOptionP[ObjectType, TypeImmutability_new], Set[Expr[V]])] = Map.empty
        //TypeImmutability
    ...
    val raterFqn: String = project.config.as[String](
        "org.opalj.fpcf.analyses.L2PurityAnalysis_new.domainSpecificRater"
    )
