 ...
 */
class TypeImmutabilityAnalysis_new(final val project: SomeProject) extends FPCFAnalysis {

  def doDetermineTypeMutability_new(
  ...
  def step1(
    ...
    typeExtensibility(t) match {
      case Yes | Unknown => Result(t, MutableType_new) // MutableType)
      case No => step2(t)
    }
  }
  ...
  def step2(t: ObjectType): ProperPropertyComputationResult = {
    ...
    if (cf.exists(_.isFinal) || directSubtypes.isEmpty /*... the type is not extensible*/ ) {
      ...
      ps(t, ClassImmutability_new.key) match {
        case FinalP(p) =>
        ...
        case eps @ InterimLUBP(lb, ub) =>
          ...
          val thisLB = lb.correspondingTypeImmutability
          InterimResult(t, thisLB, thisUB, Seq(eps), c)
        case epk => InterimResult(t, MutableType_new, DeepImmutableType, Seq(epk), c)
        //InterimResult(t, MutableType, ImmutableType, Seq(epk), c)
      }
    } else {
      var dependencies = Map.empty[Entity, EOptionP[Entity, Property]]
      var joinedImmutability
          : TypeImmutability_new = DeepImmutableType //ImmutableType // this may become "Mutable..."
      ...
      if (dependencies.isEmpty) {
        ...
        def c(eps: EPS[Entity, Property]): ProperPropertyComputationResult = {
          ...
          (eps: @unchecked) match {
            ...
            case UBP(x)
                if (x == MutableType_new || x == MutableClass) => //MutableType | _: MutableObject) =>
              Result(t, MutableType_new) //MutableType)

            case FinalEP(e, x)
                if (x == ShallowImmutableType || x == ShallowImmutableClass) => //ImmutableContainerType | ImmutableContainer) =>
              maxImmutability = ShallowImmutableType //ImmutableContainerType
              dependencies = dependencies - e
              nextResult()
...
object EagerLxTypeImmutabilityAnalysis_new
  ...
  override def start(project: SomeProject, ps: PropertyStore, unused: Null): FPCFAnalysis = {
    ...
    val analysis = new TypeImmutabilityAnalysis_new(project)
...
object LazyLxTypeImmutabilityAnalysis_new
  ...
  override def register(p: SomeProject, ps: PropertyStore, unused: Null): FPCFAnalysis = {
    ...
    val analysis = new TypeImmutabilityAnalysis_new(p)
    val analysisRunner: ProperPropertyComputation[Entity] =
      analysis.doDetermineTypeMutability_new(typeExtensibility)
