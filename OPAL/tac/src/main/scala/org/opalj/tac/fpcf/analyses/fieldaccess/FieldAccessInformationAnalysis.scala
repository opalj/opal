...
class FieldAccessInformationAnalysis(val project: SomeProject) extends FPCFAnalysis {
    ...
    private def continuation(context: ContextType, state: State)(eps: SomeEPS): ProperPropertyComputationResult =
        eps match {
            ...
            case _ => InterimPartialResult(state.dependees, continuation(context, state))
        }

    private def returnResult(context: ContextType)(implicit state: State, fieldAccesses: DirectFieldAccesses)
        ...
        else Results(fieldAccesses.partialResults(context))

    private def handleTac(
        context: ContextType,
        tac:     TACode[TACMethodParameter, DUVar[ValueInformation]]
      )(implicit fieldAccesses: DirectFieldAccesses): Unit = tac.stmts.foreach {
        ...
        case _ @ExprStmt(_, GetField(pc, declaringClass, fieldName, fieldType, objRef)) => fieldAccesses.addFieldRead(
                context,
                pc,
                declaredFields(declaringClass, fieldName, fieldType),
                persistentUVar(objRef.asVar)(tac.stmts)
        ...
        case PutField(pc, declaringClass, fieldName, fieldType, objRef, value) => fieldAccesses.addFieldWrite(
                context,
                pc,
                declaredFields(declaringClass, fieldName, fieldType),
                persistentUVar(objRef.asVar)(tac.stmts),
        ...
        case PutStatic(pc, declaringClass, fieldName, fieldType, value) => fieldAccesses.addFieldWrite(
                context,
                pc,
                declaredFields(declaringClass, fieldName, fieldType),
                None,
