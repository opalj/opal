 ...
 */
class ConfiguredNativeMethodsPointsToScalaAnalysis private[analyses] (
        ...
        for (PointsToRelation(lhs, rhs) ← data) {
            rhs match {
                case asd: AllocationSiteDescription ⇒
                    ...
                    val pts = AllocationSitePointsToSetScala(Set(as), UIDSet(instantiatedType))
                    results += createPartialResultOpt(lhs.entity, pts).get
                case _ ⇒
                    val pointsToEOptP = propertyStore(rhs.entity, AllocationSitePointsToSetScala.key)
    ...
    private[this] def createPartialResultOpt(lhs: Entity, newPointsTo: AllocationSitePointsToSetScala) = {
        if (newPointsTo.numTypes > 0) {
            Some(PartialResult[Entity, AllocationSitePointsToSetScala](lhs, AllocationSitePointsToSetScala.key, {
                case InterimUBP(ub: AllocationSitePointsToSetScala) ⇒
                ...
                case _: EPK[Entity, AllocationSitePointsToSetScala] ⇒
                    Some(InterimEUBP(lhs, newPointsTo))

                case fep: FinalEP[Entity, AllocationSitePointsToSetScala] ⇒
    ...
    private[this] def c(
        lhs: Entity, rhsEOptP: EOptionP[Entity, AllocationSitePointsToSetScala]
    )(eps: SomeEPS): ProperPropertyComputationResult = eps match {
        case UBPS(rhsUB: AllocationSitePointsToSetScala, rhsIsFinal) ⇒
            ...
            if (rhsEOptP.hasUBP && (rhsEOptP.ub eq rhsUB) && eps.isRefinable) {
                InterimPartialResult(Some(eps), c(lhs, eps.asInstanceOf[EPS[Entity, AllocationSitePointsToSetScala]]))
            } else {
                val pr = PartialResult[Entity, AllocationSitePointsToSetScala](lhs, AllocationSitePointsToSetScala.key, {
                    case InterimUBP(lhsUB: AllocationSitePointsToSetScala) ⇒
                ...
                if (rhsIsFinal) {
                    ...
                    InterimPartialResult(
                        Some(pr), Some(eps), c(lhs, eps.asInstanceOf[EPS[Entity, AllocationSitePointsToSetScala]])
                    )
                }
            }
        case _ ⇒
...
object ConfiguredNativeMethodsPointsToScalaAnalysisScheduler extends FPCFTriggeredAnalysisScheduler {
    ...
    override def derivesCollaboratively: Set[PropertyBounds] = PropertyBounds.ubs(AllocationSitePointsToSetScala)
    ...
    ): ConfiguredNativeMethodsPointsToScalaAnalysis = {
        val analysis = new ConfiguredNativeMethodsPointsToScalaAnalysis(p)
