...
class SystemPropertiesAnalysisScheduler private[analyses] (
        final val project: SomeProject) extends ReachableMethodAnalysis {

    def processMethod(
        callContext: ContextType,
        tacaiEP:     EPS[Method, TACAI]): ProperPropertyComputationResult = {
        ...
        def update(
            currentVal: EOptionP[SomeProject, SystemProperties]): Option[InterimEP[SomeProject, SystemProperties]] =
            currentVal match {
                case UBP(ub) =>
                    var oldProperties = ub.properties
                    val noNewProperty = propertyMap.forall {
                        case (key, values) => oldProperties.contains(key) && {
    ...
    def computeProperties(
        ...
        stmts:       Array[Stmt[DUVar[ValueInformation]]]): Map[String, Set[String]] = {
        var res = propertyMap
    ...
    def getPossibleStrings(
        ...
        stmts: Array[Stmt[DUVar[ValueInformation]]]): Set[String] = value.asVar.definedBy filter { index =>
...
object SystemPropertiesAnalysisScheduler extends BasicFPCFTriggeredAnalysisScheduler {
    ...
    override def register(
        p:      SomeProject,
        ps:     PropertyStore,
        unused: Null): SystemPropertiesAnalysisScheduler = {
