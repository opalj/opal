...
class FieldLocalityState(val field: Field, val thisIsCloneable: Boolean) {
    ...
    private[this] var definitionSitesDependees
        : Map[(Context, DefinitionSiteLike), (EOptionP[(Context, DefinitionSiteLike), EscapeProperty], Boolean)] =
        Map.empty
    ...
    def hasNoDependees: Boolean = fieldReadAccessDependee.forall(_.isFinal) &&
    ...
    def isDefinitionSiteOfClone(e: (Context, DefinitionSiteLike)): Boolean = clonedDependees.contains(e)

    def addMethodDependee(ep: EOptionP[Context, ReturnValueFreshness]): Unit = declaredMethodsDependees += ep
    ...
    def addClonedDefinitionSiteDependee(
        ep: EOptionP[(Context, DefinitionSiteLike), EscapeProperty]): Unit = {
    ...
    def addDefinitionSiteDependee(
        ...
        isGetFieldOfReceiver: Boolean): Unit = definitionSitesDependees += ep.e -> ((ep, isGetFieldOfReceiver))

    def removeDefinitionSiteDependee(
        ep: EOptionP[(Context, DefinitionSiteLike), EscapeProperty]): Unit = definitionSitesDependees -= ep.e

    def isGetFieldOfReceiver(defSite: (Context, DefinitionSiteLike)): Boolean = definitionSitesDependees(defSite)._2
    ...
    def addTACDependee(ep: EOptionP[Method, TACAI]): Unit = tacDependees += ep.e -> ep

    def getTACDependee(m: Method): Option[EOptionP[Method, TACAI]] = tacDependees.get(m)

    def addCallersDependee(ep: EOptionP[DeclaredMethod, Callers]): Unit = callerDependees += ep.e -> ep

    def getCallersDependee(dm: DeclaredMethod): Option[EOptionP[DeclaredMethod, Callers]] = callerDependees.get(dm)
    ...
    def addCallsite(
        ...
        if (calleeDependees.contains(caller)) {
            ...
            if (!pcs.contains(pc)) calleeDependees.update(caller, (oldEP, pcs + pc))
            oldEP
        } else {
    ...
    def getCallsites(caller: DeclaredMethod): IntTrieSet = calleeDependees(caller)._2
