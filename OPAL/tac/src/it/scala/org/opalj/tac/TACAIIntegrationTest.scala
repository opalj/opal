...
class TACAIIntegrationTest extends AnyFunSpec with Matchers {
    ...
    def checkProject(
        ...
        performanceEvaluation: PerformanceEvaluation): Unit = {
        ...
        if (errors.nonEmpty) {
            val summary = s"successfully transformed ${successfullyCompleted.get} methods: " +
    ...
    protected def domainFactories = Seq[(String, (SomeProject, Method) => Domain with RecordDefUse)](
        (
