...
class FPCFAnalysesIntegrationTest extends AnyFunSpec {
                ...
                it("should compute the correct properties") {
                    ...
                    val actual = properties.iterator.flatMap { property =>
                        ps.entities(property.key).filter { ep =>
                            if (ep.isRefinable) fail(s"intermediate results left over $ep")
    ...
    def getAnalysis(id: String): ComputationSpecification[FPCFAnalysis] = FPCFAnalysesRegistry.eagerFactory(id.trim)
    ...
    def getConfig: Seq[(String, Set[ComputationSpecification[FPCFAnalysis]], Seq[PropertyMetaInformation])] = {
        val configInputStream = this.getClass.getResourceAsStream("FPCFAnalysesIntegrationTest.config")
        ...
        for (line <- configLines) {
            if (line.startsWith(" ")) {
                if (readProperties) curConfig = (curConfig._1, curConfig._2, curConfig._3 :+ getProperty(line))
                else curConfig = (curConfig._1, curConfig._2 + getAnalysis(line), curConfig._3)
