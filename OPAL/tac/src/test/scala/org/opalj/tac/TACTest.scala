...
private[tac] class TACTest extends AnyFunSpec with Matchers {
    ...
    def compareStatements[V <: Var[V]](
        ...
        actualStmts:   IndexedSeq[Stmt[V]]): Unit = compareStatements(expectedStmts.toArray, actualStmts.toArray)

    def compareStatements[V <: Var[V]](
        expectedStmts: Array[Stmt[V]],
        actualStmts:   Array[Stmt[V]]): Unit = {
        ...
        if (!Arrays.equals(expected: Array[Object], actual: Array[Object])) {
            val message = actualStmts.zip(expectedStmts).filter(p => p._1 != p._2).map(p =>
