            ...
            describe(s"analysis of $projectName with jdk: $jdk") {
                ...
                for { Seq(_, _, className, methodName, test, domainName) <- tests } {
                    it(test + s" ($className.$methodName using $domainName)") {
                        ...
                        if (cfOption.isEmpty) fail(s"cannot find class: $className")
                        val cf = cfOption.get

                        val mChain = cf.findMethod(methodName)
                        if (mChain.size != 1) fail(s"invalid method name: $className{ $methodName; found: $mChain }")
                        val m = mChain.head

                        if (m.body.isEmpty) fail(s"method body is empty: ${m.toJava}")

                        val d: Domain with RecordDefUse = Class.forName(domainName).asInstanceOf[Class[Domain
                            with RecordDefUse]].getConstructor(classOf[Project[_]], classOf[Method]).newInstance(p, m)
                        ...
                        val expectedFileName = projectName.substring(0, projectName.indexOf('.')).replace("-9", "") +
                        ...
                        if (expectedInputStream eq null) fail(
