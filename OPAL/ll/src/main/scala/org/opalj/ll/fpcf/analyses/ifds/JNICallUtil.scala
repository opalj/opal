...
object JNICallUtil {
    ...
    private def resolveMethodId(methodId: Value)(implicit declaredMethods: DeclaredMethods): Set[JNIMethod] = {
        ...
        sources.filter(_.isInstanceOf[Call]).map(_.asInstanceOf[Call]).flatMap(call => {
    ...
    private def findJavaMethods(classFqn: String, methodName: String, signature: String)(implicit declaredMethods: DeclaredMethods): Set[JNIMethod] = {
