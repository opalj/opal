...
abstract class NativeForwardTaintProblem(project: SomeProject)
    ...
    override def normalFlow(
        ...
        case store: Store =>
            in match {
                case NativeVariable(value) if value == store.src =>
                    store.dst match {
                        case dst: Alloca => Set(in, NativeVariable(dst))
                        case gep: GetElementPtr if gep.isConstant =>
                            Set(in, NativeArrayElement(gep.base, gep.constants))
                    }
                case NativeArrayElement(base, indices) if store.src == base =>
                    Set(in, NativeArrayElement(store.dst, indices))
        ...
        case load: Load =>
            in match {
                ...
                case NativeArrayElement(base, indices) =>
                    load.src match {
                        case gep: GetElementPtr if gep.isConstant && gep.base == base && gep.constants == indices =>
                            Set(in, NativeVariable(load))
        ...
        case add: Add =>
            in match {
                case NativeVariable(value) if value == add.op1 || value == add.op2 =>
                    Set(in, NativeVariable(add))
                case _ => Set(in)
            }
        case sub: Sub =>
            in match {
                case NativeVariable(value) if value == sub.op1 || value == sub.op2 =>
                    Set(in, NativeVariable(sub))
                case _ => Set(in)
            }
        case gep: GetElementPtr =>
            in match {
                case NativeVariable(value) if value == gep.base => Set(in, NativeVariable(gep))
                case NativeArrayElement(base, indices) if base == gep.base && gep.isZero =>
                    Set(in, NativeArrayElement(gep, indices))
                case _ => Set(in)
            }
        case bitcast: BitCast =>
            in match {
                case NativeVariable(value) if value == bitcast.operand(0) =>
                    Set(in, NativeVariable(bitcast))
                case _ => Set(in)
            }
        case _ => Set(in)
    }
    ...
    override def callFlow(
        ...
        case LLVMFunction(callee) =>
            in match {
                ...
                case NativeArrayElement(base, indices) =>
                    call.instruction.asInstanceOf[Call].indexOfArgument(base) match {
    ...
    override def outsideAnalysisContextUnbReturn(
        callee: NativeFunction
    ): Option[OutsideAnalysisContextUnbReturnHandler] = None
            ...
            case NativeVariable(value) =>
                call.instruction.asInstanceOf[Call].indexOfArgument(value) match {
                    case Some(index) =>
                        Set(
                            Variable(
                                JavaIFDSProblem.remapParamAndVariableIndex(
                                    index - 2,
                                    callee.isStatic
                                )
                            )
