...
import org.opalj.ll.fpcf.analyses.ifds.NativeIFDSProblem
import org.opalj.tac.fpcf.analyses.ifds.AbstractIFDSAnalysis.V
import org.opalj.tac.fpcf.analyses.ifds.taint.{Fact, NullFact, TaintProblem}
...
abstract class NativeForwardTaintProblem(project: SomeProject) extends NativeIFDSProblem[Fact](project) {
   ...
   */
  protected def createTaints(callee: DeclaredMethod, call: JavaStatement): Set[Fact]
   ...
   */
  private def createNewTaints(expression: Expr[V], statement: JavaStatement, in: Set[Fact]): Set[Fact] =
