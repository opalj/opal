...
trait SimpleBooleanPropertyTracing
    ...
    class BooleanProperty private[SimpleBooleanPropertyTracing] (val state: Boolean)
        ...
        def join(otherProperty: DomainProperty): Update[DomainProperty] = this.state & otherProperty.state match {
            case `state`  => NoUpdate
    ...
    def updateProperty(pc: Int, newState: Boolean): Unit = setProperty(pc, new BooleanProperty(newState))
    ...
    def hasPropertyOnExit: Boolean = allReturnFromMethodInstructions forall { pc => getProperty(pc).state }

    def hasPropertyOnNormalReturn: Boolean = allReturnFromMethodInstructions forall { pc =>
