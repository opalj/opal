...
trait LongSetValuesShiftOperators extends LongValuesDomain {
    ...
    def lshl(pc: Int, value: DomainValue, shift: DomainValue): DomainValue = {
        value match {
            case LongSetLike(leftValues) =>
                intValue[DomainValue](shift) { s => LongSet(leftValues.map(_ << s)) } {
            ...
            case _ =>
                intValue(shift) { s => if (s == 0) value else LongValue(origin = pc) } {
                    LongValue(origin = pc)
                }
        }
    }
    ...
    /*override*/
    def lshr(pc: Int, value: DomainValue, shift: DomainValue): DomainValue = {
        value match {
            case LongSetLike(leftValues) =>
                intValue[DomainValue](shift) { s => LongSet(leftValues.map(_ >> s)) } {
                    if (leftValues.size == 1 && leftValues.head == 0)
    ...
    /*override*/
    def lushr(pc: Int, value: DomainValue, shift: DomainValue): DomainValue = {
        value match {
