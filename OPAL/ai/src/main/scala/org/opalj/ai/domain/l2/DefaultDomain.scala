...
class DefaultDomain[Source](
        ...
        val maxCallChainLength: Int) extends SharedDefaultDomain[Source](project, method)
    ...
    def this(project: Project[Source], method: Method) = this(project, method, 256, 2)
    ...
    def shouldInvocationBePerformed(method: Method): Boolean = maxCallChainLength > 0 && !method.returnType.isVoidType
    ...
    lazy val calledMethodsStore: CalledMethodsStore { val domain: coordinatingDomain.type } = {
        val operands = localsArray(0).foldLeft(List.empty[DomainValue])((l, n) =>
