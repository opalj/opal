...
trait DefaultIntegerSetValues extends DefaultSpecialDomainValuesBinding with IntegerSetValues {
    ...
    class AnIntegerValue extends super.AnIntegerValueLike {
        ...
        override def adapt(target: TargetDomain, pc: Int): target.DomainValue = target.IntegerValue(pc)
    ...
    trait BaseTypesBasedSet extends super.BaseTypesBasedSetLike { this: DomainValue =>
        ...
        override def doJoin(pc: Int, other: DomainValue): Update[DomainValue] = other match {
            ...
            case that: BaseTypesBasedSet =>
                ...
                val newLB  = min(thisLB, that.lowerBound)
                val newUB  = max(thisUB, that.upperBound)
                if (thisLB == newLB && thisUB == newUB) MetaInformationUpdate(newInstance)
                else StructuralUpdate(approximateSet(pc, newLB, newUB))
            ...
            case IntegerSetLike(thatValues) =>
                ...
                if (thisLB <= thatLB && thisUB >= thatUB) MetaInformationUpdate(newInstance)
                else {
        ...
        override def abstractsOver(other: DomainValue): Boolean = (this eq other) || (
            other match {
                case other: BaseTypesBasedSet => lowerBound <= other.lowerBound && upperBound >= other.upperBound
                case that: IntegerSet         => lowerBound <= that.values.firstKey && upperBound >= that.values.lastKey
        ...
        override def equals(other: Any): Boolean = other match {
            case that: BaseTypesBasedSet => this.lowerBound == that.lowerBound && this.upperBound == that.upperBound
            case _                       => false
        }
    ...
    class ByteSet extends super.ByteSetLike with BaseTypesBasedSet {
        ...
        override def adapt(target: TargetDomain, pc: Int): target.DomainValue = target.ByteValue(pc)
        override def constantValue: Option[Int] = None
    }
    ...
    class ShortSet extends super.ShortSetLike with BaseTypesBasedSet {
        ...
        override def adapt(target: TargetDomain, pc: Int): target.DomainValue = target.ShortValue(pc)
        override def constantValue: Option[Int] = None
    }
    ...
    class CharSet extends super.CharSetLike with BaseTypesBasedSet {
        ...
        override def adapt(target: TargetDomain, pc: Int): target.DomainValue = target.CharValue(pc)
        override def constantValue: Option[Int] = None
    }
    ...
    override def BooleanValue(pc: Int): IntegerLikeValue =
        if (maxCardinalityOfIntegerSets > 1) IntegerSet(SortedSet(0, 1))
        else new ByteSet()
