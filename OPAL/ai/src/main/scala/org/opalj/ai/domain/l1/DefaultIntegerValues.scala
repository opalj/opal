...
trait DefaultIntegerValues extends DefaultSpecialDomainValuesBinding with IntegerValues {
    ...
    class ConcreteIntegerValue(val value: Int) extends super.TheIntegerValue {
        ...
        override def doJoin(pc: Int, other: DomainValue): Update[DomainValue] = other match {
            ...
            case that: ConcreteIntegerValue =>
                if (that.value == this.value) NoUpdate
                else StructuralUpdate(AnIntegerValue)
        }
        ...
        override def adapt(target: TargetDomain, pc: Int): target.DomainValue = target.IntegerValue(pc, value)
    ...
    override def IntegerValue(origin: ValueOrigin, value: Int): ConcreteIntegerValue = new ConcreteIntegerValue(value)
