...
trait DefaultTypeLevelReferenceValues
    ...
    protected[this] class ANullValue() extends super.NullValueLike { this: DomainNullValue =>
        ...
        override def abstractsOver(other: DomainValue): Boolean = other.isInstanceOf[ANullValue]
    }
    ...
    protected[this] trait AnArrayValue
        ...
        override def isAssignable(value: DomainValue): Answer = value match {
            ...
            case elementValue @ TypeOfReferenceValue(EmptyUpperTypeBound) =>
                ...
                if (theUpperTypeBound.componentType.isReferenceType) Yes
                else No

            case elementValue @ TypeOfReferenceValue(UIDSet1(elementValueType)) => classHierarchy.canBeStoredIn(
                    elementValueType,
        ...
        override protected def doStore(
            ...
            value:            DomainValue,
            index:            DomainValue,
            thrownExceptions: ExceptionValues): ArrayStoreResult = ComputationWithSideEffectOrException(thrownExceptions)
            ...
            other match {
                ...
                case SObjectValueLike(thatUpperTypeBound) =>
                    classHierarchy.joinAnyArrayTypeWithObjectType(thatUpperTypeBound) match {
                        case UIDSet1(newUpperTypeBound) =>
                            if (newUpperTypeBound eq `thatUpperTypeBound`) StructuralUpdate(other)
                            else StructuralUpdate(ReferenceValue(joinPC, newUpperTypeBound))
                        case newUpperTypeBound => StructuralUpdate(ObjectValue(joinPC, newUpperTypeBound))
                    }

                case MObjectValueLike(thatUpperTypeBound) =>
                    classHierarchy.joinAnyArrayTypeWithMultipleTypesBound(thatUpperTypeBound) match {
                        case `thatUpperTypeBound`       => StructuralUpdate(other)
                        case UIDSet1(newUpperTypeBound) => StructuralUpdate(ReferenceValue(joinPC, newUpperTypeBound))
                ...
                case AnArrayValue(thatUpperTypeBound) =>
                    classHierarchy.joinArrayTypes(thisUTB, thatUpperTypeBound) match {
                        case Left(`thisUTB`)            => NoUpdate
                        case Left(`thatUpperTypeBound`) => StructuralUpdate(other)
                        case Left(newUpperTypeBound)    => StructuralUpdate(ArrayValue(joinPC, newUpperTypeBound))
                        case Right(newUpperTypeBound)   => StructuralUpdate(ObjectValue(joinPC, newUpperTypeBound))
                    }
        ...
        override def abstractsOver(other: DomainValue): Boolean = other match {
            ...
            case AnArrayValue(thatUpperTypeBound) => domain.isSubtypeOf(thatUpperTypeBound, this.theUpperTypeBound)
            case _                                => false
        }
    ...
    protected trait SObjectValueLike
            ...
            other match {
                ...
                case SObjectValueLike(thatUpperTypeBound) =>
                    classHierarchy.joinObjectTypes(thisUpperTypeBound, thatUpperTypeBound, true) match {
                        case UIDSet1(newUpperTypeBound) =>
                            if (newUpperTypeBound eq `thisUpperTypeBound`) NoUpdate
                            else if (newUpperTypeBound eq `thatUpperTypeBound`) StructuralUpdate(other)
                            else StructuralUpdate(ObjectValue(pc, newUpperTypeBound))
                        case newUpperTypeBound => StructuralUpdate(ObjectValue(pc, newUpperTypeBound))
                    }

                case MObjectValueLike(thatUpperTypeBound) =>
                    classHierarchy.joinObjectTypes(thisUpperTypeBound, thatUpperTypeBound, true) match {
                        case `thatUpperTypeBound`          => StructuralUpdate(other)
                        case UIDSet1(`thisUpperTypeBound`) => NoUpdate
                        case newUpperTypeBound             => asStructuralUpdate(pc, newUpperTypeBound)
                    }

                case _: AnArrayValue => classHierarchy.joinAnyArrayTypeWithObjectType(thisUpperTypeBound) match {
                        case UIDSet1(newUpperTypeBound) =>
                            if (newUpperTypeBound eq `thisUpperTypeBound`) NoUpdate
                            else StructuralUpdate(ObjectValue(pc, newUpperTypeBound))
                        case newUpperTypeBound => StructuralUpdate(ObjectValue(pc, newUpperTypeBound))
        ...
        override def abstractsOver(other: DomainValue): Boolean = other match {
            case SObjectValueLike(thatUpperTypeBound) => domain.isSubtypeOf(thatUpperTypeBound, this.theUpperTypeBound)

            case AnArrayValue(thatUpperTypeBound) => domain.isSubtypeOf(thatUpperTypeBound, this.theUpperTypeBound)

            case MObjectValueLike(thatUpperTypeBound) => classHierarchy.isSubtypeOf(
    ...
    protected trait MObjectValueLike extends AnObjectValue with IsMObjectValue {
        ...
        override protected def doJoin(pc: Int, other: DomainValue): Update[DomainValue] = {
            ...
            other match {
                ...
                case SObjectValueLike(thatUTB) => classHierarchy.joinObjectTypes(thatUTB, thisUTB, true) match {
                        case `thisUTB`          => NoUpdate
                        case UIDSet1(`thatUTB`) => StructuralUpdate(other)
                        case newUTB             => asStructuralUpdate(pc, newUTB)
                    }

                case MObjectValueLike(thatUTB) => classHierarchy.joinUpperTypeBounds(thisUTB, thatUTB, true) match {
                        case `thisUTB` => NoUpdate
                        case `thatUTB` => StructuralUpdate(other)
                        case newUTB    => asStructuralUpdate(pc, newUTB)
                    }

                case _: AnArrayValue => classHierarchy.joinAnyArrayTypeWithMultipleTypesBound(thisUTB) match {
                        case `thisUTB` => NoUpdate
                        case newUTB    => asStructuralUpdate(pc, newUTB)
                    }
        ...
        override def adapt(target: TargetDomain, origin: ValueOrigin): target.DomainValue = target match {
            case td: TypeLevelReferenceValues => td.ObjectValue(origin, upperTypeBound).asInstanceOf[target.DomainValue]
            case _                            => super.adapt(target, origin)
        }

        override def summarize(origin: ValueOrigin): this.type = this

        override def toString: String = upperTypeBound.map(_.toJava).mkString("ReferenceValue(", " with ", ")")
    }
