...
trait IntegerValues extends IntegerValuesDomain with ConcreteIntegerValues {
    ...
    @inline protected final def intValues[T](
      ...
      )(orElse: => T): T = intValue(value1) { v1 => intValue(value2) { v2 => f(v1, v2) } { orElse } } { orElse }
    ...
    // UNARY EXPRESSIONS
    //
    override def ineg(pc: Int, value: DomainValue): DomainValue = value match {
    ...
    override def iinc(pc: Int, value: DomainValue, increment: Int): DomainValue = value match {
    ...
    override def isub(pc: Int, left: DomainValue, right: DomainValue): DomainValue = intValues(left, right) { (l, r) =>
        IntegerValue(pc, l - r)
    } {
        IntegerValue(origin = pc)
    }
    ...
    override def iand(pc: Int, value1: DomainValue, value2: DomainValue): DomainValue = (value1, value2) match {
        case (_, TheIntegerValue(-1)) => value1
        case (_, TheIntegerValue(0))  => value2
        case (TheIntegerValue(-1), _) => value2
        case (TheIntegerValue(0), _)  => value1
    ...
    override def ior(pc: Int, value1: DomainValue, value2: DomainValue): DomainValue = (value1, value2) match {
