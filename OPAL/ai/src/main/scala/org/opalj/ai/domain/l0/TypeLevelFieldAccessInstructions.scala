...
trait TypeLevelFieldAccessInstructions extends FieldAccessesDomain {
    ...
    def doGetfield(
        pc:         Int,
        objectref:  DomainValue,
        fieldValue: DomainValue): Computation[DomainValue, ExceptionValue] = refIsNull(pc, objectref) match {
    ...
    def getstatic(
        ...
        declaringClass: ObjectType,
        fieldName:      String,
        fieldType:      FieldType): Computation[DomainValue, Nothing] = doGetstatic(pc, TypedValue(pc, fieldType))

    /*override*/
    def doGetstatic(
        pc:         Int,
        fieldValue: DomainValue): Computation[DomainValue, Nothing] = ComputedValue(fieldValue)
    ...
    def putfield(
        ...
        declaringClass: ObjectType,
        fieldName:      String,
        fieldType:      FieldType): Computation[Nothing, ExceptionValue] = refIsNull(pc, objectref) match {
        case Yes => throws(VMNullPointerException(pc))
        case Unknown if throwNullPointerExceptionOnFieldAccess =>
    ...
    def putstatic(
        ...
        declaringClass: ObjectType,
        fieldName:      String,
        fieldType:      FieldType): Computation[Nothing, Nothing] = ComputationWithSideEffectOnly
