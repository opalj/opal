...
trait LongValues extends LongValuesDomain with ConcreteLongValues {
    ...
    @inline final protected def longValues[T](
      ...
      )(orElse: => T): T = longValue(value1) { v1 =>
    ...
    override def lcmp(pc: Int, left: DomainValue, right: DomainValue): DomainValue = (left, right) match {
    ...
    override def ladd(pc: Int, value1: DomainValue, value2: DomainValue): DomainValue = (value1, value2) match {
        case (TheLongValue(l), TheLongValue(r)) => LongValue(pc, l + r)
        case _                                  => LongValue(origin = pc)
    }

    override def lsub(pc: Int, left: DomainValue, right: DomainValue): DomainValue = (left, right) match {
        case (TheLongValue(l), TheLongValue(r)) => LongValue(pc, l - r)
        case _                                  => LongValue(origin = pc)
    }

    override def lmul(pc: Int, value1: DomainValue, value2: DomainValue): DomainValue = (value1, value2) match {
    ...
    override def ldiv(
        ...
        case (TheLongValue(n), TheLongValue(d)) =>
            if (d == 0L) ThrowsException(VMArithmeticException(pc))
            else ComputedValue(LongValue(pc, n / d))
    ...
    override def land(pc: Int, value1: DomainValue, value2: DomainValue): DomainValue = (value1, value2) match {
        case (_, TheLongValue(-1L)) => value1
        case (_, TheLongValue(0L))  => value2
        case (TheLongValue(-1L), _) => value2
        case (TheLongValue(0L), _)  => value1
    ...
    override def lor(pc: Int, value1: DomainValue, value2: DomainValue): DomainValue = (value1, value2) match {
    ...
    override def lxor(pc: Int, value1: DomainValue, value2: DomainValue): DomainValue = (value1, value2) match {
        case (TheLongValue(l), TheLongValue(r)) => LongValue(pc, l ^ r)
