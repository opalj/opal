...
object XHTML {
    ...
    private[this] def lastDump_=(currentTimeMillis: Long): Unit = _lastDump.set(currentTimeMillis)
    ...
    def annotationsAsXHTML(method: Method): Node = <div class="annotations">
            {
    ...
    def dump(
        ...
        val title = classFile.map(_.thisType.toJava + method.map("{ " + methodToString(_) + " }").getOrElse("")).orElse(
            method.map(methodToString))
    ...
    private def dumpInstructions(
        ...
        ids: Option[AnyRef => Int]): Array[Node] = {
    ...
    def dumpStack(operands: Operands[_ <: AnyRef])(implicit ids: Option[AnyRef => Int]): Node =
        if (operands eq null) <em>Information about operands is not available.</em>
        else {
            <ul class="Stack">
    ...
    def dumpLocals(locals: Locals[_ <: AnyRef])(implicit ids: Option[AnyRef => Int]): Node = {
        ...
        def mapLocal(local: AnyRef): Node =
            if (local eq null) <li><span class="unused">{"UNUSED"}</span></li>
            else <li><span>{valueToString(local)}</span></li>

        if (locals eq null) <em>Information about the local variables is not available.</em>
        else {
