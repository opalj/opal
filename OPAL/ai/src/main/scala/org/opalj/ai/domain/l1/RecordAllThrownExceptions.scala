...
trait RecordAllThrownExceptions extends domain.RecordThrownExceptions {
    ...
    override protected[this] def recordThrownException(
        pc:    Int,
        value: ExceptionValue): ThrownException = value match {
    ...
    override protected[this] def joinThrownExceptions(
        ...
        value:                     ExceptionValue): ThrownException = value match {
        case DomainMultipleReferenceValuesTag(value)   => previouslyThrownException ++ value.values
        case DomainSingleOriginReferenceValueTag(sorv) => previouslyThrownException + sorv
    }
}
