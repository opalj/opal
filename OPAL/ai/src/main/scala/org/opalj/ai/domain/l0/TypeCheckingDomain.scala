...
final class TypeCheckingDomain(
        ...
        val method: Method) extends Domain
    ...
    protected case class InitializedObjectValue(
            override val theUpperTypeBound: ObjectType) extends SObjectValueLike with Value {
    ...
    protected case class UninitializedObjectValue(
            ...
            origin: ValueOrigin) extends SObjectValueLike {
        this: DomainObjectValue =>
        ...
        final override def verificationTypeInfo: VerificationTypeInfo =
            if (origin == -1) UninitializedThisVariableInfo
            else UninitializedVariableInfo(origin)
        ...
        override def toString: String =
            if (origin == -1) "UninitializedThis"
            else s"${theUpperTypeBound.toJava}(uninitialized;origin=$origin)"
    }
    ...
    protected case class DefaultArrayValue(
            theUpperTypeBound: ArrayType) extends AnArrayValue {
        override def isNull: Answer = Unknown
    }
