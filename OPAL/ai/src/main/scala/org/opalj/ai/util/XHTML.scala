...
object XHTML {
    ...
    def styles: String = process(this.getClass.getResourceAsStream("default.css"))(
        scala.io.Source.fromInputStream(_).mkString
    )

    def jquery: String = process(this.getClass.getResourceAsStream("jquery-3.6.1.min.js"))(
        scala.io.Source.fromInputStream(_).mkString
    )

    def colResizable: String = process(this.getClass.getResourceAsStream("colResizable-1.6.min.js"))(
        scala.io.Source.fromInputStream(_).mkString
    )
    ...
    def htmlify(s: String): Node = scala.xml.Unparsed(
        s.replace("<", "&lt;").replace(">", "&gt;").replace("\n", "<br>").replace("\t", "&nbsp;&nbsp;")
    )

    def valueToString(value: AnyRef)(implicit ids: Option[AnyRef => Int]): String =
        if (value != null) value.toString + ids.map("@" + _.apply(value)).getOrElse("")
        else "null@<N/A-value>"
    ...
    def throwableToXHTML(throwable: Throwable): scala.xml.Node = {
        val node =
            if (throwable.getStackTrace == null ||
                ...
                val stackElements = for { stackElement <- throwable.getStackTrace } yield {
                    <tr>
    ...
    def instructionsToXHTML(
        title:        String,
        instructions: { def mkString(sep: String): String }): Node = <p>
