...
class MultiTracer(val tracers: AITracer*) extends AITracer {
    ...
    override def initialLocals(domain: Domain)(locals: domain.Locals): Unit = tracers foreach { tracer =>
        tracer.initialLocals(domain)(locals)
    }
    ...
    override def instructionEvalution(
        ...
        locals:      domain.Locals): Unit = tracers foreach { tracer =>
    ...
    override def flow(
        ...
        targetPC:                 Int,
        isExceptionalControlFlow: Boolean): Unit = tracers foreach {
        _.flow(domain)(currentPC, targetPC, isExceptionalControlFlow)
    }

    override def deadLocalVariable(domain: Domain)(pc: Int, lvIndex: Int): Unit = tracers foreach {
        _.deadLocalVariable(domain)(pc, lvIndex)
    }

    override def noFlow(
        domain:    Domain
      )(currentPC: Int,
        targetPC:  Int): Unit = tracers foreach { _.noFlow(domain)(currentPC, targetPC) }
    ...
    override def rescheduled(
        ...
        worklist:                 List[Int /*PC*/ ]): Unit = tracers foreach { tracer =>
    ...
    override def join(
        ...
        result:        Update[(domain.Operands, domain.Locals)]): Unit = tracers foreach { tracer =>
        tracer.join(
            domain
        )(
            pc,
    ...
    override def abruptMethodExecution(
        ...
        exception: domain.ExceptionValue): Unit = tracers foreach { _.abruptMethodExecution(domain)(pc, exception) }
    ...
    override def ret(
        ...
        newWorklist:     List[Int /*PC*/ ]): Unit = tracers foreach { tracer =>
    ...
    override def jumpToSubroutine(
        ...
        target:       Int,
        nestingLevel: Int): Unit = tracers foreach { _.jumpToSubroutine(domain)(pc, target, nestingLevel) }

    override def returnFromSubroutine(
        domain:        Domain
      )(pc:            Int,
        returnAddress: Int,
        subroutinePCs: List[Int]): Unit = tracers foreach { tracer =>
    ...
    override def result(result: AIResult): Unit = tracers foreach { _.result(result) }
    ...
    override def establishedConstraint(
        ...
        newLocals:   domain.Locals): Unit = tracers foreach { tracer =>
    ...
    override def domainMessage(
        ...
        typeID:  String,
        pc:      Option[Int /*PC*/ ],
        message: => String): Unit = tracers foreach { tracer =>
