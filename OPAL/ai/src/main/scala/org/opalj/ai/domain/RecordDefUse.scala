...
trait RecordDefUse extends RecordCFG { defUseDomain: Domain with TheCode =>
    ...
    protected[this] def thisProperty(pc: Int): Option[String] = Option(usedBy(pc)).map(_.mkString("UsedBy={", ",", "}"))
    ...
    abstract override def properties(pc: Int, propertyToString: AnyRef => String): Option[String] =
        super.properties(pc, propertyToString) match {
            case superProperty @ Some(description) => thisProperty(pc) map (_ + "; " + description) orElse superProperty
            case None                              => thisProperty(pc)
        }
    ...
    def usedBy(valueOrigin: ValueOrigin): ValueOrigins =
        if (valueOrigin > ImmediateVMExceptionsOriginOffset) used(valueOrigin + parametersOffset)
        else usedExternalExceptions(underlyingPC(valueOrigin))
    ...
    def safeUsedBy(valueOrigin: ValueOrigin): ValueOrigins = {
        ...
        if (usedBy eq null) NoValueOrigins
        else usedBy
    }
    ...
    private[this] def updateUsageInformation(usedValues: ValueOrigins, useSite: PC): Unit = usedValues foreach {
        usedValue =>
    ...
    protected[this] def propagate(
        ...
        cfJoins:       IntTrieSet,
        subroutinePCs: IntArraySet): Boolean = {
        if (cfJoins.contains(successorPC) && defLocals(successorPC) != null /*non-dead*/ ) {
            ...
            @tailrec def joinDefOps(
                ...
                if (newHead.subsetOf(oldHead)) joinDefOps(
                    oldDefOps,
                    lDefOps.tail,
                    rDefOps.tail,
                    oldIsSuperset,
            ...
            if (newDefLocals ne oldDefLocals) {
                ...
                var newUsage = false
                val joinedDefLocals = oldDefLocals.fuse(
                    newDefLocals,
                    (o, n) =>
    ...
    protected[this] def newDefOpsForExceptionalControlFlow(
        ...
        operandsArray: OperandsArray): List[ValueOrigins] = {
        ...
        val origins = originsOf(operandsArray(successorPC).head) match {
            case None =>
                                     ...
                                     !foundDefinitiveHandler && (
                                         (eh.catchType.isEmpty && { foundDefinitiveHandler = true; true }) || {
                                             ...
                                             if (isHandled.isYes) {
                                                 ...
                                                 true
                                             } else if (isHandled.isYesOrUnknown) true
                                             else false
                                         }
                                     )
                                 }
    ...
    protected[this] def stackOperation(
        ...
        val newDefOps: List[ValueOrigins] =
            if (isExceptionalControlFlow) {
                ...
                else currentDefOps.drop(usedValues)
            }
    ...
    protected[this] def registerReadWrite(
        ...
        cfJoins:       IntTrieSet,
        subroutinePCs: IntArraySet,
        localsArray:   LocalsArray): Boolean = {
    ...
    abstract override def abstractInterpretationEnded(
        ...
        def handleFlow(
            ...
            def stackOperation(usedValues: Int, pushesValue: Boolean): Boolean = defUseDomain.stackOperation(
                currentPC,
                currentInstruction,
                successorPC,
            ...
            val scheduleNextPC: Boolean = (currentInstruction.opcode: @switch) match {
                case GOTO.opcode | GOTO_W.opcode |
                    NOP.opcode |
                    WIDE.opcode => propagate(defOps(currentPC), defLocals(currentPC))
                ...
                case IFNULL.opcode | IFNONNULL.opcode
                    ...
                    | LOOKUPSWITCH.opcode | TABLESWITCH.opcode => stackOperation(1, pushesValue = false)
                ...
                // ARRAYS
                //
                case NEWARRAY.opcode | ANEWARRAY.opcode => stackOperation(1, pushesValue = true)

                case ARRAYLENGTH.opcode => stackOperation(1, pushesValue = true)
                ...
                case 83 /*aastore*/ |
                    ...
                    82 /*dastore*/ | 81 /*fastore*/ => stackOperation(3, pushesValue = false)

                //
                // FIELD ACCESS
                //
                case 180 /*getfield*/  => stackOperation(1, pushesValue = true)
                case 178 /*getstatic*/ => stackOperation(0, pushesValue = true)
                case 181 /*putfield*/  => stackOperation(2, pushesValue = false)
                case 179 /*putstatic*/ => stackOperation(1, pushesValue = false)

                //
                // MONITOR
                //

                case 194 /*monitorenter*/ => stackOperation(1, pushesValue = false)
                case 195 /*monitorexit*/  => stackOperation(1, pushesValue = false)
                ...
                case 42 /*aload_0*/ |
                    38 /*dload_0*/ | 34 /*fload_0*/ | 26 /*iload_0*/ | 30 /*lload_0*/ => load(0)
                case 43 /*aload_1*/ |
                    39 /*dload_1*/ | 35 /*fload_1*/ | 27 /*iload_1*/ | 31 /*lload_1*/ => load(1)
                case 44 /*aload_2*/ |
                    40 /*dload_2*/ | 36 /*fload_2*/ | 28 /*iload_2*/ | 32 /*lload_2*/ => load(2)
                case 45 /*aload_3*/ |
                    41 /*dload_3*/ | 37 /*fload_3*/ | 29 /*iload_3*/ | 33 /*lload_3*/ => load(3)

                case 58 /*astore*/ |
                    57 /*dstore*/ | 56 /*fstore*/ | 54 /*istore*/ | 55 /*lstore*/ =>
                    store(currentInstruction.asStoreLocalVariableInstruction.lvIndex)
                case 75 /*astore_0*/ |
                    71 /*dstore_0*/ | 67 /*fstore_0*/ | 63 /*lstore_0*/ | 59 /*istore_0*/ => store(0)
                case 76 /*astore_1*/ |
                    72 /*dstore_1*/ | 68 /*fstore_1*/ | 64 /*lstore_1*/ | 60 /*istore_1*/ => store(1)
                case 77 /*astore_2*/ |
                    73 /*dstore_2*/ | 69 /*fstore_2*/ | 65 /*lstore_2*/ | 61 /*istore_2*/ => store(2)
                case 78 /*astore_3*/ |
                    74 /*dstore_3*/ | 70 /*fstore_3*/ | 66 /*lstore_3*/ | 62 /*istore_3*/ => store(3)
                ...
                case 1 /*aconst_null*/ |
                    ...
                    16 /*bipush*/ | 17 /*sipush*/ |
                    18 /*ldc*/ | 19 /*ldc_w*/ | 20 /*ldc2_w*/ => stackOperation(0, pushesValue = true)
                ...
                case 148 /*lcmp*/ |
                    150 /*fcmpg*/ | 149 /*fcmpl*/ |
                    152 /*dcmpg*/ | 151 /*dcmpl*/ => stackOperation(2, pushesValue = true)

                //
                // UNARY EXPRESSIONS
                //
                case 116 /*ineg*/ | 117 /*lneg*/ | 119 /*dneg*/ | 118 /*fneg*/ => stackOperation(1, pushesValue = true)

                case NEW.opcode => stackOperation(0, pushesValue = true)
                ...
                case 99 /*dadd*/ | 111 /*ddiv*/ | 107 /*dmul*/ | 115 /*drem*/ | 103 /*dsub*/ |
                    ...
                    121 /*lshl*/ | 123 /*lshr*/ | 125 /*lushr*/ => stackOperation(2, pushesValue = true)
                ...
                case 91 /*dup_x2*/ => operandsArray(currentPC) match {
                ...
                case 92 /*dup2*/ => operandsArray(currentPC) match {
                        case (_ @CTC1()) :: _ =>
                            val currentDefOps = defOps(currentPC)
                ...
                case 93 /*dup2_x1*/ => operandsArray(currentPC) match {
                        case (_ @CTC1()) :: _ =>
                            val v1 :: v2 :: v3 :: rest = defOps(currentPC)
                ...
                case 94 /*dup2_x2*/ => operandsArray(currentPC) match {
                ...
                case 87 /*pop*/ => propagate(defOps(currentPC).tail, defLocals(currentPC))
                case 88 /*pop2*/ =>
                    if (operandsArray(currentPC).head.computationalType.operandSize == 1)
                        propagate(defOps(currentPC).drop(2), defLocals(currentPC))
                    else propagate(defOps(currentPC).tail, defLocals(currentPC))
                ...
                case 144 /*d2f*/ | 142 /*d2i*/ | 143 /*d2l*/ |
                    ...
                    193 /*instanceof*/ => stackOperation(1, pushesValue = true)
            ...
            if (jsrPCs.top.nonEmpty &&
                ...
                lastSubroutinePCs foreach { pc =>
                    ...
                    if (subroutineUsed(usedPC) == null) {
                        ...
                        if (usedPCs != null) subroutineUsed(usedPC) ++= usedPCs
                    }
                    ...
                    if (subroutineDefOps(pc) == null) {
                        ...
                        subroutineDefOps(pc) = (subroutineDefOps(pc) zip defOps(pc)).map(vos => vos._1 ++ vos._2)
                    }
                    if (subroutineDefLocals(pc) == null) {
                        subroutineDefLocals(pc) = defLocals(pc)
                    } else {
                        subroutineDefLocals(pc) = subroutineDefLocals(pc).fuse(
                            defLocals(pc),
                    ...
                    try {
                        ...
                        case e: Throwable =>
                            ...
                            try {
                                ...
                                val localsDump = defLocals(currentPC).zipWithIndex.map { e =>
                                ...
                                case t: Throwable => message += s"<fatal error while collecting : ${t.getMessage}>"
                            }
                            // val htmlMessage =
                            // message.
                            //     replace("\n", "<br>").
        ...
        if (subroutinePCs.nonEmpty) {
            foreachNonNullValue(subroutineDefOps) { (pc, subroutineDefOpsAtPC) =>
                ...
                if (defOps(pc) == null) {
                    ...
                    defOps(pc) = (defOps(pc) zip subroutineDefOps(pc)).map(vos => vos._1 ++ vos._2)
                }
                if (defLocals(pc) == null) {
                    defLocals(pc) = subroutineDefLocals(pc)
                } else {
                    defLocals(pc) = defLocals(pc).fuse(
                ...
                if (allSubroutineUsedExternalExceptions != null) {
                    usedExternalExceptions(pc) =
                        if (oldUsedExternalExceptions == null) allSubroutineUsedExternalExceptions
                        else oldUsedExternalExceptions ++ allSubroutineUsedExternalExceptions
                }
                ...
                if (allSubroutineUsed != null) {
                    used(usedPC) =
                        if (oldUsedPC == null) allSubroutineUsed
                        else oldUsedPC ++ allSubroutineUsed
                }
            }
        }
    }
     ...
     * information.
     */
    def dumpDefUseInfo(): Node = XHTML.createXHTML(Some("Definition/Use Information"), dumpDefUseTable())
    ...
    def dumpDefUseTable(): Node = {
        val instructions = code.instructions
        val perInstruction =
            defOps.zip(defLocals).zipWithIndex.filter(e => e._1._1 != null || e._1._2 != null).map { e =>
                val ((os, ls), i) = e
                val operands =
                    if (os eq null) <i>{"N/A"}</i>
                    else os.map { o => <li>{if (o eq null) "N/A" else o.mkString("{", ",", "}")}</li> }.toList

                val locals =
                    if (ls eq null) <i>{"N/A"}</i>
                    else ls.toSeq.reverse.map { e => <li>{if (e eq null) "N/A" else e.mkString("{", ",", "}")}</li> }
    ...
    def createDefUseGraph(code: Code): Set[DefaultMutableNode[ValueOrigin]] = {
        ...
        def instructionToString(vo: ValueOrigin): String =
            if (ai.isImplicitOrExternalException(vo)) s"<exception thrown by\\linstruction: ${ai.underlyingPC(vo)}>"
            else if (vo < 0) s"<parameter: ${-vo - 1}>"
            else s"$vo: " + code.instructions(vo).toString(vo)

        val unusedNode = new DefaultMutableNode(
        ...
        var nodes: Map[ValueOrigin, DefaultMutableNode[ValueOrigin]] = defSites.map { defSite =>
            val color =
                if (ai.isImplicitOrExternalException(defSite)) Some("orange")
                else if (defSite < 0) Some("green")
                else if (code.exceptionHandlers.exists { _.handlerPC == defSite }) Some("yellow")
                else None
            (
                defSite,
        ...
        defSites foreach { lvar =>
            ...
            if ((usages eq null) || usages.isEmpty) unusedNode.addChild(thisNode)
            else usages.foreach { usage =>
                val usageNode = nodes.get(usage)
                if (usageNode.isDefined) usageNode.get.addChild(thisNode)
                else {
