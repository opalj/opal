...
abstract class ProjectLocation(
        ...
        val details: Seq[IssueDetails] = List.empty) extends IssueLocation {

    def compareTo(other: IssueLocation): Int = other match {
        case _: InstructionLocation => 1
        case _: MethodLocation      => 1
        case _: FieldLocation       => 1
        case _: ClassLocation       => 1
        case _: PackageLocation     => 1
        case that: ProjectLocation => that.theProject.hashCode() compare this.theProject.hashCode() match {
                case 0 => (this.description, that.description) match {
                        case (None, None)       => 0
                        case (Some(_), None)    => -1
                        case (None, Some(_))    => 1
    ...
    override def toAnsiColoredString: String = description.map(_.replace('\n', ';')).getOrElse("")

    override def toEclipseConsoleString: String = description.map(_.replace('\n', ';')).getOrElse("")
}
...
class ClassLocation(
    ...
    override def locationAsInlineXHTML(basicInfoOnly: Boolean): List[Node] = {
        ...
        if (basicInfoOnly || classFile.accessFlags == 0) locationAsInlineXHTML
        else classAccessFlagsToXHTML(classFile.accessFlags) :: Text(" ") :: locationAsInlineXHTML
    }

    override def toAnsiColoredString: String =
        theProject.source(classFile.thisType).map(_.toString).getOrElse("<No Source>") + ":"

    override def locationAsIDL: JsObject = super.locationAsIDL + (
        (
            "class",
            Json.obj(
                "fqn"  -> classFile.fqn,
...
class MethodLocation(
    ...
    override def locationAsInlineXHTML(basicInfoOnly: Boolean): List[Node] = {
        var methodNode = <span class="method" data-method={methodJVMSignature}>
                {
            if (basicInfoOnly) methodToXHTML(method.name, method.descriptor, true)
            else methodToXHTML(method.accessFlags, method.name, method.descriptor, true)
...
class InstructionLocation(
        ...
        method:      Method,
        val pc:      PC,
        details: Seq[IssueDetails] = List.empty) extends MethodLocation(description, theProject, method, details)
...
class FieldLocation(
        ...
        classFile:   ClassFile,
        val field:   Field,
        details: Seq[IssueDetails] = List.empty) extends ClassLocation(description, theProject, classFile, details)
