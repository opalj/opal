...
class Operands(
    ...
    def toXHTML(basicInfoOnly: Boolean): Node = {
        val detailsNodes = instruction match {
            ...
            case cbi: CompoundConditionalBranchInstruction => Seq(
            ...
            case smi: StackManagementInstruction =>
                val representation = <span class="keyword">{smi.mnemonic} </span> ::
            ...
            case instruction =>
                val operandsCount = instruction.numberOfPoppedOperands { x => throw new UnknownError() }

                val parametersNode = operands.take(operandsCount).reverse.map { op => <span class="value">{op} </span> }
                List(
    ...
    override def toIDL: JsValue = instruction match {
        case cbi: SimpleConditionalBranchInstruction[_] => Json.obj(
        ...
        case instruction =>
            val operandsCount = instruction.numberOfPoppedOperands { x =>
