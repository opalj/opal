...
class LocalVariables(
        val code: Code,
        val pc:   PC,
        val localVariables: Locals[_ <: AnyRef]) extends IssueDetails {
    ...
    def toXHTML(basicInfoOnly: Boolean): Node = {
        ...
        if (basicInfoOnly) return Text("");

        val sortedLVDefs = localVariableDefinitions.toSeq.sortWith((a, b) => a._1 < b._1)
        val lvsAsXHTML = for ((index, theLV) <- sortedLVDefs) yield {
            val localValue = localVariables(index)
            val localValueAsXHTML =
                if (localValue == null) <span class="warning">unused</span>
                else Text(localVariableToString(theLV, localValue))
    ...
    override def toIDL: JsValue = {
        ...
        val lvValues = lvDefs.toSeq.sortWith((a, b) => a._1 < b._1).map { e =>
            ...
            Json.obj(
                ...
                "value" -> {
                    if (localValue == null) JsNull
                    else localVariableToString(localVariable, localValue)
                }
            )
        }
