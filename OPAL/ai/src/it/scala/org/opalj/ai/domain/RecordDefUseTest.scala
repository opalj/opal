...
class RecordDefUseTest extends AnyFunSpec with Matchers {
    ...
    protected[this] class DefUseDomain(
            val method: Method,
            val project: Project[URL]) extends CorrelationalDomain
            with TheProject
            with TheMethod
    ...
    protected[this] class RefinedDefUseDomain(
            method: Method,
            project: Project[URL]) extends DefUseDomain(method, project)
    ...
    protected[this] def analyzeDefUse(
        ...
        refinedDefUseInformation: Boolean): Unit = {
        ...
        for {
            ...
            d.safeUsedBy(pc) foreach { useSite =>
                ...
                val hasDefSite = (0 until poppedOperands).exists { poIndex =>
            ...
            for { (op, opIndex) <- ops.iterator.zipWithIndex } {
                ...
                domainOrigins foreach { o =>
                    if (!(
                        ...
                        val message = s"{pc=$pc:$instruction[isHandler=$isHandler][operand=$opIndex] " +
                            s"deviating def/use info: " +
                            s"domain=$domainOrigins vs defUse=$defUseOrigins}"
                        val messageHeader = if (refinedDefUseInformation) "[using domain.origin]" else ""
                ...
                if (opIndex < usedOperands &&
                    ...
                    defUseOrigins foreach { defUseOrigin => // the origins of a value...
                        ...
                        if (defUseUseSites == null) {
                            ...
                            val defSiteBelongsToSubroutine = belongsToSubroutine(underlyingPC(defUseOrigin))
                            val useSiteBelongsToSubroutine = belongsToSubroutine(pc)
                            fail(
