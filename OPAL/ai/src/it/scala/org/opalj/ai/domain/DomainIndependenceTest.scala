...
class DomainIndependenceTest extends AnyFlatSpec with Matchers {
    ...
    it should "always calculate the same result" in {
            ...
            r1.operandsArray.corresponds(r2.operandsArray) { (lOperands, rOperands) =>
                ...
                (lOperands != null && rOperands != null &&
                    lOperands.corresponds(rOperands) { (lValue, rValue) =>
                        ...
                        if (!(lVD.abstractsOver(rVD) && rVD.abstractsOver(lVD))) return Some(
                        ...
                        (lValue == null && rValue == null) || (
                            lValue != null && rValue != null && {
                                ...
                                if (lVD.isInstanceOf[ValuesDomain.ReturnAddressValue] || rVD.isInstanceOf[ValuesDomain.ReturnAddressValue]) {
                                    if ((lVD.isInstanceOf[ValuesDomain.ReturnAddressValue] && !rVD.isInstanceOf[
                                            ValuesDomain.ReturnAddressValue]) ||
                                        (rVD.isInstanceOf[ValuesDomain.ReturnAddressValue] && !lVD.isInstanceOf[
                                            ValuesDomain.ReturnAddressValue])) return Some(
                                        Console.BLUE_B + "the register value " + lVD + " does not correspond with " + rVD)
                                    else true
                                } else if (!(lVD.abstractsOver(rVD) && rVD.abstractsOver(lVD))) return Some(
                                    Console.BLUE_B + "the register value " + lVD + " does not correspond with " + rVD)
                                else true
                            }
                        )
        ...
        for {
            ...
            def abort(ai: InstructionCountBoundedAI[_], r: AIResult): Unit = fail(
                    ...
                    info(
                        classFile.thisType.toJava + "{ " +
                            method.signatureToJava(
                                false) + "(Instructions " + method.body.get.instructions.size + ")}\n" +
        ...
        if (comparisonCount.get() < 2) fail("did not find any class files/method to analyze")
        if (failed.get() > 0) {
