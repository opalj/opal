    ...
    def analyzeProject(
        ...
        time {            // 1. Test eager analysis...
           ...
           time { // 2. Test lazy analysis...
                ...
                try {p.parForeachMethodWithBody() { mi =>
                    counter.incrementAndGet() % 3 match {
                        case 0 => ps.force(mi.method, BaseAIResult.key)
                        case 1 => ps.force(mi.method, TACAI.key)
                        case 2 =>
