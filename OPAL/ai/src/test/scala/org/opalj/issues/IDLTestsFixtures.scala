...
object IDLTestsFixtures {
    ...
    private[issues] def toIDL(relevance: Relevance): JsObject = relevanceToIDL(relevance.name, relevance.value)

    private[issues] def relevanceToIDL(name: String, value: Int): JsObject = Json.obj("name" -> name, "value" -> value)
    ...
    val simpleLocalVariablesIDL: JsObject = Json.obj("type" -> "LocalVariables", "values" -> Json.arr())
