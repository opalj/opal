...
object PerformInvocationsWithRecursionDetectionTestFixture {
    ...
    abstract class SharedInvocationDomain(
            ...
            val method: Method) extends BaseDomain(project) with Domain
            with TheMethod
    ...
    class InvocationDomain(
            project: Project[java.net.URL],
            method:  Method,
            val frequentEvaluationWarningLevel: Int = 10) extends SharedInvocationDomain(project, method) {
        callingDomain =>

        lazy val calledMethodsStore
            : CalledMethodsStore { val domain: coordinatingDomain.type; def warningIssued: Boolean } = {
            val operands = mapOperands(
        ...
        override def calledMethodDomain(method: Method) = new ChildInvocationDomain(project, method, this)
    ...
    class ChildInvocationDomain(
            project: Project[java.net.URL],
            method:  Method,
            val callerDomain: SharedInvocationDomain) extends SharedInvocationDomain(project, method)
        ...
        def calledMethodDomain(method: Method) = new ChildInvocationDomain(project, method, callingDomain)
