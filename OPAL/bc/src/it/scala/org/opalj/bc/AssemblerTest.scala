    ...
    for {
        ...
        it should s"be able to recreate every class of $file" in {
                ...
                try {
                    ...
                    reassembledClassFile.zip(raw).zipWithIndex foreach { e =>
                        ...
                        if (c != r) {
                            ...
                            val successfullyReadBytes = s"(i.e., successfully read ${succeeded.last._2} bytes)"
                            val message = s"the class files differ starting with index $i ($failedSegment): " +
            ...
            if (exceptions.nonEmpty) {
                ...
                val message = exceptions.mkString(
