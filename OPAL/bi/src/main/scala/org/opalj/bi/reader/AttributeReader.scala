...
trait AttributeReader extends Constant_PoolAbstractions with AttributesAbstractions {
    ...
    def registerAttributeReader(
        reader: (
            String,
            (Constant_Pool,
                AttributeParent, /* the (class|field|method)name index of the attribute parent */ Constant_Pool_Index,
                /* the (field|method)descriptor index of the parent or -1 in case of a class */ Constant_Pool_Index,
                /* attribute_name_index */ Constant_Pool_Index,
                DataInputStream) => Attribute)): Unit
