...
object DependencyAnalysis extends AnalysisApplication {
    ...
    if (template eq null) throw new RuntimeException(
    ...
    def readParameter(param: String, args: Seq[String], default: String = ""): (String, Seq[String]) =
        args.partition(_.startsWith("-" + param + "=")) match {
            ...
            case (Seq(p), parameters1) => {
                ...
                else (p.substring(param.length + 2), parameters1)
            }
        }
    ...
    override def checkAnalysisSpecificParameters(args: Seq[String]): Iterable[String] = {
        ...
        if (parameters4.isEmpty) Iterable.empty
        else parameters4.map("unknown parameter: " + _)
    }
        ...
        def analyze(
            ...
            val dependencyProcessor = new DependencyProcessor {
                ...
                def addDependency(sourcePN: String, targetPN: String): Unit = {
                    ...
                    if (filter != "" && !sourcePackage.startsWith(filter) || !targetPackage.startsWith(filter)) return
                ...
                override def processDependency(
                    ...
                    dType:  DependencyType): Unit = if (source.isClass && target.isClass) addDependency(
                ...
                def getPackageName(pn: String): String = {
                    if (pn == "") // standard package = <default>
                        return "<default>"
                    if (mainPackage != "" && pn.startsWith(mainPackage)) return pn
                ...
                def processDependency(
                    source:    VirtualSourceElement,
                    arrayType: ArrayType,
                    dType:     DependencyType): Unit = if (source.isClass && arrayType.componentType.isObjectType)
                    addDependency(
                        source.asInstanceOf[VirtualClass].thisType.packageName,
            ...
            var data = ("[" + packages.foldRight("")((p1, l1) =>
                "[" +
                    packages.foldRight("")((p2, l2) =>
                        s"${dependencyProcessor.currentDependencyCount(p1, p2) / maxCount},$l2") + "]," + l1) + "]").replaceAll(
                ",]",
                "]")

            if (inverse) data = "d3.transpose(" + data + ")"

            val cS = """ style="border-style:solid;border-width:1px;""""

            val addOut =
                if (debug) "<table> <tr><th" + cS + "></th>" + packages.foldRight("</tr>")((p, l) =>
                    "<th" + cS + ">" + p + "</th>" + l) + packages.foldRight("</table>")((p1, l1) =>
                    "<tr><td" + cS + "><b>" + p1 + "</b></td>" +
                        packages.foldRight("</tr>\n")((p2, l2) =>
                            "<td" + cS + ">" + (dependencyProcessor.currentDependencyCount(p1, p2)) + "</td>" + l2) + l1)
                else ""
            // read the template
