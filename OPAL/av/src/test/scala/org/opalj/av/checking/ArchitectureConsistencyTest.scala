...
class ArchitectureConsistencyTest extends AnyFlatSpec with Matchers with BeforeAndAfterAll {
    ...
    def testEnsemblesAreNonEmpty(specification: Specification): Unit = specification.ensembles.foreach { e =>
        val (ensembleID, (matcher, extent)) = e
        if (ensembleID != Symbol("Empty") && extent.isEmpty) fail(s"$ensembleID didn't match any elements ($matcher)")
    }
