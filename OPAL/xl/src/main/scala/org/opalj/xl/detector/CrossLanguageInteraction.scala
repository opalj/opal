...
case class ScriptEngineInteraction[ContextType, PointsToSet](
    ...
    javaScriptFunctionCalls: List[JavaScriptFunctionCall[ContextType, PointsToSet]] =
        List.empty[JavaScriptFunctionCall[ContextType, PointsToSet]],
    puts: Map[(String, ContextType, TheTACAI), (PointsToSet, V, Option[ObjectType])] =
        Map.empty[(String, ContextType, TheTACAI), (PointsToSet, V, Option[ObjectType])]
) extends CrossLanguageInteraction {
    def updated(scriptEngineInteraction: ScriptEngineInteraction[ContextType, PointsToSet]): ScriptEngineInteraction[
        ContextType,
        PointsToSet
    ] = {
