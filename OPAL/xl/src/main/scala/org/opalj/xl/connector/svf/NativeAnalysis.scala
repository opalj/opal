...
abstract class NativeAnalysis(
    ...
    def runSVF(implicit svfConnectorState: SVFConnectorState): ProperPropertyComputationResult = {
        ...
        for (f <- functionSelection) {
            val resultPTS = svfConnectorState.svfModule.processFunction(f, basePTS.toArray, parameterPointsToSets, new SVFAnalysisListener() {

                // called at GetArrayElement. index insensitive
                override def getArrayElement(basePTS: Array[Long]) = basePTS

                // called at SetArrayElement. index insensitive
                override def setArrayElement(basePTS: Array[Long], argPTS: Array[Long]): Unit = {}
            ...
            println(s"result count: ${resultPTS.count}")
            resultPTS.foreach(l => {
                println(s"l: $l")
