...
abstract class NativeAnalysis(
    ...
    def runSVF(implicit svfConnectorState: SVFConnectorState): ProperPropertyComputationResult = this.synchronized {
        ...
        val listener = new SVFAnalysisListener() {
            ...
            override def getNativeFunctionArgument(methodName: String, argumentIndex: Int): Array[Long] = {
                ...
                val allNativeMethods = project.allProjectClassFiles.flatMap(_.methods).filter(_.isNative).map(method =>
                    ("Java/" + method.classFile.thisType.fqn + "/" + method.name) -> method
                ).toMap
            ...
            override def nativeToJavaCallDetected(
                basePTS:         Array[Long],
                className:       String,
                methodName:      String,
                methodSignature: String,
                argsPTSs:        Array[Array[Long]]
            ): Array[Long] = {
                        ...
                        tpe =>
                            if (tpe.isObjectType) {
                                ...
                                if (project.instanceMethods.contains(objectType)) {
                                    possibleMethods ++= project.instanceMethods(objectType).filter(
                                        _.name == methodName
                                    ).map(_.method)
                                }
                            }
                ...
                objectTypeOptional.foreach(objectType => {
                    if (possibleMethods.isEmpty) {
                        possibleMethods ++= project.allMethods.filter(method =>
                    ...
                    possibleMethods.foreach(method => {
                        ...
                        // Call Graph
                        svfConnectorState.connectorResults += PartialResult[DeclaredMethod, Callers](
                            declaredMethod,
                            Callers.key,
                            {
                ...
                for (l <- baseLongArray) {
                    ...
                    if (tpe.isDefined) {
                        ...
                        possibleFields.foreach(field => {
                            ...
                            svfConnectorState.indirectFieldAccesses.addFieldRead(
                                svfConnectorState.calleeContext,
                                5,
                                field,
                                None
                            )
                    ...
                    svfConnectorState.connectorResults ++= createResults ++ svfConnectorState.indirectFieldAccesses.partialResults(
                        svfConnectorState.calleeContext
                    )
                }
            ...
            override def setField(
                ...
                try { // TODO remove try
                    project.classFile(svfConnectorState.calleeContext.method.declaringClassType).get.fields.filter(
                        _.name.contains(fieldName)
                    ).map(declaredFields(_)).foreach(declaredField => {
                        svfConnectorState.indirectFieldAccesses.addFieldWrite(
                            svfConnectorState.callerContext,
                            svfConnectorState.pc,
                            declaredField,
                            None,
                            None
                        )
      ...
      function =>
        ...
        for (f <- functionSelection) {
           ...
           |""".stripMargin
            ...
            val resultPTS =
                svfConnectorState.svfModule.processFunction(f, basePTS.toArray, parameterPointsToSets, listener)
        ...
        Results(
            createResults ++ svfConnectorState.connectorResults,
            InterimPartialResult(svfConnectorState.connectorDependees, svfConnectorContinuation)
        )
    }
    ...
    def svfConnectorContinuation(eps: SomeEPS)(implicit
        svfConnectorState: SVFConnectorState
    ): ProperPropertyComputationResult = this.synchronized {
        svfConnectorState.connectorDependees =
            svfConnectorState.connectorDependees.filter(dependee => dependee.e != eps.e)
        eps match {
