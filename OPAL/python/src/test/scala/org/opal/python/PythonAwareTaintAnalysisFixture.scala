...
import org.opalj.python.PythonAwareTaintProblem
 ...
 */
class PythonAwareTaintAnalysisFixture(project: SomeProject)
    extends IFDSAnalysis()(project, new PythonAwareTaintProblemProblemFixture(project), Taint)

class PythonAwareTaintProblemProblemFixture(p: SomeProject) extends PythonAwareTaintProblem(p) {
    ...
    override val entryPoints: Seq[(Method, TaintFact)] = p.allProjectClassFiles.filter(classFile =>
        classFile.thisType.fqn == "org/opalj/fpcf/fixtures/python/Java2PythonTestClass")
...
object IFDSAnalysisPythonFixtureScheduler extends IFDSAnalysisScheduler[TaintFact, Method, JavaStatement] {
    override def init(p: SomeProject, ps: PropertyStore) = new PythonAwareTaintAnalysisFixture(p)
