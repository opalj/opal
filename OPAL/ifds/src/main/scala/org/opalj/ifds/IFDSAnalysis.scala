...
class IFDSFact[Fact <: AbstractIFDSFact, S <: Statement[_, _]](
    ...
    def this(fact: Fact) = this(fact, false, None, None)
    ...
    override def equals(obj: Any): Boolean = obj match {
        case other: IFDSFact[Fact @unchecked, S @unchecked] => this.eq(other) ||
    ...
    override def hashCode(): Int = 64 * (fact.hashCode() + isUnbalancedReturn.hashCode() + callStmt.hashCode())
}
...
case class Dependees[WorklistItem]() {
    ...
    def get(
        ...
        worklistItem:  WorklistItem): SomeEOptionP = {
    ...
    def forResult: Set[SomeEOptionP] = dependees.values.map(_.eOptionP).toSet
...
case class PathEdges[Fact <: AbstractIFDSFact, S <: Statement[_ <: C, _], C <: AnyRef](
     ...
     */
    def add(statement: S, fact: Fact, predecessor: Option[S] = None): Boolean = edges.get(statement) match {
        case None =>
            predecessor match {
                case Some(predecessor) => edges = edges.updated(statement, Right(Map(predecessor -> Set(fact))))
        ...
        case Some(Right(existingFacts)) => predecessor match {
    ...
    def debugData: Map[S, Set[Fact]] = edges.foldLeft(Map.empty[S, Set[Fact]]) { (result, elem) =>
...
class IFDSAnalysis[Fact <: AbstractIFDSFact, C <: AnyRef, S <: Statement[_ <: C, _]](
        ...
        val propertyKey: IFDSPropertyMetaInformation[S, Fact]) extends FPCFAnalysis {
    ...
    def performAnalysis(entity: (C, IFDSFact[Fact, S])): ProperPropertyComputationResult = {
        ...
        val startStatements: Set[(Option[S], Option[S])] =
            if (sourceFact.isUnbalancedReturn) {
    ...
    private def createPropertyValue()(implicit state: State): IFDSProperty[S, Fact] =
        ...
        else propertyKey.create(collectResult)
     ...
     */
    private def process()(implicit state: State, worklist: Worklist): Unit = while (worklist.nonEmpty) { // ifds line 10
        ...
        statement match {
            ...
            case None if icfg.isExitStatement(predecessor.get) => handleExit(predecessor.get, in.fact) // ifds line 21
    ...
    private def handleCall(
        ...
        for (callee <- callees) {
            val callFlowHandler = ifdsProblem.outsideAnalysisContextCall(callee) match {
                case Some(outsideAnalysisHandler) => outsideAnalysisHandler
    ...
    private def concreteCallFlow(
        ...
        val entryFacts     = icfg.startStatements(callee).flatMap(entry => callFlow(entry, in, call, callee))
        ...
        for (entryFact <- entryIFDSFacts) { // ifds line 14
            ...
            val exitFacts: Map[S, Set[Fact]] =
                if (newEntity == state.source) {
                    ...
                    callFlows match {
                        case ep: FinalEP[_, IFDSProperty[S, Fact]]     => ep.p.flows
                        case ep: InterimEUBP[_, IFDSProperty[S, Fact]] => ep.ub.flows
                        case _                                         => Map.empty
                    }
                }
            for {
    ...
    private def handleOther(
        statement:   S,
        in:          Fact,
        predecessor: Option[S]
      )(implicit
        state:    State,
        worklist: Worklist): Unit = {
...
abstract class IFDSAnalysisScheduler[Fact <: AbstractIFDSFact, C <: AnyRef, S <: Statement[_ <: C, _]]
    ...
    override def register(
        p:        SomeProject,
        ps:       PropertyStore,
        analysis: IFDSAnalysis[Fact, C, S]): FPCFAnalysis = {
