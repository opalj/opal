...
sealed abstract class IntTrieSet
     ...
     * @see `+!(Int)` for details!
     */
    final def ++!(that: IntTrieSet): IntTrieSet = that.foldLeft(this)(_ +! _) // We have to expand `this`!
...
final class FilteredIntTrieSet(
        ...
        private val p: Int => Boolean) extends IntTrieSet {

    override def iterator: IntIterator = s.iterator.withFilter(p)

    override def foreach[U](f: Int => U): Unit = s.foreach { i => if (p(i)) f(i) }
    override def map(f: Int => Int): IntTrieSet = s.foldLeft(EmptyIntTrieSet: IntTrieSet) { (c, i) =>
        if (p(i)) c +! f(i) else c
    }
    override def map(map: Array[Int]): IntTrieSet = s.foldLeft(EmptyIntTrieSet: IntTrieSet) { (c, i) =>
        if (p(i)) c +! map(i) else c
    }
    override def flatMap(f:    Int => IntTrieSet): IntTrieSet = s.flatMap(i => if (p(i)) f(i) else EmptyIntTrieSet)
    override def withFilter(p: Int => Boolean): IntTrieSet    = new FilteredIntTrieSet(s, i => p(i) && this.p(i))
    ...
    override private[immutable] def constringe(): IntTrieSet = filtered.constringe()
    override private[immutable] def remove(i:       Int, key:          Int): IntTrieSet = filtered.remove(i, key)
    override private[immutable] def add(i:          Int, level:        Int): IntTrieSet = filtered.add(i, level)
    override private[immutable] def addMutate(i:    Int, level:        Int): IntTrieSet = filtered.addMutate(i, level)
    override private[immutable] def contains(value: Int, key:          Int): Boolean    = filtered.contains(value, key)
    override private[immutable] def subsetOf(other: IntTrieSet, level: Int): Boolean    = filtered.subsetOf(other, level)
}
...
case object EmptyIntTrieSet extends IntTrieSetL {
    ...
    override def headAndTail: IntRefPair[IntTrieSet] = throw new UnsupportedOperationException("headAndTail")
    ...
    override private[immutable] def subsetOf(other: IntTrieSet, level: Int): Boolean                      = true
}
...
final case class IntTrieSet1 private (i: Int) extends IntTrieSetL {
    ...
    override def intersect(other: IntTrieSet): IntTrieSet = if (other.contains(this.i)) this else EmptyIntTrieSet
    ...
    override def equals(other: IntTrieSet): Boolean = (other eq this) || (other match {
    ...
    override private[immutable] def add(i:          Int, level:        Int): IntTrieSet                   = this.+(i)
    override private[immutable] def addMutate(i:    Int, level:        Int /* irrelevant */ ): IntTrieSet = this.+!(i)
    override private[immutable] def subsetOf(other: IntTrieSet, level: Int): Boolean                      = other.contains(i, i >>> level)
}
...
final private[immutable] class IntTrieSet2 private[immutable] (
    ...
    override def filter(p: Int => Boolean): IntTrieSet =
        if (p(i1)) {
            ...
            if (p(i2)) IntTrieSet1(i2)
            else EmptyIntTrieSet
        }
    override def withFilter(p: Int => Boolean): IntTrieSet = new FilteredIntTrieSet(this, p)
    override def map(f: Int => Int): IntTrieSet = {
        val i1    = this.i1
        ...
        val i2    = this.i2
        val newI2 = f(i2)
        if (newI1 == newI2) IntTrieSet1(newI1)
        else if ((newI1 == i1 && newI2 == i2) || (newI1 == i2 && newI2 == i1)) this
        else IntTrieSet.from(newI1, newI2)
    }
    override def map(map: Array[Int]): IntTrieSet = {
    ...
    override def +!(i: Int): IntTrieSet = this + i
    override def intersect(other: IntTrieSet): IntTrieSet = other.size match {
        case 0 => other
    ...
    override private[immutable] def subsetOf(other: IntTrieSet, level: Int): Boolean = other.size match {
        case 0 | 1 => false
        case 2 => other match {
                case that: IntTrieSet2 => that.i1 == this.i1 && that.i2 == this.i2
                case _                 =>
                    // ... this case should never occur...
                    other.contains(i1, i1 >>> level) && other.contains(i2, i2 >>> level)
            }
        case _ => other.contains(i1, i1 >>> level) && other.contains(i2, i2 >>> level)
    }
...
final private[immutable] class IntTrieSet3 private[immutable] (
    ...
    override def headAndTail: IntRefPair[IntTrieSet] = IntRefPair(i3, new IntTrieSet2(i1, i2))
    ...
    override def filter(p: Int => Boolean): IntTrieSet =
        if (p(i1)) {
            if (p(i2)) {
                if (p(i3)) this
                else new IntTrieSet2(i1, i2)
            } else {
                if (p(i3)) new IntTrieSet2(i1, i3)
                else IntTrieSet1(i1)
            }
        } else {
            if (p(i2)) {
                if (p(i3)) new IntTrieSet2(i2, i3)
                else IntTrieSet1(i2)
            } else {
                if (p(i3)) IntTrieSet1(i3)
                else EmptyIntTrieSet
            }
        }
    override def withFilter(p: Int => Boolean): IntTrieSet = new FilteredIntTrieSet(this, p)
    override def map(f: Int => Int): IntTrieSet = {
        ...
        val i3    = this.i3
        val newI3 = f(i3)
        if (newI1 != i1 || newI2 != i2 || newI3 != i3) IntTrieSet(newI1, newI2, newI3) // ensures invariant
        else this
    }
    override def map(map: Array[Int]): IntTrieSet = {
        val newI1 = map(i1)
    ...
    override private[immutable] def subsetOf(other: IntTrieSet, level: Int): Boolean = other.size match {
        case 0 | 1 | 2 => false
        case 3 => other match {
                case that: IntTrieSet3 => that.i1 == this.i1 && that.i2 == this.i2 && that.i3 == this.i3
                case _                 =>
                    // ... this case should never occur...
        ...
        case _ => other.contains(i1, i1 >>> level) &&
            other.contains(i2, i2 >>> level) &&
            other.contains(i3, i3 >>> level)
    }
    ...
    override def equals(other: IntTrieSet): Boolean = (other eq this) || (
        other match {
            case that: IntTrieSet3 => this.i1 == that.i1 && this.i2 == that.i2 && this.i3 == that.i3
            case that              => that.size == 3 && that.contains(i1) && that.contains(i2) && that.contains(i3)
        }
    )
    ...
    override private[immutable] def add(i: Int, level: Int): IntTrieSet =
        if (i == i1 || i == i2 || i == i3) this
        else IntTrieSet.from(i, i1, i2, i3, level)
...
final private[immutable] class IntTrieSetN private[immutable] (
    ...
    override private[immutable] def subsetOf(other: IntTrieSet, level: Int): Boolean = {
        ...
        other match {
            case that: IntTrieSetN => this.right.size <= that.right.size && // check if we have a chance...
                this.left.subsetOf(that.left, level + 1) &&
                this.right.subsetOf(that.right, level + 1)
            case that: IntTrieSetNJustLeft  => this.right.isEmpty && this.left.subsetOf(that.left, level + 1)
            case that: IntTrieSetNJustRight => this.left.isEmpty && this.right.subsetOf(that.right, level + 1)
            case that                       =>
    ...
    override def foldLeft[B](z: B)(f: (B, Int) => B): B = right.foldLeft(left.foldLeft(z)(f))(f)

    override private[immutable] def add(i: Int, level: Int): IntTrieSet =
        if (((i >>> level) & 1) == 0) {
            val left    = this.left
            val newLeft = left.add(i, level + 1)
            if (newLeft eq left) this
            else IntTrieSetN(newLeft, right, size + 1)
        } else {
            val right    = this.right
            val newRight = right.add(i, level + 1)
            if (newRight eq right) this
            else IntTrieSetN(left, newRight, size + 1)
        }
    ...
    override private[immutable] def contains(value: Int, key: Int): Boolean =
        if ((key & 1) == 0) left.contains(value, key >>> 1)
        else right.contains(value, key >>> 1)
    ...
    override private[immutable] def constringe(): IntTrieSet = {
        assert(size <= 2)
        if (left.isEmpty) right.constringe()
        else if (right.isEmpty) left.constringe()
        else new IntTrieSet2(left.head, right.head)
    }
    ...
    private[immutable] def remove(i: Int, key: Int): IntTrieSet =
        if ((key & 1) == 0) {
            ...
            if (newLeft eq left) this
                ...
                (size - 1) match {
                    case 0 => EmptyIntTrieSet
                    case 1 =>
                        if (newLeft.isEmpty) right.constringe()
                        else newLeft.constringe()
                    case newSize => IntTrieSetN(newLeft, right, newSize)
                }
            }
        } else {
            val right    = this.right
            ...
            if (newRight eq right) this
                ...
                val newSize = size - 1
                if (newSize == 0) EmptyIntTrieSet
                else if (newSize == 1) {
                    if (newRight.isEmpty) left.constringe()
                    else newRight.constringe()
                } else {
    ...
    override def iterator: IntIterator = new IntIterator {
        ...
        private[this] def advanceIterator(): Unit = if (!it.hasNext && !isRightIterator) {
    ...
    override def head: Int = {
        ...
        if (leftSize > rightSize) left.head
        else right.head
    }
    ...
    override def filter(p: Int => Boolean): IntTrieSet = {
        ...
        if ((newLeft eq left) && (newRight eq right)) return this;
...
private[immutable] object IntTrieSetN {
    ...
    def apply(
        ...
        size:  Int): IntTrieSet =
        if (right.isEmpty) new IntTrieSetNJustLeft(left)
        else if (left.isEmpty) new IntTrieSetNJustRight(right)
...
final private[immutable] class IntTrieSetNJustRight private[immutable] (
    ...
    override private[immutable] def add(i: Int, level: Int): IntTrieSet =
        if (((i >>> level) & 1) == 0) {
            ...
            val newRight = right.add(i, level + 1)
            if (newRight eq right) this
            else new IntTrieSetNJustRight(newRight)
        }
    ...
    override def filter(p: Int => Boolean): IntTrieSet = {
        ...
        if (newRight eq right) return this;
...
final private[immutable] class IntTrieSetNJustLeft private[immutable] (
    ...
    override private[immutable] def add(i: Int, level: Int): IntTrieSet =
        if (((i >>> level) & 1) == 0) {
            ...
            val newLeft = left.add(i, level + 1)
            if (newLeft eq left) this
            else new IntTrieSetNJustLeft(newLeft)
        } else {
    ...
    private[immutable] def remove(i: Int, key: Int): IntTrieSet =
        if ((key & 1) == 0) {
            ...
            if (newLeft eq left) this
                ...
                newSize match {
                    case 0 => EmptyIntTrieSet
                    case 1 => newLeft.constringe()
                    case _ => new IntTrieSetNJustLeft(newLeft)
                }
            }
        } else {
            this
    ...
    override def filter(p: Int => Boolean): IntTrieSet = {
        ...
        if (newLeft eq left) return this;
...
object IntTrieSet {
    ...
    def apply(data: IterableOnce[Int]): IntTrieSet = data.iterator.foldLeft(empty)(_ + _)

    def apply(i1: Int): IntTrieSet = IntTrieSet1(i1)

    def apply(i1: Int, i2: Int): IntTrieSet =
        if (i1 == i2) IntTrieSet1(i1)
        else {
            from(i1, i2)
        }
    ...
    def apply(i1: Int, i2: Int, i3: Int): IntTrieSet =
        if (i1 == i2) IntTrieSet(i1, i3) // this also handles the case i1 == i3
    ...
    private[immutable] def from(i1: Int, i2: Int, i3: Int, i4: Int, level: Int): IntTrieSet = {
        val root =
            if (((i1 >>> level) & 1) == 0) new IntTrieSetNJustLeft(IntTrieSet1(i1))
            else new IntTrieSetNJustRight(IntTrieSet1(i1))
