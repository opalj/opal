...
private[immutable] final class BitArraySet32(val set: Int) extends BitArraySet { thisSet =>
    ...
    override def -(i: Int): BitArraySet =
        if (i < 32) {
            ...
            if (newSet == set) this
            else if (newSet == 0) BitArraySet0
            else new BitArraySet32(newSet)
        } else {
            this
        }

    override def ++(that: BitArraySet): BitArraySet = that match {
...
private[immutable] final class BitArraySet64(val set: Long) extends BitArraySet { thisSet =>
    ...
    override def -(i: Int): BitArraySet =
        if (i < 64) {
            ...
            if (newSet == set) this
            else if (newSet == 0L) BitArraySet0
            else if ((newSet & BitArraySet.HigherWordMask) == 0) new BitArraySet32(newSet.toInt)
            else new BitArraySet64(newSet)
        } else {
            this
        }
...
private[immutable] final class BitArraySetN(val set: Array[Int]) extends BitArraySet { self =>
    ...
    override def +(i: Int): BitArraySet = {
        ...
        if (bucket >= setLength) {
            ...
            if (oldBucketValue == newBucketValue) this
            else {
    ...
    override def -(i: Int): BitArraySet = {
        ...
        val bucket = i / 32
        if (bucket >= setLength) return this;

        val oldBucketValue = set(bucket)
        val newBucketValue = oldBucketValue & (-1 & ~(1 << (i - 32 * bucket)))
        if (newBucketValue == oldBucketValue) return this;
    ...
    override def contains(i: Int): Boolean = {
        ...
        val bucket = i / 32
        if (bucket >= set.length) return false;
...
object BitArraySet {
    ...
    def apply(i: Int): BitArraySet =
        if (i < 32) new BitArraySet32(1 << i)
        else if (i < 64) new BitArraySet64(1L << i)
        else new BitArraySetN(new Array[Int]((i / 32) + 1)) + i
}
