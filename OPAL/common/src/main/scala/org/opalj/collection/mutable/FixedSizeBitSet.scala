...
final private[mutable] class FixedSizeBitSet64 extends FixedSizeBitSet { thisSet =>
    ...
    override def iterator: IntIterator = new IntIterator {
        ...
        private[this] def advanceIterator(): Unit = do { i += 1 } while (i < 64 && !thisSet.contains(i))
        advanceIterator()
...
final private[mutable] class FixedSizeBitSet128 extends FixedSizeBitSet { thisSet =>
    ...
    override def -=(i: Int): this.type = {
        if (i <= 63) set1 &= (-1L & ~(1L << i))
        else set2 &= (-1L & ~(1L << (i - 64)))
        this
    }
    override def contains(i: Int): Boolean =
        if (i <= 63) (set1 & (1L << i)) != 0L
        else (set2 & (1L << (i - 64))) != 0L

    override def iterator: IntIterator = new IntIterator {
        private[this] var i: Int                  = -1
        private[this] def advanceIterator(): Unit = do { i += 1 } while (i < 128 && !thisSet.contains(i))
        advanceIterator()
...
final private[mutable] class FixedSizeBitSetN private[mutable] (
    ...
    override def iterator: IntIterator = new IntIterator {
        ...
        private[this] var i: Int                  = -1
        private[this] def advanceIterator(): Unit = do { i += 1 } while (i < max && !thisSet.contains(i))
        advanceIterator()
    ...
    override def equals(other: Any): Boolean = other match {
        case that: FixedSizeBitSetN if this.set.length == that.set.length => java.util.Arrays.equals(this.set, that.set)
    ...
    override def hashCode: Int = {
        ...
        while (i < max) {
            ...
            if (elementHash != 0) result = 31 * result + elementHash
            i += 1
        }
        result
    }
