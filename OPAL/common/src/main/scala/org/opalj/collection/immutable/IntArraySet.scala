...
case class IntArraySet1(i: Int) extends IntArraySet {
    override def apply(index: Int): Int = if (index == 0) i else throw new IndexOutOfBoundsException()
    ...
    override def map(f: Int => Int): IntArraySet = {
        val i    = this.i
        val newI = f(i)
        if (newI != i) new IntArraySet1(newI)
        else this
    }
    override def map(map: Array[Int]): IntArraySet = {
        val mappedI = map(i)
        if (mappedI == i) this
        else new IntArraySet1(mappedI)
    }
    ...
    override def +(i: Int): IntArraySet = {
        val thisI = this.i
        if (thisI == i) this
        else if (thisI < i) new IntArraySet2(thisI, i)
        else new IntArraySet2(i, thisI)
    }
...
private[immutable] case class IntArraySet2(i1: Int, i2: Int) extends IntArraySet {
    ...
    override def apply(index: Int): Int = index match {
        case 0 => i1
        case 1 => i2
    ...
    override def map(f: Int => Int): IntArraySet = {
        ...
        val i2    = this.i2
        val newI2 = f(i2)
        if (newI1 != i1 || newI2 != i2) IntArraySet(newI1, newI2) // ensures invariant
        else this
    }
    override def map(map:   Array[Int]): IntArraySet         = IntArraySet2(map(i1), map(i2))
...
private[immutable] case class IntArraySet3(i1: Int, i2: Int, i3: Int) extends IntArraySet {
    ...
    override def withFilter(p: Int => Boolean): IntArraySet =
        if (p(i1)) {
            if (p(i2)) {
                if (p(i3)) this
                else new IntArraySet2(i1, i2)
            } else {
                if (p(i3)) new IntArraySet2(i1, i3)
                else new IntArraySet1(i1)
            }
        } else {
            if (p(i2)) {
                if (p(i3)) new IntArraySet2(i2, i3)
                else new IntArraySet1(i2)
            } else {
                if (p(i3)) new IntArraySet1(i3)
    ...
    override def map(f: Int => Int): IntArraySet = {
        ...
        val i3    = this.i3
        val newI3 = f(i3)
        if (newI1 != i1 || newI2 != i2 || newI3 != i3) IntArraySet(newI1, newI2, newI3) // ensures invariant
        else this
    }
    override def map(map:   Array[Int]): IntArraySet         = IntArraySet3(map(i1), map(i2), map(i3))
    ...
    override def +(i: Int): IntArraySet =
        if (i < i2) {
            if (i < i1) new IntArraySetN(Array[Int](i, i1, i2, i3))
            else if (i == i1) this
            else new IntArraySetN(Array[Int](i1, i, i2, i3))
        } else if (i < i3) {
            if (i == i2) this
            else new IntArraySetN(Array[Int](i1, i2, i, i3))
        } else if (i == i3) this
        else new IntArraySetN(Array[Int](i1, i2, i3, i))
    ...
    override def equals(other: Any): Boolean = other match {
        case that: IntArraySet => that.size == 3 && this.i1 == that(0) && this.i2 == that(1) && this.i3 == that(2)
        case _                 => false
    }
...
case class IntArraySetN private[immutable] (
        private[immutable] val is: Array[Int]) extends IntArraySet {
    ...
    override def flatMap(f: Int => IntArraySet): IntArraySet = foldLeft(EmptyIntArraySet: IntArraySet)(_ ++ f(_))
    ...
    override def contains(value: Int): Boolean = JArrays.binarySearch(is, 0, size, value) >= 0
...
private[immutable] class FilteredIntArraySet(
        p: Int => Boolean,
        origS: IntArraySetN) extends IntArraySet {
...
class IntArraySetBuilder private[immutable] (
        ...
        private[this] var size: Int) extends Builder[Int, IntArraySet] {
    ...
    override def result(): IntArraySet = size match {
        case 0 => EmptyIntArraySet
        case 1 => new IntArraySet1(is(0))
        case 2 => new IntArraySet2(is(0), is(1))
        case 3 => new IntArraySet3(is(0), is(1), is(2))
        case _ =>
            if (size == is.length) new IntArraySetN(is)
            else {
...
object IntArraySet {
    ...
    def apply(i1: Int, i2: Int, i3: Int): IntArraySet = {
        if (i1 == i2) return IntArraySet(i2, i3);
        if (i1 == i3 || i2 == i3) return if (i1 < i2) new IntArraySet2(i1, i2) else new IntArraySet2(i2, i1);
     ...
     */
    def _UNSAFE_fromSorted(data: Array[Int]): IntArraySet = data.length match {
        case 0 => EmptyIntArraySet
    ...
    def _UNSAFE_from(data: Array[Int]): IntArraySet = data.length match {
        case 0 => EmptyIntArraySet
        case 1 => new IntArraySet1(data(0))
