...
object UIDSet0 extends UIDSet[UID] {
    ...
    override def compare(that:          UIDSet[UID]): SetRelation = if (that.isEmpty) EqualSets else /* this is a */ StrictSubset
...
final case class UIDSet1[T <: UID](value: T) extends NonEmptyUIDSet[T] {
    ...
    override def compare(that: UIDSet[T]): SetRelation =
        ...
        } else if (that.contains(this.value)) StrictSubset
        else UncomparableSets

    override def mapUIDSet[B <: UID](f: T => B): UIDSet[B] = UIDSet1(f(value))
}
...
final class UIDSet2[T <: UID](value1: T, value2: T) extends NonEmptyUIDSet[T] {
    ...
    override def drop(n: Int): UIDSet[T] = if (n == 0) this else if (n == 1) new UIDSet1(value2) else empty

    override def incl(e: T): UIDSet[T] = {
        val eId    = e.id
        val value1 = this.value1
        if (eId == value1.id) return this;
        val value2 = this.value2
        if (eId == value2.id) return this;
    ...
    override def excl(e: T): UIDSet[T] = {
        val eId = e.id
        if (value1.id == eId) new UIDSet1(value2)
        else if (value2.id == eId) new UIDSet1(value1)
        else this
    }

    override def hashCode: Int = value1.id ^ value2.id // ordering independent
    override def equals(other: Any): Boolean = other match {
        case that: UIDSet[_] => that.size == 2 && {
                if (that.head.id == value1.id) that.last.id == value2.id
                else that.head.id == value2.id && that.last.id == value1.id
            }
        case _ => false
    }
...
final class UIDSet3[T <: UID](value1: T, value2: T, value3: T) extends NonEmptyUIDSet[T] {
    ...
    override def filter(p: T => Boolean): UIDSet[T] =
        if (p(value1)) {
            if (p(value2)) {
                if (p(value3)) this
                else new UIDSet2[T](value1, value2)
            } else {
                if (p(value3)) new UIDSet2[T](value1, value3)
                else new UIDSet1[T](value1)
            }
        } else {
            if (p(value2)) {
                if (p(value3)) new UIDSet2[T](value2, value3)
                else new UIDSet1[T](value2)
            } else {
                if (p(value3)) new UIDSet1[T](value3)
                else empty
            }
        }
    ...
    override def drop(n: Int): UIDSet[T] = n match {
        case 0 => this
    ...
    override def excl(e: T): UIDSet[T] = {
        val eId = e.id
        if (value1.id == eId) new UIDSet2(value2, value3)
        else if (value2.id == eId) new UIDSet2(value1, value3)
        else if (value3.id == eId) new UIDSet2(value1, value2)
        else this
    }

    override def hashCode: Int = value1.id ^ value2.id ^ value3.id // ordering independent
    override def equals(other: Any): Boolean = other match {
        case that: UIDSet[_] => (that eq this) || {
                that.size == 3 &&
    ...
    override def findById(id: Int): Option[T] =
        if (value1.id == id) Some(value1)
        else if (value2.id == id) Some(value2)
        else if (value3.id == id) Some(value3)
        else None
    ...
    override def isSingletonSet: Boolean = false
    override def containsId(id: Int): Boolean = value1.id == id || value2.id == id || value3.id == id
    override def ++(es: UIDSet[T]): UIDSet[T] = {
        if (es eq this) return this;
...
sealed abstract private[immutable] class UIDSetNodeLike[T <: UID] extends NonEmptyUIDSet[T] {
    ...
    override def tail: UIDSet[T] =
        /*current...*/ size match {
            ...
            case 3 =>
                val left  = this.left
                val right = this.right
                if (left eq null) new UIDSet2(right.head, right.last)
                else if (right eq null) new UIDSet2(left.head, left.last)
                else new UIDSet2(left.head, right.head)
            case _ => dropHead
        }
    ...
    final def excl(e: T): UIDSet[T] = size match {
        case 1 => throw new UnknownError
        case 2 =>
            val value = this.value
            val eId   = e.id
            if (value.id == eId) new UIDSet1(if (left ne null) left.head else right.head)
            else {
                val value1          = value
                val value2Candidate = if (left ne null) left.head else right.head
                if (value2Candidate.id == eId) new UIDSet1(value)
                else new UIDSet2(value1, value2Candidate)
            }
        case 3 =>
            val value = this.value
            val eId   = e.id
            if (value.id == eId) {
                ...
                if (left ne null) {
                    if (right ne null) new UIDSet2(left.head, right.head)
                    else new UIDSet2(left.head, left.last)
                } else {
                ...
                if (value2Candidate.id == eId) new UIDSet2(value, value3Candidate)
                else if (value3Candidate.id == eId) new UIDSet2(value, value2Candidate)
                else this
            }
        case _ =>
            val eId = e.id
    ...
    private def filter0(p: T => Boolean): UIDSetNodeLike[T] = {
        ...
        if (p(value)) {
            if ((newLeft ne left) || (newRight ne right)) {
                ...
                if (newSize == 1) new UIDSetLeaf(value)
                else new UIDSetInnerNode(newSize, value, newLeft, newRight)
            } else {
                this
            }
        } else {
    ...
    override def idIterator: IntIterator = new IntIterator {
    ...
    override def findById(id: Int): Option[T] = {
        ...
        do {
            if (currentNode.value.id == id) return Some(currentNode.value);

            if ((currentShiftedEId & 1) == 1) currentNode = currentNode.right
    ...
    override def ++(es: UIDSet[T]): UIDSet[T] = {
        ...
        es.size match {
            ...
            case 2 => this.incl(es.head).incl(es.last)
            case esSize =>
                if (this.size > esSize) es.foldLeft(this: UIDSet[T])(_ incl _)
                else this.foldLeft(es: UIDSet[T])(_ incl _)
        }
    }

    private def selectHead(
        left:  UIDSetNodeLike[T],
        right: UIDSetNodeLike[T]): UIDSetNodeLike[T] = {
        ...
        if (left ne null) {
            ...
            if (leftSize == 1) {
                if (right eq null) new UIDSetLeaf(leftValue)
                else new UIDSetInnerNode(newSize, leftValue, null, right)
            } else {
            ...
            if (rightSize == 1) {
                if (left eq null) new UIDSetLeaf(rightValue)
                else new UIDSetInnerNode(newSize, rightValue, left, null)
            } else {
    ...
    private def dropHead: UIDSetNodeLike[T] =
        if (left ne null) {
            ...
            if (left.size == 1) {
                if (right eq null) new UIDSetLeaf(leftValue)
                else new UIDSetInnerNode(size - 1, leftValue, null, right)
            } else { // left.size >= 2... but maybe we can pull the right value...
                if ((right ne null) && right.size == 1) new UIDSetInnerNode(size - 1, right.head, left, null)
                else new UIDSetInnerNode(size - 1, leftValue, left.dropHead, right)
            }
        } else if (right ne null) {
            val rightValue = right.head
            if (right.size == 1) {
                if (left eq null) new UIDSetLeaf(rightValue)
                else new UIDSetInnerNode(size - 1, rightValue, left, null)
            } else { // right.size >= 2... but maybe we can pull the left value...
                if ((left ne null) && left.size == 1) new UIDSetInnerNode(size - 1, left.head, null, right)
                else new UIDSetInnerNode(size - 1, rightValue, left, right.dropHead)
            }
        } else {
            null
        }
    ...
    private[immutable] def containsId(id: Int, shiftedId: Int): Boolean = {
        ...
        do {
            if (currentNode.value.id == id) return true;

            if ((currentShiftedId & 1) == 1) currentNode = currentNode.right
            else currentNode = currentNode.left
    ...
    private def add(e: T, eId: Int, shiftedEId: Int, level: Int): UIDSetNodeLike[T] = {
        ...
        if ((shiftedEId & 1) == 1) {
            ...
            if (newRight eq null) newRight = new UIDSetLeaf(e)
            else {
            ...
            if (newLeft eq null) newLeft = new UIDSetLeaf(e)
            else {
                val newShiftedEId = shiftedEId >>> 1
                if ((newRight eq null) &&
                    (valueId >>> level & 1) == 1 &&
...
private[immutable] object UIDSetNode {
    ...
    def apply[T <: UID](
        ...
        left:  UIDSetNodeLike[T],
        right: UIDSetNodeLike[T]): UIDSetNodeLike[T] =
        if (size == 1) new UIDSetLeaf(value)
        else new UIDSetInnerNode(size, value, left, right)

}

final class UIDSetLeaf[T <: UID] private[immutable] (
        val value: T) extends UIDSetNodeLike[T] {
    override def size: Int                = 1
    ...
    private[immutable] def addMutate(e: T, eId: Int, shiftedEId: Int, level: Int): UIDSetNodeLike[T] = {
        if (value.id == eId) return this;

        if ((shiftedEId & 1) == 1) new UIDSetInnerNode(2, value, null, new UIDSetLeaf(e))
        else new UIDSetInnerNode(2, value, new UIDSetLeaf(e), null)
    }
...
final class UIDSetInnerNode[T <: UID] private[immutable] (
        ...
        protected[immutable] var right: UIDSetNodeLike[T]) extends UIDSetNodeLike[T] {

    override def size: Int = theSize

    override def last: T =
        if (right ne null) right.last
        else if (left ne null) left.last
        else value
    ...
    private[immutable] def addMutate(
        ...
        eId:        Int,
        shiftedEId: Int,
        level:      Int): UIDSetNodeLike[T] = {
        val value   = this.value
        val valueId = value.id
        if (eId == valueId) return this;
