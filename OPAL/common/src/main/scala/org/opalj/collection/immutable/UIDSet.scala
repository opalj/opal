...
final case class UIDSet1[T <: UID](value: T) extends NonEmptyUIDSet[T] {
    ...
    override def equals(other: Any): Boolean = {
        other match {
            case that: UIDSet[?] => that.size == 1 && that.head.id == value.id
            case _               => false
        }
    }
...
sealed abstract private[immutable] class UIDSetNodeLike[T <: UID] extends NonEmptyUIDSet[T] {
    ...
    override def iterator: Iterator[T] = new Iterator[T] {
        private val nextNodes = mutable.Stack[UIDSetNodeLike[T]](self)
...
final class UIDSetLeaf[T <: UID] private[immutable] (
    ...
    override def equals(that: Any): Boolean = {
        that match {
            case that: UIDSet[?] => that.size == 1 && that.head.id == this.value.id
            case _               => false
        }
    }
