...
final class DominatorTree private (
        ...
        private[graphs] val idom: Array[Int]) extends AbstractDominatorTree {
...
object DominatorTree {
    ...
    def apply(
        ...
        maxNode:                  Int): DominatorTree = this(
        startNode,
        ...
        if (startNodeHasPredecessors) {
            ...
            create(
                ...
                /* newForeachSuccessorOf */ (n: Int) => {
                    if (n == newStartNode) (f: Int => Unit) => { f(startNode) }
                    else foreachSuccessorOf(n)
                },
                /* newForeachPredecessorOf */ (n: Int) => {
                    if (n == newStartNode) (f: Int => Unit) => {}
                    else if (n == startNode) (f: Int => Unit) => { f(newStartNode) }
                    else foreachPredecessorOf(n)
                },
                newStartNode,
                dominatorTreeFactory
            );
