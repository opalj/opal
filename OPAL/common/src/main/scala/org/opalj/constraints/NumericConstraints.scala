...
object NumericConstraints extends Enumeration(1) {
     ...
     * E.g., `inverse(&gt;) = &lt;`; `x < y === y > x`.
     */
    def inverse(relation: Value): Value = (relation.id: @switch) match {
        case LT => >
        case LE => >=
        case GT => <
        case GE => <=
    ...
    def combine(c1: Value, c2: Value): Value = (c1.id: @switch) match {
        case LT => (c2.id: @switch) match {
                case LT => <
                case LE => <
                case NE => <
     ...
     */
    def join(c1: Value, c2: Value): Option[Value] = (c1.id: @switch) match {
        case LT => (c2.id: @switch) match {
                case LT => Some(<)
                case LE => Some(<=)
                case GT => Some(!=)
