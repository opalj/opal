...
case class NumericRange[@specialized(Int, Long, Float, Double, Boolean) T <: AnyVal](
        lowerBound: T,
        upperBound: T) extends NumericValueConstraint
...
case class NumericSet[@specialized(Int, Long, Float, Double, Boolean) T <: AnyVal](
        values: Set[T]) extends NumericValueConstraint
...
case class ObjectConstraint(
        ...
        fieldConstraints: Map[String, ValueConstraint]) extends ReferenceValueConstraint
...
case class ArrayConstraint(
        ...
        arrayValues: ValueConstraint) extends ReferenceValueConstraint

object ArrayConstraint {
    final val NonNullArrayOfNonNullValues = ArrayConstraint(arrayReference = NonNullValue, arrayValues = NonNullValue)
}
