...
class ConsoleOPALLogger(val ansiColored: Boolean, val minLogLevel: Int) extends OPALLogger {
    ...
    def this(ansiColored: Boolean = true, minLogLevel: Level = Info) = this(ansiColored, minLogLevel.value)

    def log(message: LogMessage)(implicit ctx: LogContext): Unit = {
        val messageLevel = message.level
        if (messageLevel.value < minLogLevel) return;
        ...
        if (theMessage.size > 0 && theMessage.charAt(0) == '\r') stream.print(theMessage)
        else stream.println(theMessage)
    }
