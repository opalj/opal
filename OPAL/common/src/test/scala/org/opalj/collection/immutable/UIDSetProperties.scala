...
object UIDSetProperties extends Properties("UIDSet") {
    ...
    val veryLargeListGen: Gen[(List[UByte], UByte)] = for {
        i <- Gen.choose(30000, 100000)
        s <- Gen.containerOfN[List, Int](i, Arbitrary.arbitrary[Int])
    } yield (s, i)

    val largeSetGen: Gen[(Set[UByte], UByte)] = for {
        i <- Gen.choose(20, 100)
    ...
    property("can handle large sets") = forAll(largeSetGen) { (v) =>
        ...
        val usTransformed: UIDSet[SUID] =
            (us.mapUIDSet { e => SUID(e.id % i) }.filter(_.id < i / 2).+(SUID(i)) + SUID(-i) + SUID(i + 100))

        val s = (Set.empty[SUID] ++ orig.map(SUID(_)))
        val sTransformed: Set[SUID] =
            (s.map { e => SUID(e.id % i) }.filter(_.id < i / 2).+(SUID(i)) + SUID(-i) + SUID(i + 100))
    ...
    property("can handle very large sets") = forAll(veryLargeListGen) { (v) =>
        ...
        val usTransformed = {
            ...
            (us.mapUIDSet { e => SUID(e.id % i) }.filter(_.id < i / 2).+(SUID(i)) + SUID(-i) + SUID(i + 100)) - SUID(
                1002
            )
        }
        ...
        val sTransformed = {
            ...
            (s.map { e => SUID(e.id % i) }.filter(_.id < i / 2).+(SUID(i)) + SUID(-i) + SUID(i + 100)) - SUID(1002)
        }
...
@RunWith(classOf[JUnitRunner])
class UIDSetTest extends AnyFunSpec with Matchers {
        ...
        it("foreach") {
            {
                ...
                info(s"UIDSet foreach sum was $overallSum took ${overallTime.toSeconds}")
            }
