...
object DependencyTypes extends Enumeration(0 /* <= value of first enumeration value*/ ) {
    ...
    def toSet(set: DependencyTypesBitSet): scala.collection.Set[DependencyType] = {
        ...
        while (i <= max) {
            if (((set >> i) & 1) == 1) dependencies += DependencyTypes(i)
            i += 1
        }
        dependencies
    }

    def toUsageDescription(dependencyType: DependencyType): String = dependencyType match {
        ...
        case _ => throw new UnknownError(s"unknown dependency type: $dependencyType")
    }
}
