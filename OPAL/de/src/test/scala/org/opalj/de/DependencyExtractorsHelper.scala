...
object DependencyExtractorsHelper {
    ...
    def sourceElementName(t: Type): String = if (t.isArrayType) t.asArrayType.elementType.toJava else t.toJava
    ...
    def sourceElementName(vMethod: VirtualMethod): String = sourceElementName(vMethod.declaringClassType) +
    ...
    def vseToString(vse: VirtualSourceElement): String = vse match {
    ...
    def extractDependencies(
        ...
        val dependencyExtractor = createDependencyExtractor(
            new DependencyProcessorAdapter() {
                ...
                override def processDependency(
                    source: VirtualSourceElement,
                    target: ArrayType,
                    dType:  DependencyType): Unit = if (target.elementType.isObjectType) {
                    processDependency(
                        source,
                        VirtualClass(target.elementType.asObjectType),
                        dType
                    )
                }
            }
