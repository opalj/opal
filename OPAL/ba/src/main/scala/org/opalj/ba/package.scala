...
package object ba { ba =>
    ...
    implicit def methodAttributeBuilderToSeq(
        b: br.MethodAttributeBuilder): Seq[br.MethodAttributeBuilder] = Seq(b)

    implicit def codeAttributeBuilderToSome[T](
        b: br.CodeAttributeBuilder[T]): Some[br.CodeAttributeBuilder[T]] = Some(b)
    ...
    implicit def attributeToMethodAttributeBuilder(a: br.Attribute): br.MethodAttributeBuilder =
        new br.MethodAttributeBuilder {
            def apply(
                accessFlags: Int,
                name:        String,
                descriptor:  br.MethodDescriptor): Attribute = a
        }
    ...
    implicit def attributeToClassFileAttributeBuilder(
        a: br.Attribute): br.ClassFileAttributeBuilder = new br.ClassFileAttributeBuilder {
        def apply(
            version:        UShortPair,
            accessFlags:    Int,
            thisType:       ObjectType,
            ...
            methods:        ArraySeq[br.MethodTemplate]): Attribute = a
    }
    ...
    def toDA(
        ...
        toDAConfig: ToDAConfig = ToDAConfig.RetainAllAttributes): da.ClassFile = {
    ...
    def toDA(
        ...
        config:          ToDAConfig): da.Field_Info = da.Field_Info(
    ...
    def toDA(
        ...
        constantsBuffer: ConstantsBuffer,
        config:          ToDAConfig): da.Method_Info = {
    ...
    def toDA(
        ...
        constantsBuffer: ConstantsBuffer,
        config:          ToDAConfig): da.Code_attribute = {
        ...
        def writeMethodRef(i: Instruction): MethodInvocationInstruction = {
            ...
            val cpeRef =
                if (isInterface) CPEInterfaceMethodRef(declaringClass, name, descriptor)
                else CPEMethodRef(declaringClass, name, descriptor)
        ...
        code iterate { (pc, i) =>
            ...
            (i.opcode: @switch) match {
                ...
                case INVOKESPECIAL.opcode | INVOKEVIRTUAL.opcode | INVOKESTATIC.opcode => writeMethodRef(i)
                ...
                case NEWARRAY.opcode => instructions.writeByte(i.asInstanceOf[NEWARRAY].atype)
                ...
                case LDC_W.opcode => instructions.writeShort(
                        i match {
                            ...
                            case INCOMPLETE_LDC_W => throw ConstantPoolException("incomplete LDC_W")
                        }
                    )

                case LDC2_W.opcode => i match {
                        case LoadLong(value)   => instructions.writeShort(CPELong(value))
                        case LoadDouble(value) => instructions.writeShort(CPEDouble(value))

                        case LoadDynamic2_W(bootstrapMethod, name, descriptor) => instructions.writeShort(
                                CPEDynamic(bootstrapMethod, name, descriptor, false)
                            )
    ...
    def toDA(
        ...
        constantsBuffer: ConstantsBuffer): da.ExceptionTableEntry = {
        val index =
    ...
    def toDA(
        ...
        constantsBuffer: ConstantsBuffer,
        config:          ToDAConfig): da.ElementValue = {
        import constantsBuffer._
    ...
    def toDA(
        ...
        constantsBuffer: ConstantsBuffer,
        config:          ToDAConfig): da.Annotation = {
    ...
    def toDA(
        localvarTableEntry: br.LocalvarTableEntry): da.LocalvarTableEntry = {
        val br.LocalvarTableEntry(startPC, length, index) = localvarTableEntry
        da.LocalvarTableEntry(startPC, length, index)
    }

    def toDA(
        typeAnnotationTarget: br.TypeAnnotationTarget): da.TypeAnnotationTarget =
    ...
    def toDA(
        typeAnnotationPathElement: br.TypeAnnotationPathElement): da.TypeAnnotationPathElement =
    ...
    def toDA(
        typeAnnotationPath: br.TypeAnnotationPath): da.TypeAnnotationPath = typeAnnotationPath match {
    ...
    def toDA(
        ...
        constantsBuffer: ConstantsBuffer,
        config:          ToDAConfig): da.TypeAnnotation = da.TypeAnnotation(
    ...
    def toDA(
        ...
        constantsBuffer: ConstantsBuffer,
        config:          ToDAConfig): Option[da.Attribute] = {
        import constantsBuffer._
        (attribute.kindId: @switch) match {
            ...
            case br.Deprecated.KindId => Some(da.Deprecated_attribute(CPEUtf8(bi.DeprecatedAttribute.Name)))

            case br.Synthetic.KindId => Some(da.Synthetic_attribute(CPEUtf8(bi.SyntheticAttribute.Name)))
                ...
                Some(
                    da.LineNumberTable_attribute(
                        ...
                        attribute match {
                            ...
                            case br.UnpackedLineNumberTable(lineNumbers) => lineNumbers.map[da.LineNumberTableEntry] {
                                    l => da.LineNumberTableEntry(l.startPC, l.lineNumber)
                                }
            ...
            case br.StackMapTable.KindId =>
                ...
                implicit def toDA(vti: br.VerificationTypeInfo): da.VerificationTypeInfo = (vti.tag: @switch) match {
            ...
            case _ => throw new Error(s"unsupported attribute: ${attribute.getClass.getName}")
        }
    }

    def toDA(constantPool: Array[Constant_Pool_Entry]): Array[da.Constant_Pool_Entry] = constantPool map { cpEntry =>
        if (cpEntry eq null) null
        else {
