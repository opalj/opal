...
class ClassFileBuilderTest extends AnyFlatSpec {
    ...
    val (abstractClass, _) = CLASS[Nothing](
        accessModifiers = ABSTRACT PUBLIC,
        thisType = "org/opalj/bc/AbstractClass"
    ).toDA()

    val (simpleConcreteClass, _) = CLASS[Nothing](
    ...
    val (nestedClassInner, _) = CLASS[Nothing](
    ...
    val (nestedClassOuter, _) = CLASS[Nothing](
        version = UShortPair(0, 55),
        accessModifiers = PUBLIC.FINAL.SUPER.SYNTHETIC,
    ...
    val (recordClass, _) = CLASS[Nothing](
    ...
    val (sealedClassSubclass, _) = CLASS[Nothing](
    ...
    val (sealedClass, _) = CLASS[Nothing](
        version = UShortPair(0, 61),
    ...
    "the generated classes" should "load correctly" in {
        ...
        if (isCurrentJREAtLeastJava11) assert("NestedClassOuter" == loadClass("NestedClassOuter").getSimpleName)
        if (isCurrentJREAtLeastJava16) assert("RecordClass" == loadClass("RecordClass").getSimpleName)
        if (isCurrentJREAtLeastJava17) assert("SealedClass" == loadClass("SealedClass").getSimpleName)
