...
trait Constant_PoolBinding extends Constant_PoolReader with Constant_PoolAbstractions {
    ...
    protected[this] def createDeferredActionsStore(): DeferredActionsStore =
        new mutable.ArrayBuffer[ClassFile => ClassFile] with Constant_Pool_Entry {
            override def Constant_Type_Value: Nothing = throw new UnsupportedOperationException()

            override def asCPNode(implicit cp: Constant_Pool): Nothing = throw new UnsupportedOperationException()

            override def asInstructionParameter(implicit cp: Constant_Pool): Nothing =
                throw new UnsupportedOperationException()

            override def toString(implicit cp: Constant_Pool): Nothing = throw new UnsupportedOperationException(
    ...
    def CONSTANT_Utf8_info(r: Array[Byte], s: String): CONSTANT_Utf8_info = new CONSTANT_Utf8_info(r, s)
    ...
    def CONSTANT_Dynamic_info(
        bootstrap_method_attr_index: Int,
        name_and_type_index:         Constant_Pool_Index): org.opalj.da.ClassFileReader.CONSTANT_Dynamic_info =
