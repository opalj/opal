...
object Disassembler {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val classFileFilter =
            if (className == null) (cf: ClassFile) => true // just take the first one...
            else (cf: ClassFile) => cf.thisType.asJava == className

        val (classFile, source) = ClassFileReader.findClassFile(
            sourceFiles,
            ...
            case Right(altClassNames) =>
                if (altClassNames.isEmpty) {
                    ...
                    val allClassNames: List[(Int, String)] = altClassNames.map { cf =>
        ...
        val targetFile: Option[File] =
            if (openHTMLFile) {
                if (toFile.isEmpty) Some(File.createTempFile(classNameAsFileName, ".html"))
                else {
        ...
        val xHTML =
            if (noHeader) classFile.classFileToXHTML(Some(source)).toString
            else classFile.toXHTML(Some(source), htmlCSS, css, js, !noMethodsFilter).toString
        ...
        targetFile match {
            ...
            case None => Console.out.println(xHTML)
        }
