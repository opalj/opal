...
case class Code_attribute(
        ...
        attributes: Attributes = NoAttributes) extends Attribute {

    override def attribute_length: Int = 2 + 2 +
    ...
    def toXHTML(methodIndex: Int)(implicit cp: Constant_Pool): Node = {
        ...
        val methodBodyHeader = s"Method Body (Size: $codeSize bytes, Max Stack: $max_stack, Max Locals: $max_locals)"
    ...
    def exception_handlersAsXHTML(implicit cp: Constant_Pool): NodeSeq =
        if (exceptionTable.nonEmpty) <details class="exception_table">
