...
case class CONSTANT_Class_info(name_index: Constant_Pool_Index) extends Constant_Pool_Entry {
    ...
    override def asInstructionParameter(implicit cp: Constant_Pool): NodeSeq = asJavaReferenceType(name_index).asSpan("")

    override def toString(implicit cp: Constant_Pool): String = asJavaReferenceType(name_index).asJava
}
