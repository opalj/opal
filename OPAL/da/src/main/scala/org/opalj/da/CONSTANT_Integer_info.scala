...
case class CONSTANT_Integer_info(value: Int) extends Constant_Pool_Entry {
    ...
    override def asInstructionParameter(implicit cp: Constant_Pool): NodeSeq = {
        val repr =
            if (value < 0 || value >= 10) {
                ...
                if (value == Int.MinValue) additionalInfo += " = Int.Min"
                else if (value == Int.MaxValue) additionalInfo += " = Int.Max"
                Seq(
                    Text(value.toString),
    ...
    override def toString(implicit cp: Constant_Pool): String =
        if (value < 0 || value >= 10) {
            ...
            if (value == Int.MinValue) r += " = Int.Min"
            else if (value == Int.MaxValue) r += " = Int.Max"
            r + ")"
        } else {
            value.toString
        }
