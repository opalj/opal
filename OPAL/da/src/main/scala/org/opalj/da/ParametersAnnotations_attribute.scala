...
trait ParametersAnnotations_attribute extends Attribute {
    ...
    override final def attribute_length: Int = parameters_annotations.foldLeft(1 /*num_parameters*/ ) { (c, n) =>
    ...
    def parametersAnnotationstoXHTML(implicit cp: Constant_Pool): Node = {
        val ans = for { // TODO This doesn't make sense: it is no longer possible to distinguish parameters
