...
package object da {
    ...
    def accessFlagsToXHTML(access_flags: Int, context: AccessFlagsContext): (NodeSeq, String) = {
        ...
        val explicitAccessFlags = VisibilityModifier.get(access_flags) match {
        ...
        (
            {
                if (accessFlags.isEmpty) NodeSeq.Empty
                else Seq(<span class="access_flags">{accessFlags}</span>, Text(" "))
            },
            explicitAccessFlags
        )
    }
    ...
    def asJavaReferenceType(cpIndex: Int)(implicit cp: Constant_Pool): FieldTypeInfo = {
        ...
        if (t.charAt(0) == '[') parseFieldType(t)
        else asJavaObjectType(t)
    }

    def asJavaObjectType(
        cpIndex: Constant_Pool_Index
      )(implicit
        cp: Constant_Pool): ObjectTypeInfo = asJavaObjectType(cp(cpIndex).toString(cp))
    ...
    def returnTypeAsJavaType(
        ...
        cp: Constant_Pool): TypeInfo = parseReturnType(cp(type_index).toString)

    def parseReturnType(rt: String): TypeInfo = if (rt.charAt(0) == 'V') VoidTypeInfo else parseFieldType(rt)
    ...
    def parseFieldType(
        ...
        cp: Constant_Pool): FieldTypeInfo = parseFieldType(cp(type_index).toString)

    /**
     * Returns a string representation of the type and the information whether the (element) type
     * is a base type.
     */
    def parseFieldType(descriptor: String): FieldTypeInfo = (descriptor.charAt(0): @scala.annotation.switch) match {
        case 'B' => ByteTypeInfo
        case 'C' => CharTypeInfo
        ...
        case '[' =>
            ...
            parseFieldType(componentType) match {
                ...
                case TypeInfo(elementType, elementTypeIsBaseType) => ArrayTypeInfo(elementType, 1, elementTypeIsBaseType)
            }
        ...
        (td: @switch) match {
            case 'L' =>
                ...
                ( // this is the return tuple
                    ...
                    endIndex + 1)
            case '[' => parseParameterType(md, startIndex + 1) match {
                    ...
                    case (t, index) => (ArrayTypeInfo(t.asJava, 1, t.elementTypeIsBaseType), index)
                }
            case _ => (
