...
case class CONSTANT_MethodHandle_info(
    ...
    def refrenceKindAsNode(implicit cp: Constant_Pool): Node = <span class="method_handle_reference_kind">{
        reference_kind match {
            ...
            case 6 => cp(reference_index) match {
                    case _: CONSTANT_InterfaceMethodref_info => "REF_invokeStatic(interface) invokestatic C.m:(A*)T"
                    case _: CONSTANT_Methodref_info          => "REF_invokeStatic(class) invokestatic C.m:(A*)T"
                }
            case 7 => cp(reference_index) match {
                    case _: CONSTANT_InterfaceMethodref_info => "REF_invokeSpecial(interface) invokeSpecial C.m:(A*)T"
                    case _: CONSTANT_Methodref_info          => "REF_invokeSpecial(class) invokeSpecial C.m:(A*)T"
                }
