...
case class TypeAnnotation(
        ...
        element_value_pairs: ElementValuePairs) extends AbstractAnnotation {

    final def attribute_length: Int = target_type.attribute_length +
        target_path.attribute_length +
        2 +
        element_value_pairs.foldLeft(2 /*num_...*/ )(_ + _.attribute_length)

    def toXHTML(implicit cp: Constant_Pool): Node = <div class="type_annotation">
            {target_type.toXHTML}
            <div>
                {target_path.toXHTML}
                <b>Annotation Type</b>
