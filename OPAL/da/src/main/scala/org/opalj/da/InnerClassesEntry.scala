...
case class InnerClassesEntry(
    ...
    def toXHTML(definingClass: FieldTypeInfo)(implicit cp: Constant_Pool): Node = {
        ...
        val accessFlags      = AccessFlags.toString(inner_class_access_flags, AccessFlagsContexts.INNER_CLASS)

        // IMPROVE Use ObjectTypeInfo
        val definedType = cp(inner_class_info_index).toString
        val outerClassFQN =
            if (outer_class_info_index != 0) cp(outer_class_info_index).toString
            else ""
        ...
        if (definingClassFQN == outerClassFQN && inner_name_index != 0) {
            ...
            val innerName = if (inner_name_index != 0) cp(inner_name_index).toString else ""
            val outerName =
                if (outerClassFQN == "") {
                    if (innerName == "") "..." // <= anonymous inner type
                    else {
                        // named inner type of an anonymous outer type
                        val outerName = definedType.substring(0, definedType.length() - innerName.length() + 1)

                        if (outerName.length < definingClassFQN.length) outerName.substring(0, outerName.length() - 1)
                        else if (outerName.length == definingClassFQN.length) "..."
                        else "..." + outerName.substring(definingClassFQN.length(), outerName.length() - 1)
                    }
                } else outerClassFQN
