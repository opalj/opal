...
case class Field_Info(
        ...
        attributes: Attributes = NoAttributes) extends ClassMember {
     ...
     */
    def fieldType(implicit cp: Constant_Pool): FieldTypeInfo = parseFieldType(cp(descriptor_index).asString)

    def toXHTML(implicit cp: Constant_Pool): Node = {
        val (accessFlags, explicitAccessFlags) = accessFlagsToXHTML(access_flags, FIELD)
        val (constantValue, otherAttributes)   = attributes.partition(a => a.attribute_name == "ConstantValue")
        val fieldName                          = this.fieldName
        val fieldDeclaration = <span class="field_declaration">
                {accessFlags}
