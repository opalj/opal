...
case class ClassFile(
    ...
    implicit private[this] val cp: Constant_Pool = constant_pool
    ...
    def attributeToXHTML(attribute: Attribute): Node = attribute match {
    ...
    def methodsToXHTML: Iterator[Node] = methods.iterator.zipWithIndex.map { mi =>
        val (method, index) = mi; method.toXHTML(index)
    }

    protected def accessFlags: Node = <span class="access_flags">{AccessFlags.classFlagsToJava(access_flags)}</span>

    protected def filter: Node = <details class="filter_settings" open="true">
    ...
    def toXHTML(
        ...
        withMethodsFilter: Boolean = true): Node = <html>
            <head>
        ...
        if (withMethodsFilter) <script>{scala.xml.Unparsed(ClassFile.FilterJS)}</script>
    }
                {
    ...
    private[this] def classFileToXHTML(source: Option[AnyRef], withMethodsFilter: Boolean): Node = {
        ...
        <div class="class_file">
            ...
            <div class="members">
                {
            if (attributes1.nonEmpty) <div class="attributes">
                            <details>
