...
case class Code(instructions: Array[Byte]) {
    ...
    def toXHTML(
        ...
        <table class="method_bytecode">
            ...
            lineNumberTable.map(_ => <th class="line">Line</th>).getOrElse(NodeSeq.Empty)
                ...
                <th>Instruction</th>
                {
            if (exceptionTable.nonEmpty) Seq(
    ...
    private[this] def createTableRowForInstruction(
        ...
        exceptions:      Seq[Node],
        pc:              Int,
        lineNumberTable: Option[Seq[LineNumberTableEntry]]): Node = <tr>
    ...
    private[this] def InstructionsToXHTML(
        ...
        while (in.available > 0) {
            ...
            instructions(pc) = (in.readUnsignedByte: @scala.annotation.switch) match {
                ...
                case 18 =>
                    val constantValue = cp(in.readUnsignedByte()) match {
                ...
                case 19 =>
                    val constantValue = cp(in.readUnsignedShort()) match {
                        case ci: CONSTANT_Class_info => Seq(ci.asInstructionParameter, Text(".class"))
                        case cv                      => Seq(cv.asInstructionParameter)
                    }
                    <span>
    ...
    def ExceptionsToXHTMLTableElements(
        ...
        cp: Constant_Pool): Array[Array[Node]] = {
        ...
        for { (exceptionHandler, index) <- exceptionTable.iterator.zipWithIndex } {
            val exceptionName = (index + 1).toString + ": " + (
                if (exceptionHandler.catch_type != 0) cp(exceptionHandler.catch_type).toString
                else "Any"
            )
            ...
            for {
                ...
                if (exceptionPCLength == 0) exceptionPCStart = i
                exceptionPCLength += 1
            }
            ...
            if (exceptionPCStart == exceptionHandler.handler_pc) classes += " handler_overlap"
            else exceptions(index)(exceptionHandler.handler_pc) = <td class="exception">
                        <div title={
                exceptionName
            } class="exception_handler" alt="the start of the exception handler"></div>
                    </td>

            exceptions(index)(exceptionPCStart) = <td rowspan={exceptionPCLength.toString} class="exception">
