...
class JoinObjectTypesTest
    ...
    def testJoinOfTwoObjectTypes(param1: ObjectType, param2: ObjectType, reflexive: Boolean, expected: UIDSet[ObjectType]) = {
        val result = classhierachy.joinObjectTypes(param1, param2, reflexive)
        if (result != expected)
            fail(s"${param1} join${if (reflexive) "(reflexive)" else ""} ${param2} is ${mkString(result)}; expected ${mkString(expected)}")
    }

    def testJoinOfObjectTypesWithUpperBound(param1: ObjectType, param2: UIDSet[ObjectType], reflexive: Boolean, expected: UIDSet[ObjectType]) = {
        val result = classhierachy.joinObjectTypes(param1, param2, reflexive)
        if (result != expected)
            fail(s"${param1} join${if (reflexive) "(reflexive)" else ""} ${mkString(param2)} is ${mkString(result)}; expected ${mkString(expected)}")
    }
