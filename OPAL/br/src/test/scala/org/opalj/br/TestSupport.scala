...
object TestSupport {
    ...
    def biProjectWithJDK(projectJARName: String, jdkAPIOnly: Boolean = false): Project[URL] = {
        ...
        val jreClassFiles: Seq[(ClassFile, URL)] =
            if (jdkAPIOnly) Java9LibraryFramework.ClassFiles(JRELibraryFolder)
            else readJREClassFiles()
        Project(projectClassFiles, jreClassFiles, libraryClassFilesAreInterfacesOnly = jdkAPIOnly)
    }

    def biProject(projectJARName: String): Project[URL] = Project(locateTestResources(projectJARName, "bi"))

    def brProject(projectJARName: String): Project[URL] = Project(locateTestResources(projectJARName, "br"))
    ...
    def allBIProjects(
        ...
        config:        Config = BaseConfig): Iterator[(String, () => Project[URL])] = {
        ...
        jreReader match {
            case Some(jreReader) =>
                ...
                (allBITestJARs().iterator ++ allBITestProjectFolders().iterator) map { biProject =>
                    ...
                    val readerFactory                 = () => Project(projectClassFiles, jreCFs, jrePublicAPIOnly, Iterable.empty)
                    (biProject.getName, readerFactory)
                }
            case None => (allBITestJARs().iterator ++ allBITestProjectFolders().iterator) map { biProjectJAR =>
                    val readerFactory = () => Project(biProjectJAR, GlobalLogContext, configForProject(biProjectJAR))
    ...
    def allManagedBITestProjects(
        projectReader: ClassFileReader = DefaultJava11Reader,
        jreReader:     ClassFileReader = Java9LibraryFramework): Iterator[(String, () => Project[URL])] = {
        val jreCFs           = jreReader.ClassFiles(RTJar) // we share the loaded JRE!
        val jrePublicAPIOnly = jreReader.loadsInterfacesOnly
    ...
    def foreachBIProject(
        projectReader: ClassFileReader = DefaultJava11Reader,
        jreReader:     Option[ClassFileReader] = Some(Java9LibraryFramework)
      )(f: (String, Project[URL]) => Unit): Unit = {
