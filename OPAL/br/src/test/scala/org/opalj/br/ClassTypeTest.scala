...
class ClassTypeTest extends AnyFunSuite {
    ...
    test("primitiveTypeWrapperMatcher method") {
        ...
        val matcher = ClassType.primitiveTypeWrapperMatcher[Int, (Int, Int)](
        ...
        assert(matcher(ClassType.String, ClassType.String.id) == ((-1, ClassType.String.id)))
    ...
    test("pattern matching on ClassTypes") {
        ...
        ot1 match {
            ...
            case _            => fail(s"pattern match on ClassType ($ot1) failed")
        }
    }
}
