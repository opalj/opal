...
class ClassFileFactoryTest extends AnyFunSpec with Matchers {
    ...
    val lambdasProject = {
        ...
        val config =
            baseConfig.withValue(rewritingConfigKey, ConfigValueFactory.fromAnyRef(java.lang.Boolean.FALSE)).withValue(
                logRewritingsConfigKey,
                ConfigValueFactory.fromAnyRef(java.lang.Boolean.TRUE))
    ...
    private def getMethods(
        ...
        repository: ClassFileRepository): Iterable[(ObjectType, Method)] = repository.classFile(classType) match {
    ...
    private def collectTheConstructor(classFile: ClassFile): Method = collectTheMethodOf(classFile)(_.isConstructor)

    private def collectTheFactoryMethod(classFile: ClassFile): Method = collectTheMethodOf(classFile) { m =>
        m.isStatic && m.isPublic && (m.name == "$newInstance" || m.name == "$createInstance")
    }

    private def collectTheForwardingMethod(classFile: ClassFile): Method = collectTheMethodOf(classFile) { m =>
    ...
    describe("ClassFileFactory") {
        ...
        describe("should be able to proxify methods") {
            ...
            it("and computes correct maxLocals/maxStack values") {
                testMethods(methods, testProject) { (classFile, calleeTypeAndMethod) =>
                    val method = collectTheForwardingMethod(classFile)
                    val body   = method.body.get
                    val operandsSize: Int = (if (calleeTypeAndMethod._2.isStatic) 0 else 1 /* for `this`*/ ) +
            ...
            it("and handles multiple parameters of the same type correctly") {
                val methodWithFiveDoubleParameters = testProject.classFile(StaticMethods).get.findMethod(
            ...
            it("and returns correctly") {
                ...
                for {
                    ...
                    var currentPC         = instructions(indexOfInvocation).indexOfNextInstruction(indexOfInvocation, false)
        ...
        describe("should be able to create correct proxies for method reference invokedynamics") {
            ...
            describe("references to constructors") {
                it("should be correctly identified") {
                    ...
                    val indy = body.instructionIterator.collectFirst({
        ...
        describe("should create a bridge method to the forwarding method if so desired") {
            ...
            it("the bridge method should stack & cast parameters and call the forwarding method") {
                ...
                val methodDescriptor = MethodDescriptor(ArraySeq(ObjectType.String, DoubleType), IntegerType)
                val proxy = ClassFileFactory.Proxy(
                    lambdas.thisType,
                    lambdas.isInterfaceDeclaration,
        ...
        describe("should create correct instructions for stacking parameters for the forwarding call") {
            ...
            it("should forward all parameters for identical, non-empty descriptors") {
                ...
                d = MethodDescriptor(
                    ArraySeq(DoubleType, ObjectType.String, ByteType, LongType, DoubleType, FloatType),
                    VoidType
                )
                ClassFileFactory.parameterForwardingInstructions(
                    d,
        ...
        describe("should create correct return/conversion instructions") {
            ...
            it("should convert Object to any reference type by casting") {
                Seq(ObjectType.String, ArrayType(LongType), ObjectType.Integer).foreach(t =>
                    ClassFileFactory.returnAndConvertInstructions(t, ObjectType.Object) should be(
        ...
        describe("should handle serializable lambdas correctly") {
            ...
            it("should create a SerializedLambda object inside writeReplace") {
                ...
                wr.body.get.instructions should be(Array(
                    ...
                    null,
                    DUP,
                    LoadClass(ObjectType("SerializableProxy")),
                    null,
                    LDC(ConstantString(ObjectType("java/util/Function").fqn)),
                    null,
                    ...
                    null,
                    ALOAD_0,
                    GETFIELD(ObjectType("SerializableProxy"), "staticParameter0", ObjectType.Float),
                    null,
                    null,
                    AASTORE,
                    DUP,
                    ...
                    null,
                    ALOAD_0,
                    GETFIELD(ObjectType("SerializableProxy"), "staticParameter1", ObjectType.String),
                    null,
                    null,
                    AASTORE,
                    INVOKESPECIAL(
    ...
    private def requireIntLoad(remainingInstructions: Array[Instruction]): Int = require(
        Set(
            ILOAD_0.opcode,
            ILOAD_1.opcode,
            ILOAD_2.opcode,
    ...
    private def requireLongLoad(remainingInstructions: Array[Instruction]): Int = require(
        Set(
            LLOAD_0.opcode,
            LLOAD_1.opcode,
            LLOAD_2.opcode,
    ...
    private def requireFloatLoad(remainingInstructions: Array[Instruction]): Int = require(
        Set(
            FLOAD_0.opcode,
            FLOAD_1.opcode,
            FLOAD_2.opcode,
    ...
    private def requireDoubleLoad(remainingInstructions: Array[Instruction]): Int = require(
        Set(
            DLOAD_0.opcode,
            DLOAD_1.opcode,
            DLOAD_2.opcode,
    ...
    private def testMethods(
      ...
      )(test: (ClassFile, (ObjectType, Method)) => Unit): Unit = for ((calleeType, calleeMethod) <- methods) {
