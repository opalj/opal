...
object ValueInformation {
     ...
     */
    def forProperValue(t: Type)(implicit classHierarchy: ClassHierarchy): ValueInformation = (t.id: @switch) match {
...
trait IsIllegalValue extends ValueInformation {
    ...
    override final def isIllegalValue: Boolean = true

    override final def isVoid: Boolean = false

    override final def isPrimitiveValue: Boolean = throw new IllegalStateException("illegal value")

    override final def isReferenceValue: Boolean = throw new IllegalStateException("illegal value")

    override final def isArrayValue: Answer = No

    override final def computationalType: ComputationalType = throw new IllegalStateException("illegal value")

    override final def hasCategory2ComputationalType: Boolean = false

    override final def verificationTypeInfo: VerificationTypeInfo = TopVariableInfo

    override final def toCanonicalForm: ValueInformation = IsIllegalValue
...
sealed trait KnownValue extends ValueInformation {
    ...
    override final def isIllegalValue: Boolean = false
...
object TheVoidValue extends KnownValue {
    ...
    override def verificationTypeInfo: VerificationTypeInfo = throw new IllegalStateException("void")
...
trait IsReturnAddressValue extends KnownTypedValue {
    ...
    override final def isPrimitiveValue: Boolean = false

    override final def isReferenceValue: Boolean = false

    override final def isArrayValue: Answer = No

    override final def computationalType: ComputationalType = ComputationalTypeReturnAddress
...
sealed trait IsPrimitiveValue[T <: BaseType]
    ...
    override final def computationalType: ComputationalType = primitiveType.computationalType

}

object IsPrimitiveValue {

    def unapply[T <: BaseType](underlying: IsPrimitiveValue[T]): Some[T] = Some(underlying.primitiveType)
...
trait IsReferenceValue extends KnownTypedValue {
    ...
    override final def isReferenceValue: Boolean          = true
    override final def asReferenceValue: IsReferenceValue = this
...
trait IsBaseReferenceValue extends IsReferenceValue {
    override final def baseValues: Iterable[this.type] = Nil
    override final def allValues: Iterable[this.type]  = List(this)
...
trait IsNullValue extends IsBaseReferenceValue {
    ...
    override final def isValueASubtypeOf(
        ...
        classHierarchy: ClassHierarchy): Answer = throw new IllegalStateException("null value")
...
trait IsMObjectValue extends IsBaseReferenceValue {
    ...
    override final def verificationTypeInfo: VerificationTypeInfo = ObjectVariableInfo(leastUpperType.get)
...
case class AProperMObjectValue(
    ...
    override def toString: String = "ProperMObjectValue(" +
        ...
        classHierarchy.isASubtypeOf(subtype, supertype) match {
            ...
            case _
                if isPrecise &&
                    ...
                    classHierarchy.isSubtypeOf(supertype, subtype) => No
            case _ => Unknown
        }
    }

    // Non-final to enable subclasses to identify cases of uninitializedThis/Variable cases.
    override def verificationTypeInfo: VerificationTypeInfo = ObjectVariableInfo(theUpperTypeBound)

    // Non-final to enable subclasses to provide more detailed information.
    override def toCanonicalForm: IsBaseReferenceValue = ASObjectValue(isNull, isPrecise, theUpperTypeBound)
}
...
case class ASObjectValue(
        isNull:                 Answer,
        override val isPrecise: Boolean,
        theUpperTypeBound: ObjectType) extends IsSObjectValue {
...
trait IsSArrayValue extends IsSReferenceValue[ArrayType] {
    ...
    override final def isArrayValue: Answer = isNull.negate // isNull is either "No" or "Unknown"

    override def isValueASubtypeOf(
        supertype: ReferenceType
      )(implicit
        classHierarchy: ClassHierarchy): Answer = classHierarchy.isASubtypeOf(theUpperTypeBound, supertype) match {
        case Yes => Yes
        case No
            if isPrecise ||
    ...
    // Non-final to enable subclasses to identify cases of uninitializedThis/Variable cases.
    override final def verificationTypeInfo: VerificationTypeInfo = ObjectVariableInfo(theUpperTypeBound)
}

case class ASArrayValue(
        override val isNull:    Answer,
        override val isPrecise: Boolean,
        theUpperTypeBound: ArrayType) extends IsSArrayValue {
    override def toCanonicalForm: IsSArrayValue = this
    override def toString: String = "SArrayValue(" +
...
case class ASArrayWithLengthValue(
    ...
    override def toCanonicalForm: IsSArrayValue = this
    override def toString: String = s"SArrayValue(" +
        s"type=${theUpperTypeBound.toJava}," +
...
trait IsStringValue
    ...
    override final def verificationTypeInfo: VerificationTypeInfo = ObjectVariableInfo(ObjectType.String)

    override final def isValueASubtypeOf(
        supertype: ReferenceType
      )(implicit
        classHierarchy: ClassHierarchy): Answer = supertype.id match {
        case ObjectType.ObjectId
            | ObjectType.SerializableId
            | ObjectType.CloneableId
            | ObjectType.ComparableId
            | ObjectType.StringId => Yes
        case _ => No
    }
...
trait IsClassValue
    ...
    override final def verificationTypeInfo: VerificationTypeInfo = ObjectVariableInfo(ObjectType.Class)
...
trait IsMultipleReferenceValue extends IsReferenceValue {
    ...
    override def isValueASubtypeOf(
        ...
        if (isASubtypeOf eq Yes) return Yes;
        if ((isASubtypeOf eq No) && isPrecise) return No;
        ...
        values foreach { value =>                 /* the first value is already removed */
            if (answer eq Unknown) return answer; // isSubtype
