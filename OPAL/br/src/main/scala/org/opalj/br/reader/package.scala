...
package object reader {
    ...
    def read(
        ...
        : (Iterable[(ClassFile, URL)], List[Throwable]) = readClassFiles(args.view.map(new File(_)), classFilesReader)
    ...
    def readJREClassFiles(
        ...
        logContext: LogContext = GlobalLogContext): Seq[(ClassFile, URL)] = {
    ...
    def readRTJarClassFiles(
        ...
        reader:     ClassFileBinding = new Java11FrameworkWithCaching(cache),
        logContext: LogContext = GlobalLogContext): Iterable[(ClassFile, URL)] = {
