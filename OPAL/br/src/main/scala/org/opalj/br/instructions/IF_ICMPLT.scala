...
case class IF_ICMPLT(branchoffset: Int) extends IFICMPInstruction[IF_ICMPLT] with IF_ICMPLTLike {
    ...
    def negate(newBranchoffset: Int = branchoffset): IF_ICMPGE = IF_ICMPGE(newBranchoffset)
...
case class LabeledIF_ICMPLT(
    ...
    override def negate(newJumpTargetLabel: InstructionLabel): LabeledIF_ICMPGE = LabeledIF_ICMPGE(newJumpTargetLabel)
}
