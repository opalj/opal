...
case class TABLESWITCH(
        ...
        low:           Int,
        high:          Int,
        jumpOffsets: ArraySeq[Int]) extends CompoundConditionalBranchInstruction with TABLESWITCHLike {
    ...
    def toLabeledInstruction(currentPC: PC): LabeledInstruction = LabeledTABLESWITCH(
    ...
    def caseValueOfJumpOffset(jumpOffset: Int): (List[Int], Boolean) = {
        ...
        while (i >= 0) {
            if (jumpOffsets(i) == jumpOffset) caseValues = high - i :: caseValues
            i -= 1
        }
    ...
    override def toString: String = s"TABLESWITCH($low -> $high; " +
        (low to high).zip(jumpOffsets).map(e => s"${e._1}⤼${e._2}").mkString(",") +
        ";default⤼" + defaultOffset +
        ")"

    override def toString(pc: PC): String = s"TABLESWITCH($low -> $high; " +
...
object TABLESWITCH extends InstructionMetaInformation {
    ...
    def apply(
        ...
        low:                 Int,
        high:                Int,
        branchTargets:       ArraySeq[InstructionLabel]): LabeledTABLESWITCH = {
        require(
...
case class LabeledTABLESWITCH(
        ...
        low:                 Int,
        high:                Int,
        jumpTargets: ArraySeq[InstructionLabel]) extends LabeledInstruction with TABLESWITCHLike {

    @throws[BranchoffsetOutOfBoundsException]("if the branchoffset is invalid")
    override def resolveJumpTargets(currentPC: PC, pcs: Map[InstructionLabel, PC]): TABLESWITCH = TABLESWITCH(
    ...
    override def branchTargets: Iterator[InstructionLabel] = jumpTargets.iterator ++ Iterator(defaultBranchTarget)

    def caseValueOfJumpTarget(jumpTarget: InstructionLabel): (List[Int], Boolean) = {
        var caseValues = List.empty[Int]
        var i          = jumpTargets.length - 1
        while (i >= 0) {
            if (jumpTargets(i) == jumpTarget) caseValues ::= high - i
            i -= 1
        }
    ...
    override def toString(pc: Int): String = (low to high).zip(jumpTargets).map { keyOffset =>
