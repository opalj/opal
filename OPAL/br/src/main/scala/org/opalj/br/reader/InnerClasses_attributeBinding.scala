...
trait InnerClasses_attributeBinding
    ...
    def InnerClasses_attribute(
        ...
        inner_classes:        InnerClasses): InnerClasses_attribute = new InnerClasses_attribute(inner_classes)
    ...
    def InnerClassesEntry(
        ...
        inner_class_access_flags: Int): InnerClassesEntry = new InnerClassesEntry(
        cp(inner_class_info_index).asObjectType(cp),
        if (outer_class_info_index == 0) None
        else Some(cp(outer_class_info_index).asObjectType(cp)),
        if (inner_name_index == 0) None
        else Some(cp(inner_name_index).asString),
