...
object VirtualSourceElement {
    ...
    def asVirtualSourceElements(
        ...
        classFiles foreach { classFile =>
            ...
            if (includeMethods) classFile.methods.foreach(sourceElements += _.asVirtualMethod(classType))
            if (includeFields) classFile.fields.foreach(sourceElements += _.asVirtualField(classType))
        }
        sourceElements
    }
        ...
        that match {
            ...
            case that: VirtualField =>
                if (this.declaringClassType eq that.declaringClassType) {
                    ...
                    if (this.declaringClassType.id < that.declaringClassType.id) -1
                    else 1
                }
            case _ /*VirtualMethod*/ => -1
        }

    override def hashCode: Int = (((declaringClassType.id * 41) + name.hashCode()) * 41) + fieldType.id

    override def equals(other: Any): Boolean = other match {
        case that: VirtualField => (this.declaringClassType eq that.declaringClassType) &&
...
sealed class VirtualMethod(
    ...
    override def getLineNumber(project: ClassFileRepository): Option[Int] = {
        if (declaringClassType.isArrayType) return None;
    ...
    override def hashCode: Int = (((declaringClassType.id * 41) + name.hashCode()) * 41) + descriptor.hashCode()

    override def equals(other: Any): Boolean = other match {
        case that: VirtualMethod => (this.declaringClassType eq that.declaringClassType) &&
            this.descriptor == that.descriptor &&
            this.name == that.name
        case _ => false
    }

    override def toString: String = s"VirtualMethod($declaringClassType,$name,$descriptor)"
}
...
object VirtualMethod {
    ...
    def apply(
        ...
        name:               String,
        descriptor:         MethodDescriptor): VirtualMethod = new VirtualMethod(declaringClassType, name, descriptor)

    def unapply(virtualMethod: VirtualMethod): Option[(ReferenceType, String, MethodDescriptor)] = Some((
...
final case class VirtualForwardingMethod(
        ...
        target: Method) extends VirtualMethod(declaringClassType, name, descriptor) {
    ...
    override def equals(other: Any): Boolean = other match {
        case that: VirtualForwardingMethod => (this.target eq that.target) &&
            super.equals(other)
        case _ => false
    }
