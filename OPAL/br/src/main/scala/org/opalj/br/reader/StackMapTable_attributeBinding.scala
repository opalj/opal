...
trait StackMapTable_attributeBinding
    ...
    implicit override val stackMapFrameType: ClassTag[StackMapFrame] = ClassTag(classOf[br.StackMapFrame])
    ...
    def StackMapTable_attribute(
        ...
        stack_map_frames:     StackMapFrames): StackMapTable_attribute = StackMapTable(stack_map_frames)
    ...
    def SameLocals1StackItemFrame(
        ...
        br.SameLocals1StackItemFrame(frame_type, verification_type_info_stack)
    ...
    def ChopFrame(
        frame_type:   Int,
        offset_delta: Int): ChopFrame = br.ChopFrame(frame_type, offset_delta)

    def SameFrameExtended(
        frame_type:   Int,
        offset_delta: Int): StackMapFrame = br.SameFrameExtended(offset_delta)

    def AppendFrame(
        frame_type:                    Int,
        offset_delta:                  Int,
        verification_type_info_locals: VerificationTypeInfoLocals): StackMapFrame =
        br.AppendFrame(frame_type, offset_delta, verification_type_info_locals)
    ...
    def UninitializedThisVariableInfo(): br.UninitializedThisVariableInfo.type = br.UninitializedThisVariableInfo

    def UninitializedVariableInfo(offset: Int) = new UninitializedVariableInfo(offset)

    def ObjectVariableInfo(
        cp:         Constant_Pool,
        type_index: Constant_Pool_Index): ObjectVariableInfo = new ObjectVariableInfo(cp(type_index).asReferenceType(cp))
