...
class BytecodeInstructionsCache {
    ...
    def MethodName(name: String): String = methodNames.getOrElseUpdate(name, name.intern)

    private[this] val fieldNames = TrieMap.empty[String, String]
    def FieldName(name: String): String = fieldNames.getOrElseUpdate(name, name.intern)
    ...
    def NEW(objectType: ObjectType): NEW = newInstructions.getOrElseUpdate(objectType, new NEW(objectType))
