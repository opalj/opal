...
sealed trait MethodFieldAccessInformation[S <: MethodFieldAccessInformation[S]] extends OrderedProperty
    ...
    def indirectAccessReceiver(
        accessContext: Context,
        pc:            PC,
        field:         DeclaredField): AccessReceiver = _indirectAccessedReceiversByField(accessContext.id)(pc)(field.id)

    def indirectAccessParameter(
        accessContext: Context,
        pc:            PC,
        field:         DeclaredField): AccessParameter = _indirectAccessedParametersByField(accessContext.id)(pc)(field.id)
    ...
    protected def integrateAccessedFieldsForContext(
        ...
        contextId:   Int,
        updateValue: IntMap[IntList]): IntMap[IntMap[IntList]] = baseMap.updateWith(
        contextId,
        updateValue,
        (o, n) =>
            o.unionWith(
    ...
    protected def integrateAccessInformationForContext[AIT](
        ...
        failure:     () => Throwable): IntMap[IntMap[IntMap[AIT]]] = baseMap.updateWith(
        contextId,
        updateValue,
        (o, n) =>
            o.unionWith(
...
case class MethodFieldReadAccessInformation(
    ...
    def updateWithFieldAccesses(
        ...
        indirectAccessReceivers: AccessReceivers): MethodFieldReadAccessInformation = {
...
case class MethodFieldWriteAccessInformation(
    ...
    def updateWithFieldAccesses(
        ...
        indirectAccessParameters: IntMap[IntMap[AccessParameter]]): MethodFieldWriteAccessInformation = {
        val cId = accessContext.id
