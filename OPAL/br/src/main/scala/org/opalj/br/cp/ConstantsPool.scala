...
class ConstantsPool(
        ...
        val bootstrapMethods: IndexedSeq[BootstrapMethod]) extends ConstantsPoolLike {
    ...
    override def CPEDouble(value: Double): Int = constantPool(CONSTANT_Double_info(ConstantDouble(value)))

    override def CPELong(value: Long): Int = constantPool(CONSTANT_Long_info(ConstantLong(value)))
    ...
    override def CPEMethodRef(
        ...
        name:       String,
        descriptor: MethodDescriptor): Int = CPEMethodRef(objectType, name, descriptor.toJVMDescriptor)
    ...
    override def CPEInterfaceMethodRef(
        objectType: ReferenceType,
        name:       String,
        descriptor: MethodDescriptor): Int = CPEInterfaceMethodRef(objectType, name, descriptor.toJVMDescriptor)
    ...
    override def CPEInvokeDynamic(
        bootstrapMethod: BootstrapMethod,
        name:            String,
        descriptor:      MethodDescriptor): Int = CPEInvokeDynamic(bootstrapMethod, name, descriptor.toJVMDescriptor)
    ...
    def CPEDynamic(
        ...
        name:               String,
        descriptor:         String,
        requiresUByteIndex: Boolean): Int = {
        val indexOfBootstrapMethod = bootstrapMethods.indexOf(bootstrapMethod)
        if (indexOfBootstrapMethod == -1) {
            throw new ConstantPoolException(s"the bootstrap method $bootstrapMethod is unknown")
        }
