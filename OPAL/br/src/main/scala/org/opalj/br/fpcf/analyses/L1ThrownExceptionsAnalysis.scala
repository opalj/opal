...
class L1ThrownExceptionsAnalysis private[analyses] (
    ...
    def determineThrownExceptions(m: Method): ProperPropertyComputationResult = {
        if (m.isNative) return Result(m, MethodIsNative);
        if (m.isAbstract) return Result(m, MethodIsAbstract);
        val body = m.body
        if (body.isEmpty) return Result(m, MethodBodyIsNotAvailable);
        ...
        def c(eps: SomeEPS): ProperPropertyComputationResult = {
            ...
            eps.ub match {
                ...
                case MethodIsAbstract |
                    ...
                    AnalysisLimitation |
                    UnresolvedInvokeDynamicInstruction => return Result(m, MethodCalledThrowsUnknownExceptions);

                case te: ThrownExceptions => exceptions = exceptions ++ te.types.concreteTypes
...
abstract class ThrownExceptionsAnalysisScheduler extends FPCFAnalysisScheduler {
    ...
    final override def uses: Set[PropertyBounds] = Set(PropertyBounds.lub(ThrownExceptionsByOverridingMethods))
