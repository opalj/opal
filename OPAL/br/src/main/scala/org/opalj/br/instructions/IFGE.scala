...
case class IFGE(branchoffset: Int) extends IF0Instruction[IFGE] with IFGELike {
    ...
    def negate(newBranchoffset: Int = branchoffset): IFLT = IFLT(newBranchoffset)

    def toLabeledInstruction(currentPC: PC): LabeledInstruction = LabeledIFGE(InstructionLabel(currentPC + branchoffset))
}
...
case class LabeledIFGE(
    ...
    override def negate(newJumpTargetLabel: InstructionLabel): LabeledIFLT = LabeledIFLT(newJumpTargetLabel)
}
