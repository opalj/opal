...
final class ExitNode(final val normalReturn: Boolean) extends CFGNode {
    ...
    override def toHRR: Option[String] = Some(if (normalReturn) "Normal Return" else "Abnormal Return")

    override def visualProperties: Map[String, String] =
        if (normalReturn) Map("labelloc" -> "l", "fillcolor" -> "green", "style" -> "filled")
        else Map("labelloc"              -> "l", "fillcolor" -> "red", "style"   -> "filled", "shape" -> "octagon")
