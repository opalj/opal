...
trait FPCFAnalysisScheduler extends ComputationSpecification[FPCFAnalysis] {
    ...
    override final def init(ps: PropertyStore): InitializationData = init(ps.context(classOf[SomeProject]), ps)

    override final def uses(ps: PropertyStore): Set[PropertyBounds] = uses ++ uses(ps.context(classOf[SomeProject]), ps)

    override final def beforeSchedule(ps: PropertyStore): Unit = beforeSchedule(ps.context(classOf[SomeProject]), ps)
