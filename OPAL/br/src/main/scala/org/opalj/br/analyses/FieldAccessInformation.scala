...
case class FieldAccessInformation(project: SomeProject) {
    ...
    private[this] def getFieldAccessInformation[S <: fieldaccess.FieldAccessInformation[S]](
        field: DeclaredField,
        key:   PropertyKey[fieldaccess.FieldAccessInformation[S]]): fieldaccess.FieldAccessInformation[S] =
        propertyStore(field, key) match {
            ...
            case r => throw new IllegalStateException(s"Unexpected property found: $r")
        }
     ...
     */
    def statistics(implicit declaredFields: DeclaredFields): Map[String, Int] = Map(
