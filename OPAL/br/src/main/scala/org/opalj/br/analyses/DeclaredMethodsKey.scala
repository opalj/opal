...
object DeclaredMethodsKey extends ProjectInformationKey[DeclaredMethods, Nothing] {
    ...
    override def compute(p: SomeProject): DeclaredMethods = {
        ...
        def insertDeclaredMethod(
            ...
            computeDeclaredMethod: Int => DeclaredMethod): Unit = {
            var computedDM: DeclaredMethod = null
            val oldDm = dms.computeIfAbsent(context,
                                            _ => {
            ...
            assert(
                (computedDM ne null) || {
                    ...
                    oldDm match {
                        case dm: DefinedMethod => computedDM.hasSingleDefinedMethod &&
                            (dm.definedMethod eq computedDM.definedMethod)
                        case mdm: MultipleDefinedMethods => mdm.hasMultipleDefinedMethods &&
    ...
    sealed private[analyses] class MethodContext(
        ...
        override def equals(other: Any): Boolean = other match {
            case that: MethodContext => methodName == that.methodName && descriptor == that.descriptor
            case _                   => false
        }
    ...
    private[analyses] object MethodContext {
         ...
         */
        def apply(project: SomeProject, objectType: ObjectType, method: Method): MethodContext = MethodContext(
            project,
            objectType,
        ...
        def apply(
            ...
            isPackagePrivate: Boolean): MethodContext =
            if (isPackagePrivate) new PackagePrivateMethodContext(declaringPackage, methodName, descriptor)
            ...
            else new MethodContext(methodName, descriptor)
    }
    ...
    private[this] class PackagePrivateMethodContext(
            ...
            methodName:      String,
            descriptor: MethodDescriptor) extends MethodContext(methodName, descriptor) {

        override def equals(other: Any): Boolean = other match {
            case that: MethodContextQuery => that.equals(this)
            case that: PackagePrivateMethodContext => packageName == that.packageName &&
    ...
    private[analyses] class MethodContextQuery(
        ...
        override def equals(other: Any): Boolean = other match {
            ...
            case that: ShadowsPackagePrivateMethodContext => methodName == that.methodName &&
                descriptor == that.descriptor &&
                !isPackagePrivateMethod
            case that: MethodContext => methodName == that.methodName && descriptor == that.descriptor
            case _                   => false
