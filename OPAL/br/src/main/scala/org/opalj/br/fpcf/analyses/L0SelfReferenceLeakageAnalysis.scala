...
class L0SelfReferenceLeakageAnalysis(
        ...
        val debug: Boolean) extends FPCFAnalysis {
    ...
    private[this] def determineSelfReferenceLeakageContinuation(
        classFile: ClassFile): ProperPropertyComputationResult = {
        ...
        def potentiallyLeaksSelfReference(method: Method): Boolean = {
            ...
            if (returnType.isObjectType && thisIsSubtypeOf(returnType.asObjectType)) return true;
            ...
            while (pc < max) {
                ...
                instruction.opcode match {
                    ...
                    case PUTSTATIC.opcode | PUTFIELD.opcode =>
                        ...
                        if (fieldType.isObjectType && thisIsSubtypeOf(fieldType.asObjectType)) return true;
        ...
        val doesLeakSelfReference = classFile.methods exists { m =>
            if (m.isNative || (
    ...
    def determineSelfReferenceLeakage(classFile: ClassFile): PropertyComputationResult = {
        ...
        val superTypes: Seq[ObjectType] =
            if (superClassType == ObjectType.Object) interfaceTypes
            else interfaceTypes :+ superClassType
