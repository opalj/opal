...
trait CodeAttributeBinding
    ...
    implicit override val exceptionTableEntryType: ClassTag[ExceptionTableEntry] = ClassTag(classOf[br.ExceptionHandler])
    ...
    def ExceptionTableEntry(
        ...
        end_pc:           Int,
        handler_pc:       Int,
        catch_type_index: Constant_Pool_Index): ExceptionTableEntry = new ExceptionTableEntry(
        start_pc,
        end_pc,
        handler_pc,
        if (catch_type_index == 0) None
        else Some(cp(catch_type_index).asObjectType(cp))
    )
}
