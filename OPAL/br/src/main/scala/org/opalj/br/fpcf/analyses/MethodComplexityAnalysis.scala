    ...
    def apply(method: Method): MethodComplexity = {
        ...
        while (nextPCs.nonEmpty && complexity < maxComplexity) {
            ...
            (currentInstruction.opcode: @annotation.switch) match {
                ...
                case opcode       => throw new BytecodeProcessingFailedException(s"unsupported opcode: $opcode")
        ...
        if (complexity >= 0 && complexity <= maxComplexity) MethodComplexity(complexity)
        else MethodComplexity.TooComplex // <= Use a shared property to safe memory!
    }
}
