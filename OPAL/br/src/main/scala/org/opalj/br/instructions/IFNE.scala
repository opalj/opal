...
case class IFNE(branchoffset: Int) extends IF0Instruction[IFNE] with IFNELike {
    ...
    def negate(newBranchoffset: Int = branchoffset): IFEQ = IFEQ(newBranchoffset)

    def toLabeledInstruction(currentPC: PC): LabeledInstruction = LabeledIFNE(InstructionLabel(currentPC + branchoffset))
}
...
case class LabeledIFNE(
    ...
    override def negate(newJumpTargetLabel: InstructionLabel): LabeledIFEQ = LabeledIFEQ(newJumpTargetLabel)
}
