...
trait TypeAnnotationsBinding
    ...
    override def SupertypeTarget(
        supertype_index: Int): TAOfSupertype = TAOfSupertype(supertype_index)
    // ______________________________
    ...
    override def LocalvarDecl(localVarTable: LocalvarTable): TAOfLocalvarDecl = TAOfLocalvarDecl(localVarTable)

    override def ResourcevarDecl(localVarTable: LocalvarTable): TAOfResourcevarDecl = TAOfResourcevarDecl(localVarTable)
    ...
    override def MethodReferenceExpressionNew /*::New*/ (
        offset: Int): TAOfMethodReferenceExpressionNew = TAOfMethodReferenceExpressionNew(offset)

    override def MethodReferenceExpressionIdentifier /*::Identifier*/ (
        offset: Int): TAOfMethodReferenceExpressionIdentifier = TAOfMethodReferenceExpressionIdentifier(offset)

    // ______________________________
    // type_arguement_target
    override def CastExpression(
        offset:              Int,
        type_argument_index: Int): TAOfCastExpression = TAOfCastExpression(offset, type_argument_index)

    override def ConstructorInvocation(
        offset:              Int,
        type_argument_index: Int): TAOfConstructorInvocation = TAOfConstructorInvocation(offset, type_argument_index)

    override def MethodInvocation(
        offset:              Int,
        type_argument_index: Int): TAOfMethodInvocation = TAOfMethodInvocation(offset, type_argument_index)
    ...
    override def TypeAnnotationDirectlyOnType: TADirectlyOnType.type = TADirectlyOnType

    override def TypeAnnotationPath(
        path: ArraySeq[TypeAnnotationPathElement]): TAOnNestedType = TAOnNestedType(path)

    override def TypeAnnotationDeeperInArrayType: TADeeperInArrayType.type = TADeeperInArrayType

    override def TypeAnnotationDeeperInNestedType: TADeeperInNestedType.type = TADeeperInNestedType

    override def TypeAnnotationOnBoundOfWildcardType: TAOnBoundOfWildcardType.type = TAOnBoundOfWildcardType
