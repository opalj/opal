...
abstract class MethodAnalysisApplication extends ProjectAnalysisApplication {
    ...
    override def analysisSpecificParametersDescription: String = "-class=<the fully qualified name of the class>\n" +
    ...
    override def checkAnalysisSpecificParameters(parameters: Seq[String]): Iterable[String] = {
        ...
        parameters.foldLeft(List.empty[String]) { (notUnderstood, p) =>
            if (!p.startsWith("-class=") && !p.startsWith("-method=")) p :: notUnderstood
            else notUnderstood
        }
    }
