...
object PCAndInstruction {
    def apply(pc: Int /* PC */, i: Instruction): PCAndInstruction = new PCAndInstruction(pc, i)

    // TODO Figure out how large the performance overhead is, when we use unapply.
    def unapply(p: PCAndInstruction): Option[(Int /* PC */, Instruction)] = Some((p.pc, p.instruction))
}
