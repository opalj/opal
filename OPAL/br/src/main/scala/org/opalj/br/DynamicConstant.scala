...
final class DynamicConstant(
        ...
        private val bsmIndex: Int) extends ConstantValue[Any] {
    ...
    def bootstrapMethod: BootstrapMethod =
        if (bsm eq null) throw BytecodeProcessingFailedException("this dynamic constant is incomplete")
        else bsm

    override def value = throw BytecodeProcessingFailedException("value is dynamic")

    override def valueToString = (Iterator(name, descriptor.toJava + ".class") ++
