...
sealed trait FieldAccesses {
    final def partialResults(accessContext: Context): IterableOnce[PartialResult[_, _ >: Null <: Property]] =
        if (containsNoMethodBasedAccessInformation) partialResultsForFieldBasedFieldAccesses
        else Iterator(
    ...
    private[this] def containsNoMethodBasedAccessInformation = directReadFields.isEmpty &&
    ...
    private[this] def partialResultForAccessedFields[S >: Null <: MethodFieldAccessInformation[S]](
        ...
        fieldAccessesValueUpdater: S => S): PartialResult[Method, S] = {
        ...
        PartialResult[Method, S](
            ...
            propertyKey,
            {
                case InterimUBP(_) if containsNoMethodBasedAccessInformation => None

                case InterimUBP(ub) => Some(InterimEUBP(method, fieldAccessesValueUpdater(ub)))

                case _: EPK[_, _] if containsNoMethodBasedAccessInformation =>
                    Some(InterimEUBP(method, noFieldAccessesValue))

                case _: EPK[_, _] => Some(InterimEUBP(method, fieldAccessesValueUpdater(noFieldAccessesValue)))
    ...
    private[this] def partialResultForReadFields(
        accessContext: Context): PartialResult[Method, MethodFieldReadAccessInformation] =
    ...
    private[this] def partialResultForWriteFields(
        accessContext: Context): PartialResult[Method, MethodFieldWriteAccessInformation] =
        partialResultForAccessedFields(
            accessContext,
    ...
    protected def partialResultsForFieldBasedFieldAccesses
        : IterableOnce[PartialResult[DeclaredField, _ >: Null <: FieldAccessInformation[_]]] = Iterator.empty
}
...
trait CompleteFieldAccesses extends FieldAccesses {
    ...
    private def createFieldPartialResultForContext[S <: FieldAccessInformation[S]](
        field:       DeclaredField,
        propertyKey: PropertyKey[S],
        property:    S): PartialResult[DeclaredField, _ >: Null <: FieldAccessInformation[_]] =
        PartialResult[DeclaredField, FieldAccessInformation[S]](
            ...
            propertyKey,
            {
                case InterimUBP(ub) =>
                    ...
                    if (accessInformation eq ub) None
                    else Some(InterimEUBP(field, accessInformation))

                case _: EPK[_, _] => Some(InterimEUBP(field, property))
    ...
    private[this] var _partialResultsForFieldBasedFieldAccesses
        : List[PartialResult[DeclaredField, _ >: Null <: Property with FieldAccessInformation[_]]] = List.empty

    protected def addFieldReadAccess(
        pc:              Int,
        field:           DeclaredField,
        propertyFactory: () => FieldReadAccessInformation): Unit = {
    ...
    protected def addFieldWriteAccess(
        pc:              Int,
        field:           DeclaredField,
        propertyFactory: () => FieldWriteAccessInformation): Unit = {
    ...
    override protected def partialResultsForFieldBasedFieldAccesses
        : IterableOnce[PartialResult[DeclaredField, _ >: Null <: FieldAccessInformation[_]]] =
