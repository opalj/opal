...
object PropertyBounds {
    ...
    def apply(pbt: PropertiesBoundType, pks: Array[PropertyKind]): Set[PropertyBounds] = pbt match {
    ...
    def apply(pbt: PropertiesBoundType, pk: PropertyKind): PropertyBounds = pbt match {
    ...
    def finalP(pk: PropertyKind): PropertyBounds = new PropertyBounds(pk) {
    ...
    def lub(pk: PropertyKind): PropertyBounds = new PropertyBounds(pk) {
    ...
    def lb(pk: PropertyKind): PropertyBounds = new PropertyBounds(pk) {
        override def lowerBound: Boolean = true
        override def upperBound: Boolean = false
    }
    ...
    def ub(pk: PropertyKind): PropertyBounds = new PropertyBounds(pk) {
        override def lowerBound: Boolean = false
        override def upperBound: Boolean = true
    }
