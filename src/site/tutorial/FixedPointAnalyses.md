...
Fixed-point analyses in OPAL take their result values from explicitly defined lattices.  
...
Second, a trait for our lattice (we chose it to be an [OrderedProperty](/library/api/SNAPSHOT/org/opalj/fpcf/OrderedProperty.html) which provides additional checks for the correctness of our analyses):
```scala
...
```
We can define whatever we might need to easily handle the lattice values later one.  
Here, we define a `meet` function to compute the common lower bound of two values<a title="In OPAL, the most optimistic value is top, the most pessimistic value is bottom.\nThus, the meet function will return a value that over-approximates its two inputs."><sup>[note]</sup></a>.

Because we chose class immutability to be an `OrderedProperty`, we have to implement the `checkIsEqualOrBetterThan` method to throw an exception if the `other` value is more precise.  
Here, we can use our defined `meet` function to implement this easily.  
...
Next, we set up a singleton that allows us to statically refer to our lattice later on:
```scala
object ClassImmutability extends ClassImmutabilityPropertyMetaInformation { [...] }
```

Finally, we have to specify the individual lattice values, called *Properties*, of which we define three simple ones here:
...
Of course we have to provide the `meet` function that we defined earlier.

We now have to declare a few more values:

First, in the `PropertyMetaInformation`, we define the type of the lattice to be `ClassImmutability`:
```scala
sealed trait ClassImmutabilityPropertyMetaInformation extends PropertyMetaInformation {
...
Then, we create a new [PropertyKey](/library/api/SNAPSHOT/org/opalj/fpcf/PropertyKey.html) which we can later use to tag and query results with:
```scala
object ClassImmutability extends ClassImmutabilityPropertyMetaInformation {
...
```
The key consists of a unique name of our choice and a sound fallback value to be used when someone tries to retrieve a class immutability value without previously executing the corresponding analysis.


Finally, we provide access to this key also for all lattice values:
```scala
sealed trait ClassImmutability extends ClassImmutabilityPropertyMetaInformation with {
...
In order to implement the analysis itself, we create a class that extends [FPCFAnalysis](/library/api/SNAPSHOT/org/opalj/br/fpcf/FPCFAnalysis.html).
```scala
class ClassImmutabilityAnalysis(val project: SomeProject) extends FPCFAnalysis { [...] }
```
This requires a [Project](/library/api/SNAPSHOT/org/opalj/br/analyses/Project.html)<a title="SomeProject gets rid of Project's type parameter for you."><sup>[note]</sup></a> through which we can later access all information about the analyzed code.
...
```
We use [ClassFile](/library/api/SNAPSHOT/org/opalj/br/ClassFile.html) for the entity, as that represents a class.  
Using a [ProperPropertyComputationResult](/library/api/SNAPSHOT/org/opalj/fpcf/ProperPropertyComputationResult.html) signals that our analysis will always be able to return a result.
...
Instead of using a predefined `meet` function, here we check the possible values explicitly.
...
```
The continuation function gets an [EPS](/library/api/SNAPSHOT/org/opalj/fpcf/EPS.html)<a title="SomeEPS gets rid of EPS's type parameters for you."><sup>[note]</sup></a> which is an `EOptionP` that is guaranteed to have a value for the property.
...
The analysis itself is completed now, but we still need to tell OPAL how to use it.  
...
```
First we define that we derive upper bounds for `ClassImmutability`, i.e., that this is an optimistic analysis of class immutability.  
...
The analysis above uses no [ProjectInformationKey](/library/api/SNAPSHOT/org/opalj/br/analyses/ProjectInformationKey.html)<a title="ProjectInformationKeys provide aggregated information about a project, such as a call graph or the set of methods that access each field."><sup>[note]</sup></a>, but if it did, we had to specify them here.
...
Notice that above we defined a trait.  
We now extend this trait to implement an actual scheduler:
```scala
...
```
This is an [FPCFEagerAnalysisScheduler](/library/api/SNAPSHOT/org/opalj/br/fpcf/FPCFEagerAnalysisScheduler.html)<a title="The BasicFPCFEagerAnalysisScheduler provides an empty implementation for some rarely used methods."><sup>[note]</sup></a> that allows us to run the analysis for a pre-defined number of entities.  
...
Finally, we tell OPAL how to execute the analysis: We create a new analysis, then we tell the property store to eagerly call the `analyzeClassImmutability` method for all classfiles in the project and finally we return the analysis.
...
To do so easily, we extend [ProjectAnalysisApplication]() which provides us with a `main` method that parses parameters for us, most importantly the "-cp=<some path>" parameter that lets users specify the path to a project that they want to analyze.
```scala
...
You can get the complete, runnable source code for this analysis [here](ClassImmutabilityAnalysis.scala).
