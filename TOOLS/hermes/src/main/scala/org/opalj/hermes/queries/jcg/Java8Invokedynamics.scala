...
class Java8Invokedynamics(
        implicit
        val hermesConfig: HermesConfig) extends DefaultFeatureQuery {
                    ...
                    if (isJava10StringConcatInvokedynamic(invDyn)) {
                        ...
                        if (bm.handle.isInvokeStaticMethodHandle) {
                            ...
                            if (descriptor == MethodDescriptor.LambdaAltMetafactoryDescriptor &&
                                ...
                                if (code.instructions(nextPC).opcode == ARETURN.opcode) 8 /* Lambda2*/
                                else {
                                    ...
                                    if (users.exists(instructions(_).opcode == AASTORE.opcode)) 9 /* Lambda3 */
                                    else handleJava8InvokeDynamic(m, handle)
                                }
                            } else handleJava8InvokeDynamic(m, handle)
                        } else handleJava8InvokeDynamic(m, handle)
                    } else {
    ...
    private def handleJava8InvokeDynamic[S](m: Method, handle: MethodCallMethodHandle) = handle.referenceKind match {
