        ...
        for {
            ...
            val kindID = invokeKind match {
                case _ @INVOKESTATIC(declaringClass, _, _, _) => {
                    ...
                    if (cf.isEmpty) -1
                    else if (cf.get.isInterfaceDeclaration) 4 /* static interface receiver */
                    else 0                                    /* static call to class type*/
                }
                        ...
                        if (callTargetResult.isEmpty) {
                            ...
                            if (declType eq declaringClass) {
                                if (callTarget.isPrivate) {
                                    val cf = project.classFile(declaringClass)
                                    if (cf.isEmpty) -1
                                    else if (cf.get.isInterfaceDeclaration) 5 /* private interface method call */
                                    else 2                                    /* private method call */
                                } else {
                                    -1
                                }
                            } else {
