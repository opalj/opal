...
class Serialization(implicit hermes: HermesConfig) extends DefaultFeatureQuery {
    ...
    override def evaluate[S](
        ...
        val serializableTypes: Set[ObjectType] = classHierarchy.allSubtypes(ObjectType.Serializable, reflexive = false)
        ...
        for {
            ...
            pcAndInvocation <- body collect ({
                                   ...
                                   case i @ INVOKEVIRTUAL(declClass, "readObject", JustReturnsObject)
                                       if classHierarchy.isSubtypeOf(declClass, OIS) => i
                                   case i @ INVOKEVIRTUAL(declClass, "registerValidation", OISregisterValidation)
                                       if classHierarchy.isSubtypeOf(declClass, OIS) => i
            ...
            if (invocation.astID == VirtualMethodCall.ASTID) {
                if (invocation.asVirtualMethodCall.name == "writeObject") handleWriteObject(
                ...
                invocation.astID match {
                    case Assignment.ASTID => handleReadObject(
                    ...
                    case ExprStmt.ASTID => handleReadObjectWithoutCasts(
                            l,
                            serializableTypes,
                            externalizableTypes,
                            notExternalizableTypes
    ...
    def handleWriteObject[S](
        ...
        if (paramVar.definedBy.exists { defSite =>
                if (defSite >= 0) {
                    ...
                    expr.astID == InvokedynamicFunctionCall.ASTID && isLambdaMetafactoryCall(
                        expr.asInvokedynamicFunctionCall)
                } else false
            }) {
            locations(11) += l
            ...
            if (param.isNull.isNo &&
                ...
                !classHierarchy.isSubtypeOf(param.asReferenceType, ObjectType.Externalizable)) locations(0) += l
            else if (param.isNull.isNo &&
                     classHierarchy.isSubtypeOf(param.asReferenceType, ObjectType.Externalizable)) locations(9) += l

            if (param.isNull.isNo &&
                hasMethod(param.asReferenceType, "writeReplace", JustReturnsObject)) locations(5) += l
            ...
            if (notExternalizableTypes.exists { subtype => hasMethod(subtype, "writeObject", WriteObjectDescriptor) })
                locations(2) += l

            if (serializableTypes.exists { subtype => hasMethod(subtype, "writeReplace", JustReturnsObject) })
                locations(5) += l

            if (externalizableTypes.nonEmpty) locations(9) += l
        }
    }
    ...
    def handleReadObject[S](
        ...
        if (castTypes.isEmpty) {
            ...
            if (castTypes.exists { cType => hasMethod(cType, "readResolve", JustReturnsObject) }) locations(6) += l
    ...
    def handleReadObjectWithoutCasts[S](
        ...
        locations: Array[LocationsContainer[S]],
        project:   SomeProject): Unit = {
        if (notExternalizableTypes.exists { subtype => hasMethod(subtype, "readObject", ReadObjectDescriptor) })
            locations(3) += l

        if (serializableTypes.exists { subtype => hasMethod(subtype, "readResolve", JustReturnsObject) })
            locations(6) += l

        if (notExternalizableTypes.nonEmpty) locations(8) += l
