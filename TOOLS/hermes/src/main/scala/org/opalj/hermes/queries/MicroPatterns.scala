...
class MicroPatterns(implicit hermes: HermesConfig) extends FeatureQuery {
    ...
    override def apply[S](
        ...
        project:              Project[S],
        rawClassFiles:        Iterable[(org.opalj.da.ClassFile, S)]): IterableOnce[Feature[S]] = {
    ...
    def isDesignator(cl: ClassFile)(implicit project: SomeProject): Boolean = {
        if (cl.thisType == ObjectType.Object) return false;
        ...
        def isDesignatorType(ot: ObjectType): Boolean = project.classFile(ot).exists(this.isDesignator)

        cl.fields.isEmpty && {
            if (cl.isInterfaceDeclaration) cl.methods.isEmpty && cl.interfaceTypes.forall(isDesignatorType)
            else
     ...
     */
    def isTaxonomy(cl: ClassFile): Boolean = cl.fields.isEmpty && {
     ...
     */
    def isPool(cl: ClassFile): Boolean = cl.fields.nonEmpty && cl.fields.forall(f => f.isFinal && f.isStatic) &&
    ...
    def isFunctionPointer(cl: ClassFile): Boolean = !cl.isInterfaceDeclaration && !cl.isAbstract && cl.methods.count {
        m => !isInitMethod(m)
    ...
    def isCobolLike(cl: ClassFile): Boolean = !cl.methods.exists { m => !isInitMethod(m) } && cl.methods.count { m =>
        !isInitMethod(m) &&
        m.isStatic
    } == 1 &&
    ...
    def isImmutable(
        cl: ClassFile,
        fa: FieldAccessInformation
      )(implicit
        contextProvider: ContextProvider,
        declaredFields:  DeclaredFields): Boolean = !cl.isInterfaceDeclaration &&
    ...
    def isExtender[S](cl: ClassFile)(implicit theProject: Project[S]): Boolean = !cl.isInterfaceDeclaration &&
    ...
    def isDataManager[S](cl: ClassFile)(implicit theProject: Project[S]): Boolean = !cl.isInterfaceDeclaration &&
        !cl.isAbstract &&
        cl.fields.nonEmpty &&
        cl.methods.count(m => !isInitMethod(m) && !isObjectMethod(m)) > 1 &&
