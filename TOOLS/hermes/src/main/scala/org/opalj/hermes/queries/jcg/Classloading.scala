...
class Classloading(implicit hermes: HermesConfig) extends DefaultFeatureQuery {
    ...
    override def evaluate[S](
        ...
        val hasCustomClassLoaders = project.allClassFiles exists { cf =>
        ...
        for {
            ...
            if (hasCustomClassLoaders) locations(1) += l // potential custom classloader
            else locations(0) += l                       // standard java classloader
        }

        ArraySeq.unsafeWrapArray(locations)
    }

    def isStandardClassLoader(receiverType: ReferenceType): Boolean = receiverType.asObjectType.fqn.startsWith("java/")
}
