...
class Library(implicit hermes: HermesConfig) extends DefaultFeatureQuery {
    ...
    override def featureIDs: Seq[String] = Seq( // CBS = call-by-signature
            ...
            if (isParameterType) {
                if (invokeKind.opcode == INVOKEINTERFACE.opcode) {
                    ...
                    if (cfO.nonEmpty) {
                        ...
                        if (publicTarget) instructionLocations(2) += l

                        if (packagePrivateTarget) instructionLocations(3) += l

                        if (subclassTarget) instructionLocations(4) += l
                    }
                }
    ...
    def getCBSTargets(
        ...
        def analyzePotentialCBSTarget(cbsCallee: Method): Unit = {
            if (!cbsCallee.isPublic) return;

            if (cbsCallee.isAbstract) return;

            if (!isInheritableMethod(cbsCallee)) return;

            val cbsCalleeDeclaringClass = cbsCallee.classFile

            if (!cbsCalleeDeclaringClass.isClassDeclaration) return;

            if (cbsCalleeDeclaringClass.isEffectivelyFinal) return;

            val cbsCalleeDeclaringType = cbsCalleeDeclaringClass.thisType

            if (cbsCalleeDeclaringType eq ObjectType.Object) return;
            ...
            if (hasSubclassWhichInheritsFromInterface(cbsCalleeDeclaringType,
                                                      interfaceType,
                                                      methodName,
                                                      methodDescriptor,
                                                      project).isYes) return;
    ...
    @inline private[this] def isInheritableMethod(method: Method): Boolean = !method.isPrivate
            ...
            project.classFile(subtype) match {
                case Some(subclassFile) =>
                    if (subclassFile.findMethod(methodName, methodDescriptor).isEmpty
                        && classHierarchy.isSubtypeOf(subtype, interfaceType)) return Yes;
                case None => isUnknown = false
            }
        }
