...
class ClassFileVersion(implicit hermes: HermesConfig) extends FeatureQuery {
    ...
    override val featureIDs: Seq[String] = featureId(Java1MajorVersion) +: (
        for (majorVersion <- Java5MajorVersion to LatestSupportedJavaMajorVersion) yield featureId(majorVersion)
    )
    ...
    override def apply[S](
...

        {
            ...
            if (data(Java1MajorVersion) eq null) Feature[S](java1MajorVersionFeatureId, 0, List.empty)
            else Feature[S](java1MajorVersionFeatureId, extensions.size, extensions)
        } +: (
            for (majorVersion <- Java5MajorVersion to LatestSupportedJavaMajorVersion) yield {
                ...
                } else Feature[S](featureId, 0, List.empty)
            }
        )
    }
}
