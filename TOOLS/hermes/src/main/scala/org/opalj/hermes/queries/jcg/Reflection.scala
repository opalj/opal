...
class Reflection(implicit hermes: HermesConfig) extends DefaultFeatureQuery {
    ...
    val GetProperty2MD = MethodDescriptor(ArraySeq(ObjectType.String, ObjectType.String), ObjectType.String)
    ...
    override def evaluate[S](
        ...
        implicit val locations: Array[LocationsContainer[S]] = Array.fill(featureIDs.size)(new LocationsContainer[S])

        implicit val p: SomeProject = project

        implicit val tacai: Method => TACode[TACMethodParameter, V] = project.get(LazyTACUsingAIKey)
            ...
            if (pcAndInstruction.value.isMethodInvocationInstruction) {
                ...
                if (index != -1) {
                    ...
                    call.declaringClass match {
                        ...
                        case FieldT => if (stmt.astID == Assignment.ASTID)
                                if (fieldUsedForInvocation(index, stmt.asAssignment)) locations(6 /* Field.get */ ) += l
    ...
    def handleInvoke[S](
        ...
        locations: Array[LocationsContainer[S]]): Unit = {
        if (call.params.head.asVar.value.asReferenceValue.isNull.isYes) locations(0 /* static invoke */ ) += l
        else locations(1 /* instance invoke */ ) += l
                ...
                if (defSite < 0) {
                    ...
                    if (definition.isConst) {
                        ...
                        case VirtualFunctionCall(_, dc, _, "getProperty", GetProperty1MD, _, _)
                            if ch.isSubtypeOf(dc, PropertiesT) => locations(13 /* string from Properties */ ) += l
    ...
    def methodUsedForInvocation[S](
        ...
        stmts:   Array[Stmt[V]]): Boolean =
        if (stmt.targetVar.usedBy.exists { useSite =>
                val stmt = stmts(useSite)
                stmt.astID match {
                    case Assignment.ASTID => stmt.asAssignment.expr.isVirtualFunctionCall &&
                        stmt.asAssignment.expr.asVirtualFunctionCall.name == "invoke"
                    case ExprStmt.ASTID => stmt.asExprStmt.expr.isVirtualFunctionCall &&
                        stmt.asExprStmt.expr.asVirtualFunctionCall.name == "invoke"
                    case _ => false
                }
            }) {
            true
        } else stmt.targetVar.usedBy.exists { useSite =>
            val stmt = stmts(useSite)
            stmt.astID match {
                ...
                case Assignment.ASTID => stmt.asAssignment.expr.astID match {
                        case VirtualFunctionCall.ASTID | NonVirtualFunctionCall.ASTID |
                            StaticFunctionCall.ASTID => mayUse(stmt.asAssignment.expr.asFunctionCall.params, pc) &&
                            projectContainsNonLocalCall(MethodT, "invoke")
                        case InstanceOf.ASTID | Compare.ASTID => false
                        case _                                => projectContainsNonLocalCall(MethodT, "invoke")
                    }
                case ExprStmt.ASTID => stmt.asExprStmt.expr.astID match {
                        case VirtualFunctionCall.ASTID | NonVirtualFunctionCall.ASTID |
                            StaticFunctionCall.ASTID => mayUse(stmt.asExprStmt.expr.asFunctionCall.params, pc) &&
                            projectContainsNonLocalCall(MethodT, "invoke")
                        case InstanceOf.ASTID | Compare.ASTID => false
                        case _                                => projectContainsNonLocalCall(MethodT, "invoke")
                    }
    ...
    def fieldUsedForInvocation[S](
        ...
        if (assignment.targetVar.usedBy.exists { useSite =>
                val stmt = stmts(useSite)
                stmt.astID match {
                    case VirtualMethodCall.ASTID => stmt.asVirtualMethodCall.receiver.asVar.definedBy.contains(pc)
                    case Assignment.ASTID => stmt.asAssignment.expr.isVirtualFunctionCall &&
                ...
                stmt.astID match {
                    ...
                    case Assignment.ASTID => stmt.asAssignment.expr.astID match {
                            case InstanceOf.ASTID | Compare.ASTID |
                                GetField.ASTID | ArrayLoad.ASTID => false
                            case _ => true
                        }
                    case ExprStmt.ASTID => stmt.asExprStmt.expr.astID match {
                            case InstanceOf.ASTID | Compare.ASTID |
            ...
            stmt.targetVar.usedBy.exists { useSite =>
                ...
                stmt.astID match {
                    case VirtualMethodCall.ASTID | NonVirtualMethodCall.ASTID |
                        StaticMethodCall.ASTID => mayUse(stmt.asMethodCall.params, pc) &&
                    ...
                    case Assignment.ASTID => stmt.asAssignment.expr.astID match {
                            case VirtualFunctionCall.ASTID | NonVirtualFunctionCall.ASTID |
                                ...
                                projectContainsNonLocalCall(FieldT, "get")
                            case InstanceOf.ASTID | Compare.ASTID => false
                            case _                                => projectContainsNonLocalCall(FieldT, "get")
                        }
                    case ExprStmt.ASTID => stmt.asExprStmt.expr.astID match {
                            case VirtualFunctionCall.ASTID | NonVirtualFunctionCall.ASTID |
                                StaticFunctionCall.ASTID => mayUse(stmt.asExprStmt.expr.asFunctionCall.params, pc) &&
    ...
    def mayUse(uvars: Seq[Expr[V]], pc: Int): Boolean = uvars.exists(_.asVar.definedBy.contains(pc))
    ...
    def projectContainsNonLocalCall(
        ...
        project: SomeProject,
        tacai:   Method => TACode[TACMethodParameter, V]): Boolean = project.allMethodsWithBody.exists { method =>
        ...
        if (invokes.isEmpty) {
            ...
            invokes.exists { pcAndInvocation =>
                ...
                val call =
                    if (stmt.astID == Assignment.ASTID) stmt.asAssignment.expr.asVirtualFunctionCall
                    else stmt.asExprStmt.expr.asVirtualFunctionCall
