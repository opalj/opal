...
class FanInFanOut(implicit hermes: HermesConfig) extends FeatureQuery {
    ...
    case class FeatureConfiguration(
        ...
        def featureIndex(value: Int): Int = Math.min(value / categorySize.toInt, _maxFeatureIndex) + offset

        def featureIndex(value: Double): Int = Math.min((value / numCategories).toInt, _maxFeatureIndex) + offset
    }
    ...
    object FeatureConfiguration {
        ...
        def apply(
            ...
            offset:              Int): FeatureConfiguration = {
        ...
        private[this] def parseNumCategories(
            ...
            config: Config): Option[Int] = {
            val numCategories = config.getInt(categoriesKey)
            if (numCategories > 0) Some(numCategories)
            else {
                val message = FanInFanOutConfigPrefix + categoriesKey +
        ...
        private[this] def parseCategorySize(
            ...
            config: Config): Option[Double] = {
            val categorySize = config.getDouble(categorySizeKey)
            if (categorySize > 0) {
                Some(categorySize)
            } else {
                val message = FanInFanOutConfigPrefix + categorySizeKey +
                    " setting invalid - value <= 0; category size has been set to default"
                OPALLogger.warn(logCategory, message)
                None
            }
            ...
            cpEntries.foreach { cpEntry =>
                ...
                if (typeInfo.charAt(0) == '(') {
                    ...
                    referencedTypes ++= md.parameterTypes.foldLeft(Set.empty[Int])((res, p) =>
                        if (p.isObjectType) res + p.asObjectType.id
                        else res)
                    if (md.returnType.isObjectType) referencedTypes += md.returnType.asObjectType.id
                } else {
                    try {
                        val ft = FieldType(typeInfo)
                        if (ft.isObjectType) referencedTypes += ft.asObjectType.id
                    } catch {
    ...
    @tailrec private[this] def getTypeInfo(
      ...
      )(implicit constant_pool: Constant_Pool): String = constant_Pool_Entry match {
        case CONSTANT_Class_info(name_index)                  => getTypeInfo(constant_pool(name_index))
        case CONSTANT_Fieldref_info(_, name_and_type_index)   => getTypeInfo(constant_pool(name_and_type_index))
        case CONSTANT_MethodHandle_info(_, reference_index)   => getTypeInfo(constant_pool(reference_index))
        case CONSTANT_Methodref_info(_, class_index)          => getTypeInfo(constant_pool(class_index))
        case CONSTANT_InterfaceMethodref_info(_, class_index) => getTypeInfo(constant_pool(class_index))
        case CONSTANT_NameAndType_info(_, descriptor_index)   => getTypeInfo(constant_pool(descriptor_index))
