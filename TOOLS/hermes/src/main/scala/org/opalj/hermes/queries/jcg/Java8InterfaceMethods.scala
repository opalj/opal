...
class Java8InterfaceMethods(implicit hermes: HermesConfig) extends DefaultFeatureQuery {
    ...
    override def featureIDs: Seq[String] = Seq( /* IDM = interface default method */
        ...
        for {
            ...
            val kindID = invokeKind match {
                case ii @ INVOKEINTERFACE(dc, name, md) => {
                    ...
                    if (hasDefaultMethodTarget) 0 /* a default method can be a target */
                    else {
                ...
                case iv @ INVOKEVIRTUAL(dc, name, md) => {
                    ...
                    if (hasDefaultMethodTarget) {
                        if (subtypeWithMultipleInterfaces) 4 /* the type inherits multiple interface with IDMs */
                        else 3                               /* a default method can be a target */
                    } else {
    ...
    private[this] def isPotentialCallOnDefaultMethod[S](
        ...
        if (!t.isObjectType) return false;
        ...
        if (relInvokeCache.containsKey(invokeID)) return relInvokeCache.get(invokeID);
